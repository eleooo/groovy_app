...
public class DifferenceErrorMsgHandlerQA {

	static Logger logger = Logger.getLogger(DifferenceErrorMsgHandlerQA.class.getName());
	static Object lockObj = new Object();
	
	private static String errorExcelPath = null;
	private static String errorExcelDBlogPath = null;
	private static String errorDiffFolderString = "";
//	private static Configuration config;
	...
	public boolean logDifference(String inputFilePath, String expectedFilePath,
		...
		synchronized(lockObj) {
				...
				if (DifferenceErrorMsgHandlerQA.errorExcelPath == null) {
					errorDiffFolderString = autoLogExcel.getParent();
					...
					if (!copyExcel(new File(CSB2BEDIConfigQA.ModelFileFolder+"/Difference-Log-Model.xlsm"),
							new File(errorDiffFolderString),autoLogExcelName))// (recordModel.getTpWorkingFolder()).replace("/"+
					// recordModel.getTpID(),
					// "")
					{
				...
				try { 
					writeDifInfo(expectedFilePath, actualFilePath,
							inputFilePath, errorDiffFolderString);
				...
				logger.error(">>> logDifference - Unknow fatal exception, plesae contact DEV to check, with your log.");
				logger.error("DifferenceErrorMsgHandlerQA.logDifference", e);
				logger.error("inputFilePath: "+inputFilePath+", expectedFilePath: "+expectedFilePath+", actualFilePath: "+actualFilePath);
				System.exit(1);
			} 
		}
		return successFlag;
	...
	public void writeDif(String e, String a) throws Exception {
		File excel = new File(errorExcelPath);
	...
	public void writeDBDifInfo(String tpID, String msgReqID,String infilename,
		...
		File excel = new File(errorExcelDBlogPath);
		if (excel.renameTo(excel)) {
			XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(excel));
			XSSFCellStyle style1 = workbook.createCellStyle();
			style1.setBorderBottom(XSSFCellStyle.BORDER_THIN); //
	...
	public boolean copyExcel(File f, File dir, String autoLogExcelName) {
		...
		if (!f.isFile()) {
			...
			try {
				...
				String newfileName = "Difference-Log-ErrorMsg-" +autoLogExcelName+"-"+ stime + ".xlsm";
				...
				finpen.close();
				foutpen.close();
				errorExcelPath = dir + "/" + newfileName;
				return true;
			} catch (Exception e) {
				logger.error(">>> copyExcel - Unknow fatal exception, plesae contact DEV to check, with your log.");
				logger.error("DifferenceErrorMsgHandlerQA.copyExcel", e);
	...
	public String copyExcelWithName(File f, File dir,String filename) {
		...
		if (!f.isFile()) {
			...
			try {
				...
				logger.error(">>> copyExcelWithName - Unknow fatal exception, plesae contact DEV to check, with your log.");
				logger.error("DifferenceErrorMsgHandlerQA.copyExcelWithName", e);
				logger.error("f: "+f.getAbsolutePath()+", dir: "+dir.getAbsolutePath());
				System.exit(1);
			}
		}
