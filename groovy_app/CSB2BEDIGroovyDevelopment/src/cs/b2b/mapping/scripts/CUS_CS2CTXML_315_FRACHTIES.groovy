...
/**
 * @author LIYU
 * FRACHTIES CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_FRACHTIES {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('FRACHTIES','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('FRACHTIES', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		//def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('FRACHTIES', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			//			select * from b2b_edi_cde_ref where tp_id = 'FRACHTIES' and convert_type_id = 'ExtReferenceCode' and seg_id = 'N9' and seg_num = '01-128'
			def extRefMap = ['FN':'FN', 'INV':'IN', 'PRN':'PO', 'SC':'E8', 'SID':'SI', 'SO':'SO', 'SR':'SR']
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('FRACHTIES','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
		...
		//	def uniqueAAO = []
			...
			list.each{current_Party ->
				...
				if (customerType == 'CN' || customerType == 'SH') {
					'N9'{
						//						'E128_01' getLegalPartyCodeTypeExtCDE('FRACHTIES','INTCTXML','315','O','LegalPartyCodeType', customerType,conn)
						if (customerType == 'CN') {
							'E128_01' '4E'
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('FRACHTIES', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
