 ...
 * 
 * @author YINEM
 * @shared by FORTFORWARD & SDV
 * @pattern after CHALLENGERDSG(DSG_STD3)
 */
public class CUS_CS2BCXML_301_SDV_STD1 {
	...
	Map<String,String> referenceTypeMap = ['BL':'BM','CR':'CR', 'CTR':'CT', 'FR':'FN', 'ICR':'CR', 'INV':'IN', 'PO':'PO', 'QUOT':'Q1', 'SC':'E8', 'SID':'SI', 'SR':'SR', 'TARIF':'TS']
			...
			'Loop_Y4' {
				'Y4' {
					...
					if(emptyCntrList){
						...
						def def309 = ''
						if(emptyCntr.Address?.LocationCode?.UNLocationCode){
							def309 = 'UN'
						}
						else if(emptyCntr.Facility?.FacilityCode){
							def309 = 'ZZ'
						}
						else if(emptyCntr.Address?.City){
							def309 = 'CI'
						}
						'E309_08' def309
					}
