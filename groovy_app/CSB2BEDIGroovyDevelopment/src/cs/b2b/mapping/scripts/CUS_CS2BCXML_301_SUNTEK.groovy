 ...
 */
public class CUS_CS2BCXML_301_SUNTEK {
	...
	Map bookingStatusMap = ['Confirmed':'A','Pending':'P','Wait Listed':'B','Cancelled':'D','No Show':'S','Declined':'R','Rejected':'R']
	
	Map<String,String> referenceTypeMap = ['BL':'BM','CR':'CR', 'CTR':'CT', 'FR':'FN', 'INV':'IK', 'PO':'PO','SID':'SI', 'TARIF':'TS']
...
//				'N9' {
//					'E128_01' 'BM'
//					String defBL = current_Body?.GeneralInformation?.BLNumber?.first()
//					'E127_02' util.substring(defBL, 1, 30)
//				}
//			} else {
//				ExternalReference defRefBL = current_Body?.ExternalReference?.find(){it.CSReferenceType == 'BL' && it.ReferenceNumber}
//				if (defRefBL) {
//					'N9' {
//						'E128_01' 'BM'
//						'E127_02' util.substring(defRefBL.ReferenceNumber, 1, 30)
//					}
//				}
//			}

			////////N1 start
			Map N1Map = ['ANP':'N1', 'BPT':'BK', 'CGN':'CN', 'FWD':'FW', 'NPT':'N2', 'SHP':'SH']
			...
			current_Body?.Party?.each { current_Party ->
				if (N1Map.get(current_Party.PartyType) && current_Party.PartyName) {
					...
					'Loop_N1' {
						...
						if (current_Party.Contact?.ContactFax?.Number) {
							'G61' {
								'E366_01' 'IC'
								'E93_02' defContact
								'E365_03' 'FX'
								'E364_04'  util.substring(current_Party.Contact?.ContactFax?.Number, 1, 80)?.trim()
								'E443_05' ''
							}
						}
					}
			...
			//////
			//looptype=loop_carrier
			'Loop_N1' {
				'N1' {
					'E98_01' 'CA'
					'E93_02' current_Body.GeneralInformation?.SCACCode
					'E66_03' '25'
					'E67_04' current_Body.GeneralInformation?.SCACCode
					'E706_05' ''
					'E98_06' ''
				}
			}
			//////
			//Map ioResultMap = ['C':'D','M':'P']
			//def ioString =''
			//ioString = ioResultMap.get(current_Body.Route?.Haulage?.OutBound) + ioResultMap.get(current_Body.Route?.Haulage?.InBound)
			//   ioString == 'PP' || ioString =='PD'
			
			def routeOutbound = (current_Body.Route?.Haulage?.OutBound==null?"":current_Body.Route?.Haulage?.OutBound.trim())
			def routeInbound = (current_Body.Route?.Haulage?.InBound==null?"":current_Body.Route?.Haulage?.InBound.trim())
			if (routeOutbound=='M' && (routeInbound=='M' || routeInbound=='C')) {
				def current_EmptyPickup= current_Body?.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup[0]
				//looptype=loop_EmptyPickup
				if (current_EmptyPickup?.Facility?.FacilityName) {
					'Loop_N1' {
						'N1' {
							'E98_01' 'PW'
							'E93_02' util.substring(current_EmptyPickup?.Facility?.FacilityName?.trim(), 1, 60)?.trim()
							'E66_03' ''
							'E67_04' ''
							'E706_05' ''
							'E98_06' ''
						...
						if(current_EmptyPickup?.Address?.AddressLines?.AddressLine) {
							if (StringUtil.isNotEmpty(current_EmptyPickup.Address.AddressLines.AddressLine[0])) {
								'N3' {
									'E166_01' util.substring(current_EmptyPickup.Address.AddressLines.AddressLine[0], 1, 55)?.trim()
									if (current_EmptyPickup.Address.AddressLines.AddressLine[0].length()>55) {
										'E166_02' util.substring(current_EmptyPickup.Address.AddressLines.AddressLine[0], 56, 55)?.trim()
									}
								}
							}
							if (current_EmptyPickup.Address.AddressLines.AddressLine[0].length() > 110) {
								'N3' {
									'E166_01' util.substring(current_EmptyPickup.Address.AddressLines.AddressLine[0], 111, 55)?.trim()
									if(current_EmptyPickup.Address.AddressLines.AddressLine[0].length()>165){
										'E166_02' util.substring(current_EmptyPickup.Address.AddressLines.AddressLine[0], 166, 55)?.trim()
									}
								}
							}
						}
						'N4' {
							'E19_01' current_EmptyPickup?.Address?.City
							def defStateCde = util.getCS2MasterCityStateCodeByStateCntryNoEmptyStateCde(current_EmptyPickup.Address?.State, current_EmptyPickup.Address?.Country, conn)
							if (defStateCde) {
								'E156_02' defStateCde
							} else {
								'E156_02' util.substring(current_EmptyPickup?.Address?.State?.toUpperCase(), 1, 2)
							}
							'E116_03' current_EmptyPickup?.Address?.PostalCode?.trim()
							if (current_EmptyPickup?.Address?.Country?.length() < 4) {
								'E26_04' current_EmptyPickup?.Address?.Country?.toUpperCase()?.trim()
							} else {
								'E26_04' countryMap.get(current_EmptyPickup?.Address?.Country?.toUpperCase())?.trim()
							}
							'E309_05' ''
							'E310_06' ''
						}
				...
				def current_FullReturn= current_Body?.ContainerGroup?.ContainerFlowInstruction?.FullReturn[0]
				//looptype=loop_FullReturn
				if (current_FullReturn?.Facility?.FacilityName) {
					'Loop_N1' {
						'N1' {
							'E98_01' 'CR'
							'E93_02' util.substring(current_FullReturn?.Facility?.FacilityName, 1, 60)?.trim()
							'E66_03' ''
							'E67_04' ''
							'E706_05' ''
							'E98_06' ''
						...
						if (current_FullReturn?.Address?.AddressLines?.AddressLine) {
							if (StringUtil.isNotEmpty(current_FullReturn.Address.AddressLines.AddressLine[0])) {
								'N3' {
									'E166_01' util.substring(current_FullReturn.Address.AddressLines.AddressLine[0],1,55)?.trim()
									if (current_FullReturn.Address.AddressLines.AddressLine[0].length()>55) {
										'E166_02' util.substring(current_FullReturn.Address.AddressLines.AddressLine[0],56,55)?.trim()
									}
								}
							}
							if (current_FullReturn.Address.AddressLines.AddressLine[0].length()>110) {
								'N3' {
									'E166_01' util.substring(current_FullReturn.Address.AddressLines.AddressLine[0],111,55)?.trim()
									if (current_FullReturn.Address.AddressLines.AddressLine[0].length()>165) {
										'E166_02' util.substring(current_FullReturn.Address.AddressLines.AddressLine[0],166,55)?.trim()
									}
								}
							}
						}
						'N4' {
							'E19_01' current_FullReturn?.Address?.City
							def defStateCde = util.getCS2MasterCityStateCodeByStateCntryNoEmptyStateCde(current_FullReturn.Address?.State, current_FullReturn.Address?.Country, conn)
							if (defStateCde) {
								'E156_02' defStateCde
							} else {
								'E156_02' util.substring(current_FullReturn?.Address?.State?.toUpperCase(), 1, 2)?.trim()
							}
							'E116_03' current_FullReturn?.Address?.PostalCode?.trim()
							if (current_FullReturn?.Address?.Country?.length() < 4) {
								'E26_04' current_FullReturn?.Address?.Country?.toUpperCase()?.trim()
							} else {
								'E26_04' countryMap.get(current_FullReturn?.Address?.Country?.toUpperCase())?.trim()
							}
							'E309_05' ''
							'E310_06' ''
						}
			...
			//Map ioResultMap = ['C':'D','M':'P']
			//ioString = ioResultMap.get(current_Body.Route?.Haulage?.OutBound)+ioResultMap.get(current_Body.Route?.Haulage?.InBound)
			//if (ioString == 'DD'||ioString == 'DP') {
			
			if (routeOutbound=='C' && (routeInbound=='C' || routeInbound=='M')) {
				//////////////////////// PickupAppointment ///////////////////////////////////
				cs.b2b.core.mapping.bean.bc.Appointment current_PickupAppointment = current_Body?.Appointment?.find{it.Type=='PICKUP'}
				//looptype=loop_Appointment
				if (current_PickupAppointment) {
					'Loop_N1' {
						'N1' {
							'E98_01' 'SF'
							'E93_02' 'PICKUP'
						}
						if (current_PickupAppointment?.Address?.AddressLines?.AddressLine) {
							def addressLineToSplit = current_PickupAppointment?.Address?.AddressLines?.AddressLine[0]?.trim()
							def addressLineToCheck = current_PickupAppointment?.Address?.AddressLines?.AddressLine[0]?.trim()
							for (i in 1..4) {
								if(current_PickupAppointment?.Address?.AddressLines?.AddressLine[i]) {
									addressLineToCheck = addressLineToCheck + current_PickupAppointment?.Address?.AddressLines?.AddressLine[i].trim()
									addressLineToSplit = addressLineToSplit + " "  + current_PickupAppointment?.Address?.AddressLines?.AddressLine[i].trim()
								} else {
									addressLineToSplit = addressLineToSplit + " "
								}
							}
						...
						'N4' {
							'E19_01' util.substring(current_PickupAppointment?.Address?.City,1,30)?.trim()
							'E156_02' util.getCS2MasterCityStateCodeByStateCntryNoEmptyStateCde(current_PickupAppointment?.Address?.State, current_PickupAppointment?.Address?.Country, conn)
							'E116_03' current_PickupAppointment?.Address?.PostalCode?.trim()
							if (current_PickupAppointment?.Address?.Country?.length() < 4) {
								'E26_04' current_PickupAppointment?.Address?.Country?.trim()
							} else {
								'E26_04' countryMap.get(current_PickupAppointment?.Address?.Country?.toUpperCase())?.trim()
							}
							'E309_05' ''
							'E310_06' ''
						}
						if (current_PickupAppointment?.Contact?.ContactPhone?.Number) {
							'G61' {
								'E366_01' 'IC'
								if (current_PickupAppointment?.Contact?.FirstName||current_PickupAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_PickupAppointment?.Contact?.FirstName?:'')+' '+(current_PickupAppointment?.Contact?.LastName?:''), 1, 60)?.trim()
								} else if (current_PickupAppointment?.Contact.ContactEmailAddress.indexOf('@')>=0) {
									'E93_02' util.substring(current_PickupAppointment?.Contact.ContactEmailAddress,1,current_PickupAppointment?.Contact.ContactEmailAddress.indexOf('@'))?.trim()
								}
								'E365_03' 'TE'
								'E364_04' util.substring(current_PickupAppointment?.Contact?.ContactPhone?.Number,1,80)?.trim()
								'E443_05' ''
							}
						}
						if (current_PickupAppointment?.Contact?.ContactEmailAddress) {
							'G61' {
								'E366_01' 'IC'
								if (current_PickupAppointment?.Contact?.FirstName||current_PickupAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_PickupAppointment?.Contact?.FirstName?:'')+' '+(current_PickupAppointment?.Contact?.LastName?:''),1,60)?.trim()
								...
								'E365_03' 'EM'
								'E364_04' util.substring(current_PickupAppointment?.Contact?.ContactEmailAddress,1,80)?.trim()
								'E443_05' ''
							}
						}
						if (current_PickupAppointment?.Contact?.ContactFax?.Number) {
							'G61' {
								'E366_01' 'IC'
								if (current_PickupAppointment?.Contact?.FirstName||current_PickupAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_PickupAppointment?.Contact?.FirstName?:'')+' '+(current_PickupAppointment?.Contact?.LastName?:''),1,60)?.trim()
								...
								'E365_03' 'FX'
								'E364_04' util.substring(current_PickupAppointment?.Contact?.ContactFax?.Number,1,80)?.trim()
								'E443_05' ''
							}
						}
					}
				...
				//////////////////////// DeliveryAppointment ///////////////////////////////////
				cs.b2b.core.mapping.bean.bc.Appointment current_DeliveryAppointment = current_Body?.Appointment?.find{it.Type=='DELIVERY'}
				if (current_DeliveryAppointment) {
					//looptype=loop_Appointment
					'Loop_N1' {
						'N1' {
							'E98_01' 'ST'
						...
						if (current_DeliveryAppointment?.Address?.AddressLines?.AddressLine) {
							def addressLineToSplit = current_DeliveryAppointment?.Address?.AddressLines?.AddressLine[0]?.trim()
							def addressLineToCheck = current_DeliveryAppointment?.Address?.AddressLines?.AddressLine[0]?.trim()
							for (i in 1..4) {
								if(current_DeliveryAppointment?.Address?.AddressLines?.AddressLine[i]) {
									addressLineToCheck = addressLineToCheck + current_DeliveryAppointment?.Address?.AddressLines?.AddressLine[i].trim()
									addressLineToSplit = addressLineToSplit + " "  + current_DeliveryAppointment?.Address?.AddressLines?.AddressLine[i].trim()
								} else {
									addressLineToSplit = addressLineToSplit + " "
								}
							}
						...
						'N4' {
							'E19_01' util.substring(current_DeliveryAppointment?.Address?.City,1,30)?.trim()
							'E156_02' util.getCS2MasterCityStateCodeByStateCntryNoEmptyStateCde(current_DeliveryAppointment?.Address?.State, current_DeliveryAppointment?.Address?.Country, conn)
							'E116_03' current_DeliveryAppointment?.Address?.PostalCode?.trim()
							if (current_DeliveryAppointment?.Address?.Country?.length()<4) {
								'E26_04' current_DeliveryAppointment?.Address?.Country?.trim()
							} else {
								'E26_04' countryMap.get(current_DeliveryAppointment?.Address?.Country?.toUpperCase())
							}
							'E309_05' ''
							'E310_06' ''
						}
						if (current_DeliveryAppointment?.Contact?.ContactEmailAddress) {
							'G61' {
								'E366_01' 'IC'
								if (current_DeliveryAppointment?.Contact?.FirstName || current_DeliveryAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_DeliveryAppointment?.Contact?.FirstName?:'')+' '+(current_DeliveryAppointment?.Contact?.LastName?:''), 1, 60)?.trim()
								} else if (current_DeliveryAppointment?.Contact.ContactEmailAddress.indexOf('@')>=0) {
									'E93_02' util.substring(current_DeliveryAppointment?.Contact.ContactEmailAddress, 1, current_DeliveryAppointment?.Contact.ContactEmailAddress.indexOf('@'))?.trim()
								}
								'E365_03' 'EM'
								'E364_04' util.substring(current_DeliveryAppointment?.Contact?.ContactEmailAddress,1,80)?.trim()
								'E443_05' ''
							}
						}
						if (current_DeliveryAppointment?.Contact?.ContactPhone?.Number) {
							'G61' {
								'E366_01' 'IC'
								if (current_DeliveryAppointment?.Contact?.FirstName||current_DeliveryAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_DeliveryAppointment?.Contact?.FirstName?:'')+' '+(current_DeliveryAppointment?.Contact?.LastName?:''), 1, 60)?.trim()
								...
								'E365_03' 'TE'
								'E364_04' util.substring(current_DeliveryAppointment?.Contact?.ContactPhone?.Number,1,80)?.trim()
								'E443_05' ''
							}
						}
						if (current_DeliveryAppointment?.Contact?.ContactFax?.Number) {
							'G61' {
								'E366_01' 'IC'
								if (current_DeliveryAppointment?.Contact?.FirstName || current_DeliveryAppointment?.Contact?.LastName) {
									'E93_02' util.substring((current_DeliveryAppointment?.Contact?.FirstName?:'') + ' ' + (current_DeliveryAppointment?.Contact?.LastName?:''), 1, 60)?.trim()
								...
								'E365_03' 'FX'
								'E364_04' util.substring(current_DeliveryAppointment?.Contact?.ContactFax?.Number,1,80)?.trim()
								'E443_05' ''
							}
						}
					}
			...
			current_Body?.Cargo?.eachWithIndex{ current_Cargo, index->
				'Loop_LX' {
					...
					'L0' {
						...
						'E458_10' ''
						'E188_11' util.substring(current_Cargo.GrossWeight?.WeightUnit?.toUpperCase(), 1, 1)
						'E56_12' ''
						'E380_13' ''
						'E211_14' ''
						'E1073_15' ''
...
//				'N9' {
//					'E128_01' 'BM'
//					'E127_02' util.substring(current_Body.GeneralInformation.BLNumber[0], 1, 30)
//				}
//			} else if (current_Body?.AssociatedExternalReference?.find{it.CSReferenceType=='BL'}?.ReferenceNumber) {
//				'N9' {
//					'E128_01' 'BM'
//					'E127_02' util.substring(current_Body.AssociatedExternalReference.find{it.CSReferenceType=='BL'}.ReferenceNumber, 1, 30)
//				}
//			}
//			
//			current_Body?.AssociatedExternalReference?.each { current_AssociatedExternalReference ->
//				if (referenceTypeMap.get(current_AssociatedExternalReference.CSReferenceType)) {
//					'N9' {
//						'E128_01' referenceTypeMap.get(current_AssociatedExternalReference.CSReferenceType)
//						'E127_02' util.substring(current_AssociatedExternalReference.ReferenceNumber, 1, 30)
//					}
//				}
//			}
//			'Loop_R4' {
