 ...
 */
class CUS_CS2CTXML_D99B_AGILITY {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml) {
        ...
        def contianerTypeConvertion = util.getConversionWithScac('AGILITY','CT','O','ContainerType',current_Body?.Container?.CarrCntrSizeType,SCAC,conn)
        outXml.'Group_UNH' {
            'UNH' {
                'E0062_01' '-999'
                'S009_02' {
            ...
            'Group4_CNI' {
                ...
                'Group5_STS' {
                    'STS' {
                        ...
                        def queryEventConversion = util.getCdeConversionFromIntCde('AGILITY','CT','O','<QUERY-NULL-SCAC>','EDIFACT','EventStatusCode',vCS1Event,conn)
                        'C555_02' {
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
        ...
        ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getCdeConversionFromIntCde('AGILITY', 'CT', 'O','<QUERY-NULL-SCAC>','EDIFACT', 'EventStatusCode', eventCS2Cde, conn).get('EXT_CDE')
