...
import cs.b2b.core.mapping.bean.ct.EventDT
...
class CUS_CS2CTXML_XML_WGLL {
    ...
    public void generateBody(Body current_Body, Header ctHeader, MarkupBuilder outXml) {
        ...
        def vCS1EventCodeConversion = util.getConversionWithScac('WGLL', 'CT', 'O', 'EventCodeType', vCS1Event,current_Body?.GeneralInformation?.SCAC, conn)
        def vCS1EventDescription = util.getConversionWithScac('WGLL', 'CT', 'O', 'EventCodeDescType', vCS1Event,current_Body?.GeneralInformation?.SCAC, conn)
        ...
        outXml.'CargoTracking'{
            ...
            'SummaryDetails'{
                ...
                current_Body?.MileStones?.each {current_MilesStones ->
                    def mlesStoneEventConversion =  util.getConversionWithScac('WGLL', 'CT', 'O', 'EventCodeType', current_MilesStones?.CS1Event,current_Body?.GeneralInformation?.SCAC, conn)
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
        ...
        ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getConversionWithScac('WGLL', 'CT', 'O', 'EventCodeType', eventCS2Cde,current_Body?.GeneralInformation?.SCAC,conn)
            //prep checking
...
//            pospValidation()
        ...
        if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
            ...
            String outputFileName = "WGLL.CT." + currentDate.format("yyyyMMddHHmmss") + ".xml"
