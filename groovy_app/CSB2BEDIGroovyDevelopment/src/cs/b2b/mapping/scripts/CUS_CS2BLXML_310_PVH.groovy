...
/**
* @author XIEMI
* @pattern after TP=ACUCUBEDOREL
*/
public class CUS_CS2BLXML_310_PVH {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				def ChargeType = current_Body?.FreightCharge[0]?.ChargeType;
				if(ChargeType?.equals(blUtil.COLLECT) || ChargeType?.equals(blUtil.PREPAID)){
					'E146_04' 'CC'
				}
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['BKG':'BN','CGO':'CG','CTR':'CT','SID':'SI']
						...
						'Loop_N7' {
							...
							if(current_cargoList[0]?.ReeferCargoSpec?.size() > 0){
								...
								'W09' {
									...
									String temperature = current_ReeferCargoSpec?.Temperature?.Temperature?.toString()
									temperature = temperature?.length() < 4 ? temperature :temperature.substring(0,4)
									def signal = null
									if(temperature?.contains(".")){   //different
										def temperatureAfterPoint = temperature?.substring(temperature?.indexOf(".")+1)
										...
										String Temperature2 = current_ReeferCargoSpec?.Temperature?.Temperature
										'E408_02' Temperature2?.length() < 4? Temperature2 : Temperature2.substring(0,4)
									}
									...
									if(current_DGCargoSpec?.UNNumber){
										'H1' {
											...
											if(StringUtil.isNotEmpty(current_DGCargoSpec?.FlashPoint?.Temperature)){
												'E77_07' blUtil.replaceZeroAfterPoint(util.substring(current_DGCargoSpec?.FlashPoint?.Temperature, 1, 4))
											}
						...
						'Loop_L1' {
							'L1' {
								...
								if(current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)){  //1
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										String str = new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString()).multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP).toString()
										'E117_06' str
									}
								}else{
									'E117_06' '00'
								...
								if(StringUtil.isNotEmpty(current_FreightCharge?.ChargePrintLable?.trim())){
									'E276_12' current_FreightCharge?.ChargePrintLable?.trim()
								}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn)  {
		...
		bl?.Body?.eachWithIndex{current_Body, current_BodyIndex ->
			...
			List<String> blockChargeTypeList = ['DEM', 'DET']
			//OceanLegReorder
			blUtil.OceanLegReorder(current_Body.Route?.OceanLeg)

			//ChargeFilter
			blUtil.filterByChargeCodeBlock(filteredFreightCharge, filteredFreightChargeCNTR, blockChargeTypeList); //temp
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.missingChargePVH(current_Body, false, null, errorKeyList)  //Transaction has only prepaid amount
		blUtil.checkSealNumberLength(current_Body, 15, true, null, errorKeyList)
		blUtil.amendments_C(current_Body, false, "No amendments can be sent.", errorKeyList)
		return errorKeyList;
	}
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkL1max(node, 99, true, null, errorKeyList)
		blUtil.checkL106Length(node, 9, true, null, errorKeyList)
		blUtil.checkL102L106L307Checker_ACUCUBE(node,true, null, errorKeyList)  //The Length of L307,L102,L106 is over
	...
	void syntaxValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkB309_B310_P(node, true, "Either B309 B310 exists then all are required.", errorKeyList)
		blUtil.checkDTM02_DTM03_DTM_05_R(node, true, "At least one of DTM02,DTM03,DTM05 required.", errorKeyList)
		...
		blUtil.checkQTY02_QTY04_C(node, true, null, errorKeyList)
		blUtil.checkN902_N903_R(node, true, null, errorKeyList)
		blUtil.checkN722_M(node, true, 'Container type N722 is mandatory', errorKeyList) //
		...
		blUtil.checkN708_N709_P(node, true, null, errorKeyList)
	}
}
