...
/**
 * For TP_ID: KOTAHIGTN, copy from CUS_CS2BCXML_301_GTN_STD2
 * 
 * @author LIANGDA   
 *
 */

public class CUS_CS2BCXML_301_GTN_KOTAHIGTN {
	...
	Map<String, String> bookingStatusMap = ["Confirmed":"A", "Cancelled":"D"]
...
//				}
				...
				if (N1Map.get(current_Party.PartyType) && current_Party.PartyName) {
					...
					'Loop_N1' {
						'N1' {
							...
							'E67_04' util.substring(current_Party.CarrierCustomerCode?.trim(), 1, 80)
							'E706_05' ''
							'E98_06' ''
						}
						'N2' {
			...
			//get the 1st empty pickup
			EmptyPickup the1stEmptyPickup = null
			if (current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup && current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup?.size()>0) {
				the1stEmptyPickup = current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup.get(0)
				if (the1stEmptyPickup.Address?.AddressLines?.AddressLine && the1stEmptyPickup.Address?.AddressLines?.AddressLine?.size()>0) {
					'Loop_N1' {
						'N1' {
							'E98_01' 'PW'
							'E93_02' util.substring(the1stEmptyPickup.Address?.AddressLines?.AddressLine.get(0)?.trim(), 1, 60)
							'E66_03' '25'
							'E67_04' util.substring(the1stEmptyPickup.Facility?.FacilityCode?.trim(), 1, 80)
							'E706_05' ''
							'E98_06' ''
						}
						'N2' {
						...
						if (the1stEmptyPickup.Address?.AddressLines?.AddressLine?.size()>1) {
							//loop1
							'N3' {
								'E166_01' util.substring(the1stEmptyPickup.Address?.AddressLines?.AddressLine?.get(1), 1, 55)?.trim()
								if (the1stEmptyPickup.Address?.AddressLines?.AddressLine?.size()>2) {
									'E166_02' util.substring(the1stEmptyPickup.Address?.AddressLines?.AddressLine?.get(2), 1, 55)?.trim()
								}
							}
						}
						if (the1stEmptyPickup.Address?.AddressLines?.AddressLine?.size()>3) {
							//loop2
							'N3' {
								'E166_01' util.substring(the1stEmptyPickup.Address?.AddressLines?.AddressLine?.get(3), 1, 55)?.trim()
								if (the1stEmptyPickup.Address?.AddressLines?.AddressLine?.size()>4) {
									'E166_02' util.substring(the1stEmptyPickup.Address?.AddressLines?.AddressLine?.get(4), 1, 55)?.trim()
								}
							}
						}
					}
			...
			//loopType = 'POL'
			OceanLeg the1stLeg = null
			if (current_Body.Route?.OceanLeg && current_Body.Route?.OceanLeg?.size()>1) {
				the1stLeg = current_Body.Route?.OceanLeg?.get(0)
			}
			if (the1stLeg && the1stLeg.POL?.Port?.Country && the1stLeg.POL?.Port?.Country?.trim() == the1stLeg.POD?.Port?.Country?.trim()) {
				if (current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.City) {
					'Loop_R4' {
						'R4' {
							'E115_01' 'L'
							if (current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.UNLocationCode) {
								'E309_02' 'UN'
							} else if (current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.SchedKDCode) {
								'E309_02' current_Body.Route?.FirstPOL?.Port?.LocationCode?.SchedKDType
							}
							if (current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.UNLocationCode) {
								'E310_03' current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.UNLocationCode
							} else if (current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.SchedKDCode) {
								'E310_03' current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.LocationCode?.SchedKDCode
							}
							'E114_04' util.substring(current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.City, 1, 24)
							'E26_05' current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.CSCountryCode
							'E156_08' util.getCS2MasterCityStateCodeByStateCntryNoEmptyStateCde(current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.State, current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.CSCountryCode, conn)
						}
						if (current_Body.Route?.OceanLeg?.get(1)?.DepartureDT?.size()>0 && util.isXmlDateTimeLaterThanNow(current_Body.Route?.OceanLeg?.get(1)?.DepartureDT?.get(0)?.LocDT?.LocDT)) {
							'DTM' {
								'E374_01' '139'
								'E373_02' util.convertXmlDateTime(current_Body.Route?.OceanLeg?.get(1)?.DepartureDT?.get(0)?.LocDT?.LocDT, yyyyMMdd)
								'E337_03' util.convertXmlDateTime(current_Body.Route?.OceanLeg?.get(1)?.DepartureDT?.get(0)?.LocDT?.LocDT, HHmm)
							}
						}
			...
			//loopType = 'Trans'
			List<OceanLeg> oceanLegLoadExtVogs = current_Body.Route?.OceanLeg?.findAll{it?.LoadingExtVoyage}
			if (current_Body.Route?.OceanLeg && current_Body.Route?.OceanLeg?.size()>1 && current_Body.Route?.OceanLeg[-1]?.POL?.Port?.City && oceanLegLoadExtVogs?.size()>1) {
				OceanLeg the1stOceanLeg = current_Body.Route?.OceanLeg?.get(0)
				//TODO
				//the1stOceanLeg.POL?.Port?.County  ==> should be Country ? wrong in 1.0 ?
				if (the1stOceanLeg.POL?.Port?.County != the1stOceanLeg.POD?.Port?.Country) {
					'Loop_R4' {
						'R4' {
							'E115_01' 'Y'
						...
						OceanLeg checkOceanLeg = current_Body.Route?.OceanLeg?.find{it.DepartureDT && it.DepartureDT?.size()>0 && it.DepartureDT?.get(0)?.LocDT?.LocDT}
						if (checkOceanLeg && util.isXmlDateTimeLaterThanNow(checkOceanLeg.DepartureDT?.get(0)?.LocDT?.LocDT)) {
							def defDt = ''
							OceanLeg mapOceanLeg = null
							if (current_Body.Route?.OceanLeg?.size()>2) {
								if (current_Body.Route?.FirstPOL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND' && current_Body.Route?.OceanLeg?.get(1).POL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND') {
									List<OceanLeg> oceanLegs = current_Body.Route?.OceanLeg?.findAll{it.POL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND'}
									mapOceanLeg = oceanLegs[-1]
								} else {
									mapOceanLeg = current_Body.Route?.OceanLeg[-1]
								}
							} else if (current_Body.Route?.OceanLeg?.size()>0) {
								if (current_Body.Route?.OceanLeg[-1]?.POL?.Port?.City && current_Body.Route?.OceanLeg?.size()>1) {
									mapOceanLeg = current_Body.Route?.OceanLeg[-1]
								} else {
									mapOceanLeg = current_Body.Route?.OceanLeg[0]
								}
							}
							if (mapOceanLeg && mapOceanLeg.DepartureDT && mapOceanLeg.DepartureDT?.size()>0) {
								defDt = mapOceanLeg.DepartureDT[0]?.LocDT?.LocDT
							}
							if (defDt) {
								'DTM' {
									if (util.isXmlDateTimeLaterThanNow(defDt)) {
										'E374_01' '139'
									} else {
										'E374_01' '140'
									}
			...
			current_Body.Cargo?.eachWithIndex{ current_Cargo, index ->
				'Loop_LX' {
					...
					//loop for all remarks of 'EDI Bkg Confirmation Remark'
					String remarkEDILine = current_Body.RemarkLines?.find{it.attr_RemarkType=='EDI Bkg Confirmation Remark'}
					if (remarkEDILine) {
						for (int i = 0; i*60 < remarkEDILine.length(); i++ ) {
							'K1' {
								'E61_01' util.substring(remarkEDILine, i*60 + 1, 30).trim()
								'E61_02' util.substring(remarkEDILine, i*60 + 31, 30).trim()
							}
						}
					}
					
					// K1 for special remarks
					if (index==0) {
						if (current_Body.Route?.FirstPOL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND' && current_Body.Route?.OceanLeg?.size()>2 && current_Body.Route?.OceanLeg?.get(1)?.POL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND' && current_Body.Route?.OceanLeg[-1]?.POL?.Port?.Country?.toUpperCase() != 'NEW ZEALAND') {
							// 1, special remarks - vessel / city
							String remarkLineR1 = 'DEST VESSEL: '
							if (current_Body.Route?.OceanLeg[-1]?.SVVD?.Loading?.VesselName) {
								remarkLineR1 += current_Body.Route?.OceanLeg[-1]?.SVVD?.Loading?.VesselName
							}
							if (current_Body.Route?.OceanLeg[-1]?.POL?.Port?.City) {
								remarkLineR1 += current_Body.Route?.OceanLeg[-1]?.POL?.Port?.City
							}
							remarkLineR1 += ' TO'
							remarkLineR1 = remarkLineR1.toUpperCase()
							
							for (int i = 0; i*60 < remarkLineR1.length(); i++ ) {
								'K1' {
									'E61_01' util.substring(remarkLineR1, i*60 + 1, 30).trim()
									'E61_02' util.substring(remarkLineR1, i*60 + 31, 30).trim()
								}
							}
							
							// 2, special Departure DT / POD city
							if (current_Body.Route?.OceanLeg[-1]?.DepartureDT?.size()>0 && current_Body.Route?.OceanLeg[-1]?.DepartureDT[0]?.LocDT?.LocDT) {
								String remarkLineR2 = ''
								if (current_Body.Route?.OceanLeg[-1]?.POD?.Port?.City) {
									remarkLineR2 = current_Body.Route?.OceanLeg[-1]?.POD?.Port?.City?.toUpperCase()
								}
								remarkLineR2 +=  " " + util.convertXmlDateTime(current_Body.Route?.OceanLeg[-1]?.DepartureDT[0]?.LocDT?.LocDT, 'yyyy-MM-dd')
								remarkLineR2 = remarkLineR2.toUpperCase()
								
								for (int i = 0; i*60 < remarkLineR2.length(); i++ ) {
									'K1' {
										'E61_01' util.substring(remarkLineR2, i*60 + 1, 30).trim()
										'E61_02' util.substring(remarkLineR2, i*60 + 31, 30).trim()
									}
								}
							}
							
							// 3, POD city
							if (current_Body.Route?.OceanLeg[-1]?.POD?.Port?.City && (current_Body.Route?.OceanLeg[-1]?.DepartureDT==null || util.isEmpty(current_Body.Route?.OceanLeg[-1]?.DepartureDT[0]?.LocDT?.LocDT))) {
								String remarkLineR3 = current_Body.Route?.OceanLeg[-1]?.POD?.Port?.City
								remarkLineR3 = remarkLineR3.toUpperCase()
								
								for (int i = 0; i*60 < remarkLineR3.length(); i++ ) {
									'K1' {
										'E61_01' util.substring(remarkLineR3, i*60 + 1, 30).trim()
										'E61_02' util.substring(remarkLineR3, i*60 + 31, 30).trim()
									}
								}
							}
						}
...
//			List<OceanLeg> v1OceanLegs = new ArrayList<OceanLeg>()
//			if (current_Body.Route?.OceanLeg?.size()==1) {
//				OceanLeg the1stOceanLeg = current_Body.Route?.OceanLeg?.get(0)
//				if (the1stOceanLeg.LoadingExtVoyage) {
//					if (! (the1stOceanLeg.POL?.Port?.Country && the1stOceanLeg.POL?.Port?.Country == the1stOceanLeg.POD?.Port?.Country)) {
//						v1OceanLegs.add(the1stOceanLeg)
//					}
//				}
//			} else if (current_Body.Route?.OceanLeg?.size()>1) {
//				current_Body.Route?.OceanLeg?.findAll{it?.LoadingExtVoyage}?.each { current_OceanLeg ->
//					v1OceanLegs.add(current_OceanLeg)
//				}
//			}
			
			if (current_Body.Route?.OceanLeg?.size()>0 && current_Body.Route?.OceanLeg?.get(0)?.LoadingExtVoyage) {
				if (! (current_Body.Route?.OceanLeg?.size()>1 && current_Body.Route?.OceanLeg?.get(0)?.POL?.Port?.Country && current_Body.Route?.OceanLeg?.get(0)?.POL?.Port?.Country == current_Body.Route?.OceanLeg?.get(0)?.POD?.Port?.Country)) {
					OceanLeg the1stOceanLeg = current_Body.Route?.OceanLeg?.get(0)
					'V1' {
						def def597 = util.substring(the1stOceanLeg.SVVD?.Loading?.LloydsNumber, 1, 8)
						if (! def597) {
						...
						if (the1stOceanLeg.LoadingVesselNationalityCode) {
							 defCountryCde = util.substring(the1stOceanLeg.LoadingVesselNationalityCode, 1, 3)
						} else if (the1stOceanLeg.SVVD?.Loading?.VesselNationality) {
							defCountryCde = util.getCS2MasterCity4CountryCodeByCountryName(the1stOceanLeg.SVVD?.Loading?.VesselNationality, conn)
						}
						'E26_03' defCountryCde
						'E55_04' util.substring(the1stOceanLeg.LoadingExtVoyage, 1, 10)
						'E140_05' ''
						'E249_06' ''
						'E854_07' ''
						'E897_08' ''
			...
			if (current_Body.Route?.OceanLeg?.size()>1) {
				...
				if (current_Body.Route?.OceanLeg?.size()>2) {
					if (current_Body.Route?.FirstPOL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND' && current_Body.Route?.OceanLeg?.get(1).POL?.Port?.Country?.toUpperCase() == 'NEW ZEALAND') {
						...
						oceanLeg = current_Body.Route?.OceanLeg[-1]
					}
				} else if (current_Body.Route?.OceanLeg?.size()>0) {
					if (current_Body.Route?.OceanLeg[-1]?.POL?.Port?.City && current_Body.Route?.OceanLeg?.size()>1) {
						oceanLeg = current_Body.Route?.OceanLeg[-1]
					} else {
						oceanLeg = current_Body.Route?.OceanLeg[0]
					}
				}
				if (oceanLeg!=null && oceanLeg.LoadingExtVoyage) {
					'V1' {
						def def597 = util.substring(oceanLeg.SVVD?.Loading?.LloydsNumber, 1, 8)
						if (! def597) {
							def597 = util.substring(oceanLeg.SVVD?.Loading?.Vessel, 1, 8)
						}
						
						'E597_01' def597
						if (oceanLeg.SVVD?.Loading?.VesselName) {
							'E182_02' util.substring(oceanLeg.SVVD?.Loading?.VesselName, 1, 28)
						} else if (oceanLeg.SVVD?.Discharge?.VesselName) {
							'E182_02' util.substring(oceanLeg.SVVD?.Discharge?.VesselName, 1, 28)
						}
						def defCountryCde = ''
						if (oceanLeg.DischargeVesselNationalityCode) {
							 defCountryCde = util.substring(oceanLeg.DischargeVesselNationalityCode, 1, 3)
						} else if (oceanLeg.SVVD?.Discharge?.VesselNationality) {
							defCountryCde = util.getCS2MasterCity4CountryCodeByCountryName(oceanLeg.SVVD?.Discharge?.VesselNationality, conn)
						}
						'E26_03' defCountryCde
						'E55_04' util.substring(oceanLeg.LoadingExtVoyage, 1, 10)
						'E140_05' ''
						'E249_06' ''
						'E854_07' ''
						'E897_08' ''
	...
	void pospValidation(Body current_Body, String outputXml, List<Map<String,String>> errorKeyList) {
		...
		//B104StatusChecking
		bcUtil.checkB104Status(node, false, "MessageFunction are not Confirmed or Cancelled", errorKeyList)
