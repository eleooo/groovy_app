...
public class CUS_CS2BLXML_310_GTNEXUS_DIAGEOGTN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			Map<String, String> referenceCodeCS2Map = ['FR':'FN', 'SC':'CT']
			Map<String, String> referenceRemarkCS2Map = ['FR':'FORWARDER REFERENCE', 'SC':'CONTRACT NUMBER']
			...
			List<ExternalReference> SIDExternalReferenceGroup = []
			if(current_Body?.ExternalReference?.findAll{it?.ReferenceDescription == 'System Reference Number'}?.size() == 1 && current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID'}?.size() > 1){
				def var = current_Body?.ExternalReference?.find{it?.ReferenceDescription == 'System Reference Number'}?.ReferenceNumber
				SIDExternalReferenceGroup.addAll(current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID' && it?.ReferenceNumber != var})
			}
			if(current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID'}?.size() == 1){
				SIDExternalReferenceGroup.addAll(current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID'})
			}
			if(current_Body?.ExternalReference?.findAll{it?.ReferenceDescription == 'System Reference Number'}?.size() > 1 && current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID'}?.size() > 1){
				SIDExternalReferenceGroup.addAll(current_Body?.ExternalReference?.findAll{it?.CSReferenceType == 'SID'}[-1])
			}
			SIDExternalReferenceGroup?.each{current_ExternalReference ->
				for(int i = 0; i * 30 < current_ExternalReference?.ReferenceNumber?.length(); i++ ) {
					if(countN9 > 14){
						break
					}
					...
					'N9' {
						...
						if(current_ExternalReference?.ReferenceDescription){
							'E369_03' "SHIPPER'S IDENTIFYING NUMBER FOR SHIPMENT (SI"
						}
					}
					countN9++
				}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.l3Checking(current_Body, false, null, errorKeyList)
		return errorKeyList;
	}
