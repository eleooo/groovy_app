 ...
 * 
 * @author YINEM
 * shared by KATOENTANKDSG/KATOENGLOBALDSG, pattern after STD3
 */
public class CUS_CS2BCXML_301_DSG_STD5 {
	...
	Map<String,String> referenceTypeMap = ['BL':'BM','CR':'CR', 'CTR':'CT', 'FR':'FN', 'ICR':'CR', 'INV':'IN', 'PO':'PO', 'QUOT':'Q1', 'SID':'SI', 'SC':'E8', 'SR':'SR', 'TARIF':'TS']
	
	Map cccMap = ['6406954000':'10401601','7451485000':'10401601']
				...
				if (N1Map.get(current_Party.PartyType) && current_Party.PartyName) {
					...
					'Loop_N1' {
						'N1' {
							...
							def def67 = ''
//							def defN104 = util.getEDICdeReffromIntCde(TP_ID, 'BookingConfirmation', DIR_ID, 'BCCS2X', 'BookingConfirmation_N104', 'N104', current_Party.CarrierCustomerCode, conn)
							def defN104 = cccMap.get(current_Party.CarrierCustomerCode)
							if(current_Party.PartyType != 'BPT' && current_Party.CarrierCustomerCode){
								def67 = current_Party.CarrierCustomerCode.trim()
							}
							else if(current_Party.PartyType == 'BPT' && defN104){
								def67 = defN104.toString().trim()
							}
							'E67_04' def67							
							'E706_05' ''
							'E98_06' ''
			...
			current_Body?.Cargo?.eachWithIndex{ current_Cargo, index->
				'Loop_LX' {
					...
					//K1 loop
					def var_Remark = current_Body?.RemarkLines?.find{it.attr_RemarkType =='Booking'}?.toString()
					if(var_Remark){
						for(int i = 0; i*60 < var_Remark.length(); i++ ){
							'K1' {
								'E61_01' util.substring(var_Remark,i*60+1, 30).trim()
								'E61_02' util.substring(var_Remark,i*60+31, 30).trim()
							}
						}
					}
					//LoopDSGRemarks
					'K1' {
						'E61_01' 'For any inquiries, please'
						'E61_02' 'select from following e-mail'
					}
					'K1' {
						'E61_01' 'addresses:'
						'E61_02' ''
					...
					'K1' {
						'E61_01' 'Logistics Service Providers:'
						'E61_02' 'anrbkgn@oocl.com'
					}
					'K1' {
						'E61_01' 'Direct Shippers:'
						'E61_02' 'anrbkgb@oocl.com'
					}
