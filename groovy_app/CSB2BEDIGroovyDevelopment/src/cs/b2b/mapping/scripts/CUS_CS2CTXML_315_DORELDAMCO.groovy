...
class CUS_CS2CTXML_315_DORELDAMCO {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('DORELDAMCO', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  util.getConversionWithoutTP('CT', 'O', 'DORELDAMCO', vCS1EventFirst5, conn)	//get the first 5 char, in case duplicate event missing direction.
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				if ( (vCS1Event == 'CS160' || vCS1Event == 'CS195') && current_Body?.GeneralInformation?.TransportMode?.toUpperCase() == 'RAIL' ) {
					...
					'E157_03' util.getConversion('DORELDAMCO', 'CT', 'O', 'EventStatusCode', int_cde, conn)
				} else {
...
//					println 'datetime  \t\t\t' + datetime
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('DORELDAMCO', conn)
...
//		List<Body> blDupBodies = ctUtil.CTBLDuplication(ct.Body)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('DORELDAMCO', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		if (['CS160', 'CS195'].contains(eventCS2Cde) && current_Body?.GeneralInformation?.TransportMode?.toUpperCase()  == 'RAIL') {
			tmp_eventExtCde = util.getConversion('DORELDAMCO', 'CT', 'O', 'EventStatusCode', eventCS2Cde + current_Body?.GeneralInformation?.TransportMode.toUpperCase(), conn)
