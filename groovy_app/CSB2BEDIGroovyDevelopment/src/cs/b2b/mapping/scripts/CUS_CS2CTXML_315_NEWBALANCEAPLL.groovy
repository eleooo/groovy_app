...
public class CUS_CS2CTXML_315_NEWBALANCEAPLL {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('NEWBALANCEAPLL', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  util.getEDICdeReffromIntCde('NEWBALANCEAPLL', 'ShipDirection', 'O', 'INTCTXML', 'Q2', 'EventStatusCode', vCS1EventFirst5, conn)	//get the first 5 char, in case duplicate event missing direction.
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				//SpecialLogicOn315 Type1
				def B404=util.convertDateTime(current_Body.Event.EventDT.LocDT, xmlDateTimeFormat, yyyyMMdd)
				def B405=util.convertDateTime(current_Body.Event.EventDT.LocDT, xmlDateTimeFormat, HHmm)
				B406 = B406 ? String.format('%-5s', B406) : null
				B411 = B411 ? util.substring(B411, 1, 30) : null
				if(['CS277'].contains(vCS1EventFirst5)){
					OceanLeg lastOceanLeg = current_Body.Route?.OceanLeg ? current_Body.Route.OceanLeg[-1] : null
					if(lastOceanLeg){
						def vActDepartureDate = util.convertDateTime(lastOceanLeg.POL?.DepartureDT?.find{it?.attr_Indicator == 'A'}?.LocDT, xmlDateTimeFormat, 'yyyyMMddHHmmss')
						def vEstDepartureDate = util.convertDateTime(lastOceanLeg.POL?.DepartureDT?.find{it?.attr_Indicator == 'E'}?.LocDT, xmlDateTimeFormat, 'yyyyMMddHHmmss')
						if(StringUtil.isNotEmpty(vActDepartureDate)&&(vActDepartureDate as long) != 0){
							B404 = util.substring(vActDepartureDate, 1, 8)
							B405 = util.substring(vActDepartureDate, 9, 4)
						}else {
							B404 = util.substring(vEstDepartureDate, 1, 8)
							B405 = util.substring(vEstDepartureDate, 9, 4)
						}
						def vPOLUnLocCode = util.substring(lastOceanLeg.POL?.Port?.LocationCode?.UNLocationCode, 1, 5)
						def vPOLScheduleKDCode = util.substring(lastOceanLeg.POL?.Port?.LocationCode?.SchedKDCode, 1, 5)
						if(StringUtil.isNotEmpty(vPOLUnLocCode)){
							B406 = vPOLUnLocCode
						}else if(StringUtil.isNotEmpty(vPOLScheduleKDCode)){
							B406 = vPOLScheduleKDCode
						}else {
							B406 = null
						}
						def vPOLCity = util.substring(lastOceanLeg.POL?.Port?.City, 1, 60)
						if(StringUtil.isNotEmpty(vPOLCity)){
							B411 = vPOLCity
						}else {
							B411 = null
						}
						if(StringUtil.isNotEmpty(vPOLUnLocCode)){
							...
							B412 = util.substring(lastOceanLeg.POL?.Port?.LocationCode?.SchedKDType, 1, 1)
						}else {
							B412 = null;
						}
					}
			...
			if (partyCGN?.CarrierCustomerCode) {
				'N9' {
					...
					if (partyCGN?.CarrierCustomerCode?.trim()?.length() == 10 && partyCGN?.PartyName) {
						'E369_03' partyCGN?.PartyName?.trim()
					}
				}
				...
				'N9' {
					...
					if (partyCGNWithPartyLevel?.CarrierCustomerCode?.trim()?.length() == 10 && partyCGNWithPartyLevel?.PartyName) {
						'E369_03' partyCGNWithPartyLevel?.PartyName?.trim()
					}
				}
			}
			...
			if ((R403D || R404D) && !['CS160', 'CS120'].contains(vCS1EventFirst5)) {
				'Loop_R4' {
					...
					//SpecialHandlingAfter process11
			...
			if ((R403D || R404D) && vCS1EventFirst5 == 'CS277') {
				'Loop_R4' {
					...
					if (current_Body.Event.EventDT.LocDT?.toString()) {
						'DTM' {
							'E374_01' '139'
							'E373_02' util.convertDateTime(current_Body.Event?.EventDT?.LocDT, xmlDateTimeFormat, yyyyMMdd)
							'E337_03' util.convertDateTime(current_Body.Event?.EventDT?.LocDT, xmlDateTimeFormat, HHmm)
...
//		def ContainerMovement = parser.parseText(inputXmlBody);
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('NEWBALANCEAPLL', conn)
		Map<String,String> specialDuplicatePairs = new HashMap<String,String>();
		specialDuplicatePairs.put('CS060',duplicatedPairs.remove('CS060'))
		specialDuplicatePairs.put('CS160',duplicatedPairs.remove('CS160'))
		...
		//CS160 Duplicate to AM when Mode is Truck
		bodies = this.duplicateCS160Truck(bodies, specialDuplicatePairs)
		
		//SpecialDuplicate-DuplicateByEvent_FirstLegLoadingServiceIsFeeder
		bodies = this.duplicateByEventFirstLegLoadingServiceIsFeeder(bodies, specialDuplicatePairs)
		
		//start body looping
		bodies.eachWithIndex { current_Body, current_BodyIndex ->

			def eventCS2Cde = current_Body.Event.CS1Event
			def eventExtCde = util.getConversion('NEWBALANCEAPLL', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
...
//		println csuploadWriter.toString();
	 ...
	 * For Duplicate
	 * @param Body
	 * @param duplicatedPairs
	 * @return
	 */
	private List<cs.b2b.core.mapping.bean.ct.Body> duplicateCS160Truck(List<cs.b2b.core.mapping.bean.ct.Body> Body, Map<String,String> duplicatedPairs) {

		List<cs.b2b.core.mapping.bean.ct.Body> bodies = [];
		List<cs.b2b.core.mapping.bean.ct.Body> duplicateBodies = [];
		Body.eachWithIndex { current_Body, current_BodyIndex ->
			bodies.add(current_Body)
			def vTransportMode = util.substring(current_Body.GeneralInformation?.TransportMode,1,10).toLowerCase()
			if (current_Body.Event.CS1Event.equals('CS160') && duplicatedPairs.get(current_Body.Event.CS1Event) && vTransportMode.equals('truck') ) {
				//deep clone
				...
				duplicateBodies.add(duplBody)
			}
		}
		bodies.addAll(duplicateBodies)
		return bodies;
	}
	
	private List<cs.b2b.core.mapping.bean.ct.Body> duplicateByEventFirstLegLoadingServiceIsFeeder(List<cs.b2b.core.mapping.bean.ct.Body> Body, Map<String,String> duplicatedPairs) {
		
		List<cs.b2b.core.mapping.bean.ct.Body> bodies = [];
		List<cs.b2b.core.mapping.bean.ct.Body> duplicateBodies = [];
		Map<String,String> isFeederPairs = this.getIsFeederPairs('NEWBALANCEAPLL',conn)

		Body.eachWithIndex { current_Body, current_BodyIndex ->
			bodies.add(current_Body)
			def firstOceanLeg = current_Body.Route?.OceanLeg[0]
			def firstLegLoadingServiceCode = util.substring(firstOceanLeg?.SVVD?.Loading?.Service,1,5)
			if (current_Body.Event.CS1Event.equals('CS060') 
				&& StringUtil.isNotEmpty(firstLegLoadingServiceCode) 
				&& isFeederPairs.get(firstLegLoadingServiceCode) == 'IsFeeder' ) {
				//deep clone
				cs.b2b.core.mapping.bean.ct.Body duplBody = (cs.b2b.core.mapping.bean.ct.Body)SerializationUtils.clone(current_Body)
				duplBody.Event.CS1Event = duplicatedPairs.get(current_Body.Event.CS1Event)
				duplicateBodies.add(duplBody)
	...
	private Map<String,String> getIsFeederPairs(String tp_id, Connection conn){
		...
		String sql = "select Int_Cde, Ext_Cde from b2b_cde_conversion where tp_id = ? and dir_id='O' and convert_type_id= 'ServiceType' and msg_type_id ='CT' order by update_ts desc, create_ts desc";
