...
*/
public class CUS_CS2BLXML_310_JASPO {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['BKG':'BN','BL':'BM','CR':'CR','CTR':'CT','EXPR':'RF','FID':'FI','GEN':'GN','ICR':'CR','INV':'IK','MB':'BN',
						...
						'Loop_L1' {
							'L1' {
								...
								if(current_FreightCharge?.ChargeType == blUtil.PREPAID){
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										'E117_06' blUtil.replaceZeroAfterPoint(new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency.toString()).multiply(hundred).toString())
									}
								}else{
									'E117_06' '00'
								}
							...
							if(current_FreightCharge?.TotalAmtInPmtCurrency?.attr_Currency){
								'C3' {
									...
									def exchangeRate = current_FreightCharge?.TotalAmtInPmtCurrency?.attr_ExchangeRate
									if(exchangeRate){
										def c3occursNum = 0
												if(c3occursNum<1){
													if(exchangeRate?.indexOf('.')>-1){
														if(exchangeRate?.length()<5){
															c3occursNum = c3occursNum + 1
																	'E280_02' padfront(exchangeRate, 5, "0")
														}else if(exchangeRate?.length()>11){
															c3occursNum = c3occursNum + 1
																	'E280_02' padfront(exchangeRate, 11, null)
														}
													}
												}
										if(c3occursNum<1){
...
//		blUtil.checkL106Length(node, 9, true, null, errorKeyList)
	...
	private static String padfront(String str, Integer length, String concat){
		if(str!=null){
