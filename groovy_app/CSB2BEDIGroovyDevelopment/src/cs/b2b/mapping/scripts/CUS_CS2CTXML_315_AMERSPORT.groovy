...
/**
 * @author LIYU
 * AMERSPORT CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_AMERSPORT {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('AMERSPORT', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				if (current_Body.Event?.Location?.LocationCode?.UNLocationCode) {
					'E310_11' util.substring(current_Body.Event?.Location?.LocationCode?.UNLocationCode?.trim(), 1, 30)?.trim()
					'E309_12' 'UN'
				}
			...
			if (current_Body.BLGeneralInfo?.findAll{BL -> StringUtil.isNotEmpty(BL.BLNumber)}?.size() == 0 && current_Body.BookingGeneralInfo?.size() > 0) {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('AMERSPORT','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
			...
			// Louis , UNIQUE-SEGN9, avoid duplicate AAO only.
			//			def uniqueAAO = []
			...
			list.each{current_Party ->
				...
				def N9_02 = util.substring(current_Party?.CarrierCustomerCode,1,10)?.trim()
				if ((customerType == 'CN' || customerType == 'SH') && N9_02) {
					'N9'{
						//						'E128_01' getLegalPartyCodeTypeExtCDE('SFMCO','INTCTXML','315','O','LegalPartyCodeType', customerType,conn)
						if (customerType == 'CN') {
							'E128_01' '4E'
						} else if (customerType == 'SH') {
							'E128_01' '4F'
			...
			if(lastPOD && (R403 || R404)){
				'Loop_R4' {
					...
					oceanLegList = current_Body?.Route?.OceanLeg
					if (oceanLegList.size() > 0) {
						last_OceanLeg = oceanLegList[-1]
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('AMERSPORT', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		missingEventLocationCode(eventCS2Cde, current_Body.Event?.Location?.LocationCode?.UNLocationCode, true, null, errorKeyList)
	...
	void missingEventLocationCode(String eventCS2Cde, String UNLocCde, boolean isError, String errorMsg, List<Map<String, String>> errorKeyList) {
		Map<String, String> errorKey = null
		if (StringUtil.isEmpty(UNLocCde)) {
			errorKey = [TYPE: isError ? ERROR_SUPPORT : ERROR_COMPLETE, IS_ERROR: isError ? YES : NO, VALUE: errorMsg != null ? eventCS2Cde + errorMsg : eventCS2Cde + '- Missing Body/Event/Location/LocationCode/Unlocation']
