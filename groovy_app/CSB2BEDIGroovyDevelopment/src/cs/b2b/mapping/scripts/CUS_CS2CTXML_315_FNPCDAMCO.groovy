...
class CUS_CS2CTXML_315_FNPCDAMCO {
	...
	// for N9 SN, strip extra spaces amid 
	public static String normalizeSpace(String str) {
		final Pattern WHITESPACE_BLOCK = Pattern.compile("\\s+");
		if (str == null) {
			return null;
		}
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('FNPCDAMCO', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  util.getConversionWithoutTP('CT', 'O', 'FNPCDAMCO', vCS1EventFirst5, conn)	//get the first 5 char, in case duplicate event missing direction.
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				if(current_Body.GeneralInformation?.TransportMode?.toUpperCase()=='TRUCK'&& 'CS190' == vCS1EventFirst5)	{
					'E157_03' 'AM'
				...
				def cd = util.substring(current_Body.Container?.ContainerCheckDigit?.trim(), 1, 2)?.trim() ?: ''
				if(container_digit){					
			...
			if (current_Body.BLGeneralInfo?.size() == 0 && current_Body.BookingGeneralInfo?.size() > 0) {
			...
			// implement normalize-space() function using regex
			current_Body.Container?.Seal?.findAll{StringUtil.isNotEmpty(it.SealNumber) && it.SealNumber.length() >= 4}.each { Seal ->
				'N9' {
					'E128_01' 'SN'
					'E127_02' util.substring(normalizeSpace(Seal?.sealNumber), 1, 12) //normalizeSpace(util.substring(Seal?.sealNumber, 1, 12))
				}
			}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('FNPCDAMCO', conn)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('FNPCDAMCO', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
...
//		ctUtil.missingBookingNumber(eventCS2Cde, current_Body?.BookingGeneralInfo, true, null, errorKeyList)
		...
		eventCs270NotSubscribeForNonFNDUS(eventCS2Cde, current_Body?.Route?.FND, false, null, errorKeyList)
		eventCs275NotSubscribeForNonFNDUS(eventCS2Cde, current_Body?.Route?.FND, false, null, errorKeyList)
	...
	void eventCs270NotSubscribeForNonFNDUS(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.FND FND, boolean isError, String errorMsg, List<Map<String, String>> errorKeyList) {
		Map<String,String> errorKey = null
		def cntrcode = null
		if (FND?.CSStandardCity?.CSCountryCode && FND?.CSStandardCity?.CSCountryCode?.length() >= 2){
			cntrcode = util.substring(FND?.CSStandardCity?.CSCountryCode,1,2)?.trim()
		}
		if(eventCS2Cde == 'CS270' && cntrcode != 'US'){
			errorKey = [TYPE: isError? ERROR_SUPPORT:ERROR_COMPLETE, IS_ERROR: isError? YES : NO, VALUE: errorMsg !=null? eventCS2Cde + errorMsg : eventCS2Cde + ' Event cs270 not subscribe for non FND:US']
			errorKeyList.add(errorKey)
		}
	}
	
	void eventCs275NotSubscribeForNonFNDUS(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.FND FND, boolean isError, String errorMsg, List<Map<String, String>> errorKeyList) {
		Map<String,String> errorKey = null
		def cntrcode = null
		if (FND?.CSStandardCity?.CSCountryCode && FND?.CSStandardCity?.CSCountryCode?.length() >= 2){
			cntrcode = util.substring(FND?.CSStandardCity?.CSCountryCode,1,2)?.trim()
		}
		if (eventCS2Cde == 'CS275' && cntrcode == 'US'){
			errorKey = [TYPE: isError? ERROR_SUPPORT:ERROR_COMPLETE, IS_ERROR: isError? YES : NO, VALUE: errorMsg !=null? eventCS2Cde + errorMsg : eventCS2Cde + ' Event cs275 not subscribe for FND:US']
			errorKeyList.add(errorKey)
		}
	}
