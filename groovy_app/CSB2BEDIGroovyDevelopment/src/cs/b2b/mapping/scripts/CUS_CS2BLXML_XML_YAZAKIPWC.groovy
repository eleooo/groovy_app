 ...
 * @pattern after WGLL
 */
public class CUS_CS2BLXML_XML_YAZAKIPWC {
									...
									if (current_OceanLeg?.ArrivalDT?.size() > 0) {
										'EventDate' {
											...
											if(actArrivalDTLocDT){
												'Arrival'('TimeZone': actArrivalDTLocDT?.attr_TimeZone ? actArrivalDTLocDT?.attr_TimeZone : estArrivalDTLocDT?.attr_TimeZone, 'EstActIndicator': '0' , util.convertXmlDateTime(actArrivalDTLocDT, "yyyyMMddHHmmss"))
					...
					'EquipmentInformation' {
						current_Body?.Container?.each { current_Container ->
							'Containers' {
								...
								'SealNumber'('null':'true', 'Type': current_Container?.Seal[0]?.SealType, current_Container?.Seal[0]?.SealNumber)
...
//								'CommodityCode' ''
								...
								if(packageTypeMap.get("EXT_CDE")){
									'Package'('Type': packageTypeMap.get("EXT_CDE"), packageQty)
								}else{
...
//			}
			//output file name
			String emailSubject = "[CargoSmart BL data ${bl?.Body[0]?.GeneralInformation?.SCACCode}]"
			String outputFileName = "YAZAKIPWC_BL_" + MSG_REQ_ID + ".xml" + emailSubject
