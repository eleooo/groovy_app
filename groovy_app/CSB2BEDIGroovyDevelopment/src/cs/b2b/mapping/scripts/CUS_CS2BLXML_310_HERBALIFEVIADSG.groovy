...
/**
 * @author XIEMI
 * @pattern after TP=TRANSAMERICAN and HDYOW
 */
public class CUS_CS2BLXML_310_HERBALIFEVIADSG {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo, List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				int countOfPrepaidCharge = filteredFreightCharge?.findAll {
					it?.ChargeType?.equals(blUtil.PREPAID)
				}?.size();
				int countOfCollectCharge = filteredFreightCharge?.findAll {
					it?.ChargeType?.equals(blUtil.COLLECT)
				}?.size();
				...
				List<FreightCharge> filteredFreigthChargeCOLLECT = filteredFreightCharge?.findAll {
					it?.ChargeType == blUtil.COLLECT
				}
				List<FreightCharge> currencyNoneUSDFreightChargeCOLLECT = filteredFreigthChargeCOLLECT?.findAll {
					it?.TotalAmtInPmtCurrency?.attr_Currency != 'USD'
				}

				if (filteredFreigthChargeCOLLECT?.find { it?.TotalAmtInPmtCurrency?.attr_Currency == 'USD' }) {
					filteredFreigthChargeCOLLECT?.each { current_FreightCharge ->
						sum = sum + new BigDecimal(div(current_FreightCharge?.TotalAmtInPmtCurrency?.TotalAmtInPmtCurrency, current_FreightCharge?.TotalAmtInPmtCurrency?.attr_ExchangeRate))
					}
				}
// else if(filteredFreigthChargeCOLLECT?.find{it?.TotalAmtInPmtCurrency?.attr_Currency == 'USD'} == null &&
// currencyNoneUSDFreightChargeCOLLECT?.size() == filteredFreigthChargeCOLLECT?.findAll{it?.TotalAmtInPmtCurrency?.attr_Currency == currencyNoneUSDFreightChargeCOLLECT[0]?.TotalAmtInPmtCurrency?.attr_Currency}?.size() ){
// filteredFreigthChargeCOLLECT?.each{current_FreightCharge ->
// sum = sum + new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString())
// }
// }                       //i think tibco is wrong itself
				...
				if (current_Body?.Route?.ArrivalAtFinalHub?.find { it?.attr_Indicator == 'A' }?.LocDT) {
					'E32_09' util.convertDateTime(current_Body?.Route?.ArrivalAtFinalHub?.find {
						it?.attr_Indicator == 'A'
			...
			///////////////////   N9-start
			String SR_SysReferenceNumber = current_Body?.ExternalReference?.find {
				it?.ReferenceDescription == 'System Reference Number'
			}?.ReferenceNumber;

			if (SR_SysReferenceNumber) {
				SR_SysReferenceNumber = SR_SysReferenceNumber.concat('0');
			}

			//Loop by BookingNumber
			int N9PositionIndex = 0
			...
			Map<String, String> referenceCodeCS2Map = ['CR' : 'CR', 'CTR': 'CT', 'EXPR': 'RF', 'FID': 'FI', 'GEN': 'GN', 'ICR': 'CR', 'INV': 'IK',
													   'OTH' : 'ZZ', 'PO': 'PO', 'QUOT': 'QN', 'SC': 'E8', 'SCA': 'SCA', 'SID': 'SI', 'SO': 'SO', 'SR':
															   'SR', 'TARIF': 'TI', 'WARG': 'QA']
			current_Body?.CarrierRate?.each { current_CarrierRate ->
				...
				if (vItemCde) {
					if (N9PositionIndex < 15) {
						N9PositionIndex = N9PositionIndex + 1
						if ((vItemCde != 'SR') || ((vItemCde == 'SR') && current_CarrierRate.CarrierRateNumber.concat('0') != SR_SysReferenceNumber)) {
//                        if (N9PositionIndex < 15) {
//                            N9PositionIndex = N9PositionIndex + 1
							if ((vItemCde != 'ZZ') || ((vItemCde == 'ZZ') && current_CarrierRate.CarrierRateNumber.concat('0') != SR_SysReferenceNumber)) {
								//                            if (N9PositionIndex < 15) {
								//                                N9PositionIndex = N9PositionIndex + 1
								'N9' {
									'E128_01' vItemCde
...
//                        }
			...
			current_Body?.ExternalReference?.each { current_ExternalReference ->
				...
				if (vItemCde) {
					if (N9PositionIndex < 15) {
						N9PositionIndex = N9PositionIndex + 1
						if ((vItemCde != 'SR') || ((vItemCde == 'SR') && current_ExternalReference?.ReferenceNumber?.concat('0') != SR_SysReferenceNumber)) {
							if ((vItemCde != 'ZZ') || ((vItemCde == 'ZZ') && current_ExternalReference?.ReferenceNumber?.concat('0') != SR_SysReferenceNumber)) {
								'N9' {
									'E128_01' vItemCde
...
//            int N3_Actual_Execute = 1;
			...
			current_Body?.Party?.each { current_Party ->
				if (partyTypeMap.get(current_Party?.PartyType)) {
					'Loop_N1' {
						...
						int length = 110;
						String[] addressLine_Array;
						double count = (vAddressLines.length() / (length * 1.0));

						if (!(count + "").endsWith(".0")) {
							count = count + 1;
						}
						int numberOfLine = (int) count;
						String originalStr = vAddressLines;
						addressLine_Array = new String[numberOfLine];

						for (int i = 0; i < numberOfLine; i++) {
							//Cut for each 55
							if (originalStr.length() > length + 1) {
								addressLine_Array[i] = originalStr.substring(0, length + 1);
								originalStr = originalStr.substring(length + 1);

							} else {
								addressLine_Array[i] = originalStr.substring(0, originalStr.length());
						...
						int N3_Actual_Execute = 1
						addressLine_Array?.each { current_addressLine -> //1 not null

							String City = current_Party?.Address?.City?.trim()
							String County = current_Party?.Address?.County?.trim()
							String State = current_Party?.Address?.State?.trim()
							String PostalCode = current_Party?.Address?.PostalCode?.trim()
							String Country = current_Party?.Address?.Country?.trim()

							if (N3_Actual_Execute <= 2) {
								'N3' {
									N3_Actual_Execute++;
									if (current_addressLine) {
										'E166_01' util.substring(current_addressLine?.trim(), 1, 55)?.trim()
									}
									if (current_addressLine.trim()?.length() >= 56) {
										'E166_02' util.substring(current_addressLine?.trim(), 56, 55)?.trim()
									}
								}
							}
						}
						...
						'N4' {
							...
							//TODO
							String state = current_Party?.Address?.State
							String city = current_Party?.Address?.City
							String country = current_Party?.Address?.Country
							//util line 933
							String STATE_CDE1 = util.getCS2MasterCityByStateNameCityNameCntryCde(state, city, country, conn)?.get("STATE_CDE");
							String STATE_CDE2 = ''
							String STATE_CDE3 = ''
							if (StringUtil.isEmpty(STATE_CDE1?.trim())) {
								STATE_CDE2 = util.getCS2MasterCityByStateNameCityName(state, city, conn)?.get("STATE_CDE");
								if (StringUtil.isEmpty(STATE_CDE2?.trim())) {
									STATE_CDE3 = util.getCS2MasterCityByStateName(state, conn)?.get("STATE_CDE")
								}
							}

							String param = null;
							if (StringUtil.isNotEmpty(STATE_CDE1)) {
								param = STATE_CDE1
							} else if (StringUtil.isNotEmpty(STATE_CDE2)) {
								param = STATE_CDE2
							} else {
								param = STATE_CDE3
							}
				...
				if (oceanLegSize > 0) {
					'DTM' {
						if (current_Body?.Route?.OceanLeg[0]?.DepartureDT?.find { it?.attr_Indicator == 'A' }?.LocDT) {
							'E374_01' '140'
							'E373_02' util.convertDateTime(current_Body?.Route?.OceanLeg[0]?.DepartureDT?.find {
								it?.attr_Indicator == 'A'
							}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
							'E337_03' util.convertDateTime(current_Body?.Route?.OceanLeg[0]?.DepartureDT?.find {
								it?.attr_Indicator == 'A'
							}?.LocDT, xmlDateTimeFormat, HHmm)
						} else if (current_Body?.Route?.OceanLeg[0]?.DepartureDT?.find {
							it?.attr_Indicator == 'E'
						}?.LocDT) {
							'E374_01' '139'
							'E373_02' util.convertDateTime(current_Body?.Route?.OceanLeg[0]?.DepartureDT?.find {
			...
			'Loop_R4' {
				...
				if (oceanLegSize > 0) {
					'DTM' {
						...
						} else if (current_Body?.Route?.OceanLeg[-1]?.ArrivalDT?.find {
							it?.attr_Indicator == 'E'
						}?.LocDT) {
							'E374_01' '139'
			...
			'Loop_R4' {
				...
				'DTM' {
					...
					} else if (oceanLegSize > 0 && current_Body?.Route?.OceanLeg[-1]?.ArrivalDT?.find {
						it?.attr_Indicator == 'A'
					}?.LocDT) {
						'E374_01' '140'
						'E373_02' util.convertDateTime(current_Body?.Route?.OceanLeg[-1]?.ArrivalDT?.find {
							it?.attr_Indicator == 'A'
						}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
						'E337_03' util.convertDateTime(current_Body?.Route?.OceanLeg[-1]?.ArrivalDT?.find {
							it?.attr_Indicator == 'A'
						}?.LocDT, xmlDateTimeFormat, HHmm)
					} else if (oceanLegSize > 0 && current_Body?.Route?.OceanLeg[-1]?.ArrivalDT?.find {
						it?.attr_Indicator == 'E'
			...
			associateContainerAndCargo?.eachWithIndex { current_Container, current_cargoList, Container_index ->
				List<Container> containerGroup = current_Body.Container.findAll {
					it?.ContainerNumber == current_Container?.ContainerNumber && it?.ContainerCheckDigit == current_Container?.ContainerCheckDigit && it?.CS1ContainerSizeType == current_Container?.CS1ContainerSizeType
				}
				'Loop_LX' {
						...
						'Loop_N7' {
							if (util.substring(current_Container?.ContainerNumber, 5, 10)) {
								'N7' {
									...
									Map<String, String> haulageMap = ['C': 'H', 'M': 'P']
							...
							if (current_cargoList[0]?.ReeferCargoSpec?.size() > 0) {
								...
								'W09' {
									...
									def temperature = util.substring(current_ReeferCargoSpec?.Temperature?.Temperature, 1, 4)
									def signal = null
									...
									if (temperature?.contains(".")) {
										...
										if (StringUtil.isNotEmpty(remainTempPoint)) {
											if (signal.equals("-")) {
										...
										String fullTemperature = util.substring(current_ReeferCargoSpec?.Temperature?.Temperature, 1, 4)
										'E408_02' fullTemperature
									}
			...
			'L3' {
				...
				//MapSumCollectChargeHERBALIFE
				sum = new BigDecimal(0)
				def filteredFreigthChargeCOLLECT = filteredFreightCharge?.findAll { it?.ChargeType == blUtil.COLLECT }

				if (filteredFreigthChargeCOLLECT?.find { it?.TotalAmtInPmtCurrency?.attr_Currency?.equals('USD') }) {
					filteredFreigthChargeCOLLECT?.each { current_inside_FreightCharge ->
						if(current_inside_FreightCharge?.TotalAmtInPmtCurrency?.attr_ExchangeRate){
							if(new BigDecimal(current_inside_FreightCharge?.TotalAmtInPmtCurrency?.attr_ExchangeRate) != 0){
								sum = sum + (new BigDecimal(current_inside_FreightCharge?.TotalAmtInPmtCurrency?.toString()) / (new BigDecimal(current_inside_FreightCharge?.TotalAmtInPmtCurrency?.attr_ExchangeRate)))
							}
						}
					}
					sum = sum.multiply(hundred).setScale(0, BigDecimal.ROUND_HALF_UP);
				}
				'E58_05' sum
						...
						'Loop_L1' {
							'L1' {
								...
								if(current_FreightCharge?.ChargeType == blUtil.PREPAID){
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										String TotalAmtInPmtCurrency = blUtil.replaceZeroAfterPoint(new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString()).multiply(hundred).toString())
										'E117_06' TotalAmtInPmtCurrency
									}
								}
	...
	List<Map<String, String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex) {
		...
		blUtil.missingCollectExchangeRateforNonUSD(current_Body, true, null, errorKeyList)
		blUtil.missingDGUNNumber(current_Body, true, null, errorKeyList)
		return errorKeyList;
	}
	...
	void pospValidation(String outputXml, List<Map<String, String>> errorKeyList) {
		...
		blUtil.checkL0Max(node, 120, true, 'L0 cannot support cargo more than 120', errorKeyList)  //checkL0Max
	...
	void syntaxValidation(String outputXml, List<Map<String, String>> errorKeyList) {
		...
		blUtil.checkN101_N102_M(node, true, null, errorKeyList)   //>N101 and N102 are mandatory
	 ...
	 * @return
	 */
	private String padfront(String str, Integer length, String concat) {
		if (str != null) {
			if (str?.length() <= length) {
				def strlen = str?.length()
	...
	private String div(String dived, String diving) {
		if (diving.toBigDecimal() == 0) {
			return dived
		} else {
			return (dived.toBigDecimal() / diving.toBigDecimal()).toString();
		}
	}
