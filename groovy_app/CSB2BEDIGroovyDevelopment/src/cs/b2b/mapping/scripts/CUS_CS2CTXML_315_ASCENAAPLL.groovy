...
class CUS_CS2CTXML_315_ASCENAAPLL {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('ASCENAAPLL', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		outXml.'Loop_ST' {
			...
			//N9 SVVDService
			if(shipDir == 'I' && util.notEmpty(lastOceanLeg?.SVVD?.Discharge?.Service)){
				'N9'{
					'E128_01' 'ZH'
					'E127_02' util?.substring(lastOceanLeg?.SVVD?.Discharge?.Service?.trim(),1,5)
					if(lastOceanLeg?.SVVD?.Discharge?.ServiceName){
						'E369_03' util.substring(lastOceanLeg?.SVVD?.Discharge?.ServiceName?.trim(), 1, 45)}
					}
			}else if(shipDir == 'O' && util.notEmpty(firstOceanLeg?.SVVD?.Loading?.Service)){
				'N9'{
					'E128_01' 'ZH'
					'E127_02' util?.substring(firstOceanLeg?.SVVD?.Loading?.Service?.trim(),1,5)
					if(firstOceanLeg?.SVVD?.Loading?.ServiceName){
						'E369_03' util.substring(firstOceanLeg?.SVVD?.Loading?.ServiceName?.trim(), 1, 45)}
					}
			}
			...
			//Since Row 712 use the same logic and param, so extract the logic here to improve preformance
			LocDT podArrivalDtA
			LocDT podArrivalDtE
...
//		generateEDIHeader(outXml, appSessionId, ct, currentSystemDt)
		...
		Map<String,String> duplicatedPairs	= ctUtil.getDuplicatedPairs('ASCENAAPLL', conn)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('ASCENAAPLL', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
...
//		ctUtil.filterIBIntermodal(eventCS2Cde, 'CS060', current_Body.Route?.Inbound_intermodal_indicator, false, null, errorKeyList)
	...
	private List<cs.b2b.core.mapping.bean.ct.Body> duplicateByEventFirstLegLoadingServiceIsFeeder(List<cs.b2b.core.mapping.bean.ct.Body> Body, Map<String,String> duplicatedPairs) {
		...
		Map<String,String> isFeederPairs = this.getIsFeederPairs('ASCENAAPLL',conn)
