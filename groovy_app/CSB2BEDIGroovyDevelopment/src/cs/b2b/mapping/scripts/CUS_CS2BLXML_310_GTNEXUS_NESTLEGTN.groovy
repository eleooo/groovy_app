...
/**
 * @author YINEM
 * @pattern after GTNEXUS, only diff is the reference PO
 */
public class CUS_CS2BLXML_310_GTNEXUS_NESTLEGTN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			Map<String, String> referenceCodeCS2Map = ['FR':'FN', 'SC':'CT', 'SID':'SI','PO':'PO']
			Map<String, String> referenceRemarkCS2Map = ['FR':'FORWARDER REFERENCE', 'SC':'CONTRACT NUMBER', 'SID':'Shipper\'s Identifying Number for Shipment (SID)','PO':'Purchase Order Number']
			...
			current_Body?.Container?.groupBy {util.getCdeConversionFromIntCde('GTNEXUS', null, 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', it?.CS1ContainerSizeType, conn)?.get("EXT_CDE")}?.each{currentContainerSizeType, currentContainerGroup->
				Map<String, String> retMap = util.getCdeConversionFromIntCde('GTNEXUS', null, 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', currentContainerGroup[0]?.CS1ContainerSizeType, conn)
