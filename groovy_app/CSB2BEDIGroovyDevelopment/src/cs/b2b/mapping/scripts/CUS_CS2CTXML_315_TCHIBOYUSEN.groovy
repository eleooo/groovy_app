 ...
 */
public class CUS_CS2CTXML_315_TCHIBOYUSEN{
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('TCHIBOYUSEN', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)

		def shipDir = util.getEDICdeReffromIntCde('TCHIBOYUSEN','ShipDirection','O','INTCTXML','Q2','EventStatusCode',vCS1Event,conn)
		...
		outXml.'Loop_ST' {
			...
			'Loop_R4' {
				'R4' {
					...
					if(!POR?.CityDetails?.LocationCode?.UNLocationCode && POR?.CityDetails?.LocationCode?.SchedKDType){
			...
			'Loop_R4' {
				'R4' {
					...
					else if (!lastPOD?.Port?.LocationCode?.UNLocationCode && lastPOD?.Port?.LocationCode?.SchedKDType) {
			...
			'Loop_R4' {
				'R4' {
					...
					else if (!firstPOL?.Port?.LocationCode?.UNLocationCode && firstPOL?.Port?.LocationCode?.SchedKDType) {
			...
			'Loop_R4' {
				'R4' {
					...
					else if (!FND?.CityDetails?.LocationCode?.UNLocationCode &&  FND?.CityDetails?.LocationCode?.SchedKDType) {
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('TCHIBOYUSEN', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		if(eventCS2Cde=='CS140')
		{
			ctUtil.missingEventStatusCode(eventCS2Cde, '', false, '- Event is not subscribed by Parnter ! ', errorKeyList)
		}
