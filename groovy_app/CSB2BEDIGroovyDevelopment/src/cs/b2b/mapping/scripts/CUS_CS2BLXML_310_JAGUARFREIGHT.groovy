...
*/
public class CUS_CS2BLXML_310_JAGUARFREIGHT {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['CGO':'CG','CR':'CR','CTR':'CT','FM':'FM','FR':'FN','INV':'IN','PO':'PO','PR':'Q1',
			...
			current_Body?.CarrierRate?.each{current_CarrierRate ->
					...
					if(vItemCde){
						if(N9PositionIndex < 15){
							...
							'N9' {
								'E128_01' vItemCde
								'E127_02' util.substring(current_CarrierRate.CarrierRateNumber,1,35)
							}
						}
					}
			}
			...
			current_Body?.ExternalReference?.each{current_ExternalReference ->
					...
					if(vItemCde ){
						if(N9PositionIndex < 15){
							...
							'N9' {
								'E128_01' vItemCde
								'E127_02' util.substring(current_ExternalReference.ReferenceNumber,1,35)
							}
						}
					}
			}
			...
			///////////////////   G61-start
			def G61index = 0
			def G61PartyList = new ArrayList<Party>()
			
			current_Body?.Party?.findAll{ current_Party -> (current_Party?.Contact?.FirstName!=null && current_Party?.Contact?.FirstName!="") && 
				(current_Party?.Contact?.ContactPhone?.Number!=null && current_Party?.Contact?.ContactPhone?.Number!="")}?.each{ current_Party ->
				G61PartyList.add(current_Party)
			}
				
			G61PartyList?.each{ current_Party ->
			...
			if(current_FreightCharge?.ChargeType == blUtil.COLLECT || current_FreightCharge?.ChargeType == blUtil.PREPAID){
				if(current_FreightCharge?.TotalAmtInPmtCurrency){
						'Loop_L1' {
							'L1' {
								'E213_01' L101index++          // i think this index has quetion
								...
								if(calculateMethod!=null&&calculateMethod!=""){
									if(calculateMethodList.contains(calculateMethod) || calculateMethod.charAt(0)== 'B' || calculateMethod.charAt(0)== '2' || calculateMethod.charAt(0)== '4' ||
											otherVar.contains(calculateMethod)){
									}
								...
								if(current_FreightCharge?.ChargeType == blUtil.PREPAID){
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										'E117_06' util.substring(blUtil.replaceZeroAfterPoint(new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency.toString()).multiply(hundred).toString()),1,9)
									}
								}
	...
	private static String replaceZeroAfterPoint(String str){
