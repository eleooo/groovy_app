 ...
 * @pattern ref TP=CVI
 */
public class CUS_CS2CTXML_315_CVI {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('CVI', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		//		def shipDir=getShipDir('CVI','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				def eventStateCode = util.substring(current_Body?.Event?.Location?.CityDetails?.State,1,2).toUpperCase()	// eventStateCode is first two char of the full state name, rather than CSStateCode
				...
				Map map1 = ctUtil.getUNLocCodeFromLocationInfo(eventCityName, eventStateCode, eventCountryCode, conn)
				Map map2 = ctUtil.getUNLocCodeFromCityInfo(eventCityName, eventCountryCode, conn)
				...
				else
				{
					...
					if (map1['UNLocCode'] || map1['SchedKDCde']) {
						if (map1['UNLocCode']) {
							'E310_11' util.substring(map1['UNLocCode']?.trim(), 1, 30)?.trim()
							'E309_12' 'UN'
						} else {
							'E310_11' util.substring(map1['SchedKDCde']?.trim(), 1, 30)?.trim()
							if (map1['SchedType'] == 'K' || map1['SchedType'] == 'D') {
								'E309_12' map1['SchedType']
							}
						}
					} else if (map2['UNLocCode'] || map2['SchedKDCde']) {
						if (map2['UNLocCode']) {
							'E310_11' util.substring(map2['UNLocCode']?.trim(), 1, 30)?.trim()
							'E309_12' 'UN'
						} else {
							'E310_11' util.substring(map2['SchedKDCde']?.trim(), 1, 30)?.trim()
							if (map2['SchedType'] == 'K' || map2['SchedType'] == 'D') {
								'E309_12' map2['SchedType']
							}
						}
					} else if (eventCityName){
						'E310_11' util.substring(eventCityName?.trim(), 1, 30)?.trim()
						'E309_12' 'CI'
					} else if (eventLocationName) {
						'E310_11' util.substring(eventLocationName?.trim(), 1, 30)?.trim()
						'E309_12' 'CI'
					}
				}
			...
			// ExtReferenceCode  hardcoded legalRefIntCde, it's tp specifc.
			List legalRefIntCde = [
				'FN',
				'INV',
				'IN',
			...
			Map extRefMap = ['FN':'FN', 'INV':'IN', 'PO':'PO', 'SC':'E8', 'SID':'SI', 'SO':'SO', 'SR':'SR']
			...
			// SCAC
			if(current_Body?.GeneralInformation?.SCAC && firstScac){
				'N9'{
					'E128_01' 'SCA'
					'E127_02' firstScac
				}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('CVI', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		// for this TP=CVI, it has four CSUpload cases, as below
		// B4 03
	...
	ctUtil.EventIsNotSubscribedByParnter(eventCS2Cde, eventExtCde, false, '', errorKeyList)
		...
		// N9 EQ  Missing Container Number
		ctUtil.missingContainerNumber(eventCS2Cde, current_Body.Container, true, null, errorKeyList)

		//for this TP=CVI, no seg_num='ALLOW_CS260_INTERMODAL' in DB, but exist in tibco, so CSUpload "filterIBIntermodal"
