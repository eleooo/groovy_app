...
/**
 * xml : copy from 1.0 BCXML app - BCXML_Std, similar and copy from CUS_CS2BCXML_XML_LEMANUSA.groovy
 *        -- some difference
 *        -- 1, LocationDetails/FacilityCodeName not output
 *        -- 2, container sizetype query use SCAC : util.getCdeConversionFromIntCde
 *        -- 3, no filter Address : Filter Address 1 if same as Facility Name
 *        -- 4, no output CarrierCustomerCode
 *        -- 5, output filename macro is different
 *        
 *        
 ...
 * 
 */
public class CUS_CS2BCXML_XML_MANITOULINGF {
...
//													}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			//output file name
			String outputFileName = TP_ID + "_BCXML_" + currentDate.format(yyyyMMddHHmmss) + fileSeqStr + ".xml"
