...
public class CUS_CS2BLXML_310_GTNEXUS_JJGTN {
		...
		outXml.'Loop_ST' {
			...
			current_Body?.Container?.groupBy {util.getCdeConversionFromIntCde('GTNEXUS', '', 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', it?.CS1ContainerSizeType, conn)?.get("EXT_CDE")}?.each{currentContainerSizeType, currentContainerGroup->
				...
				if(retMap.get("EXT_CDE")){
                    Map<String,String> Y203Map = ['TrafficMode_FCL_FCL':'HH','TrafficMode_FCL_LCL':'HP','TrafficMode_LCL_FCL':'PH','TrafficMode_LCL_LCL':'PP']
					'Y2' {
						'E95_01' currentContainerGroup?.size()
						'E56_03' Y203Map.get('TrafficMode_'+current_Body.GeneralInformation?.TrafficMode?.OutBound+'_'+current_Body.GeneralInformation?.TrafficMode?.InBound)
			...
			'L3' {
				...
				sum = new BigDecimal(BigDecimal.ZERO)
				filteredFreightCharge?.findAll{it.ChargeType == '1'}?.each{current_FreightCharge ->
					if(current_FreightCharge.TotalAmtInPmtCurrency?.toString()){
