...
/**
 * ACS-PHILIPS reference DSGOODS CT 
 * initialize on 20170112 
 */
public class CUS_CS2CTXML_315_ACS_PHILIPS {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('ACS-PHILIPS', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		def shipDir = util.getEDICdeReffromIntCde('ACS-PHILIPS','ShipDirection', 'O', 'INTCTXML','Q2','EventStatusCode', vCS1EventFirst5,conn);
		String csEventDateTime = util.convertDateTime(current_Body?.Event?.EventDT?.LocDT, xmlDateTimeFormat, yyyyMMdd)
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				def CSTransportMode = util.substring(current_Body?.GeneralInformation?.TransportMode?.trim(), 1, 10)			
				if (['CS060', 'CS090'].contains(vCS1EventFirst5) && CSTransportMode.toUpperCase() == 'FEEDER') {
					B403 = 'AP'
				} else if (['CS050'].contains(vCS1EventFirst5) && CSTransportMode.toUpperCase() == 'RAIL') {
					B403 = 'RL'
				}
		...
		if (['CS160'].contains(vCS1EventFirst5)) {
			...
			if (partyCGN?.CarrierCustomerCode) {
				'N9' {
					'E128_01' 'IC'
					'E127_02' util.substring(partyCGN?.CarrierCustomerCode?.trim(), 1, 10)

					// this should be bug in tibco
					if (partyCGN?.CarrierCustomerCode?.trim()?.length() == 10) {
						'E369_03'util.substring(partyCGN?.PartyName?.trim(), 1, 45).trim()
					}
				}
			} else if (partyCGNWithPartyLevel?.CarrierCustomerCode) {
				'N9' {
					'E128_01' 'IC'
					'E127_02' util.substring(partyCGNWithPartyLevel?.CarrierCustomerCode?.trim(), 1, 10)
					// this should be bug in tibco
					if (partyCGNWithPartyLevel?.CarrierCustomerCode?.trim()?.length() == 10 && partyCGNWithPartyLevel?.PartyName) {
						'E369_03' util.substring(partyCGNWithPartyLevel?.PartyName?.trim(), 1, 45).trim()
					}
				}
			}
			...
			if ((R403D || R404D)) {
				'Loop_R4' {
					...
					LocDT podArrivalDtA
					LocDT podArrivalDtE
					for (int i = 0; i < current_Body?.Route?.OceanLeg.size(); i ++ ) {
						OceanLeg current_OceanLeg = current_Body?.Route?.OceanLeg[i]
						if (!(StringUtil.isEmpty(current_OceanLeg.SVVD?.Loading?.Vessel)
						&& (StringUtil.isEmpty(current_OceanLeg.SVVD?.Loading?.Voyage) && StringUtil.isEmpty(current_OceanLeg.SVVD?.Loading?.Direction))
						&& StringUtil.isEmpty(current_OceanLeg.SVVD?.Loading?.VesselName))) {
			...
			if (vCS1EventFirst5 == 'CS190') {
				...
				'Loop_R4' {
					...
					//isAct = true -> 140, isAct = false -> 139
					LocDT CargoDatetime = current_Body?.Route?.CargoDeliveryDT?.find{it.attr_Indicator == 'A'}?.LocDT

					LocDT ArriDatetime = current_Body?.Route?.ArrivalAtFinalHub?.find{it.attr_Indicator == 'A'}?.LocDT

					def ActCargoDeliveryDateTime = util.convertDateTime(CargoDatetime, xmlDateTimeFormat, yyyyMMdd)
					def ArrivalAtFinalHubDateTime = util.convertDateTime(ArriDatetime, xmlDateTimeFormat, yyyyMMdd)

					if (CargoDatetime && (csEventDateTime <= ActCargoDeliveryDateTime)) {
						isAct = true
						fndDatetime = CargoDatetime
					} else if (ArriDatetime && (csEventDateTime <= ArrivalAtFinalHubDateTime)) {
						isAct = true
						fndDatetime = ArriDatetime
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('ACS-PHILIPS', conn)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('ACS-PHILIPS', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
