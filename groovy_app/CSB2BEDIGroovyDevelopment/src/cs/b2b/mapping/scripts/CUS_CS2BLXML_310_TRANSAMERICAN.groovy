...
/**
 * @author XIEMI
 * @pattern after TP=TOPOCEAN, HDYOW
 */
public class CUS_CS2BLXML_310_TRANSAMERICAN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				int countOfPrepaidCharge = filteredFreightCharge?.findAll{it?.ChargeType?.equals(blUtil.PREPAID)}?.size();
				int countOfCollectCharge = filteredFreightCharge?.findAll{it?.ChargeType?.equals(blUtil.COLLECT)}?.size();
				...
				filteredFreightCharge?.findAll{it?.ChargeType?.equals(blUtil.COLLECT)}?.each { current_FreightCharge ->
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['CR':'CR','CTR':'CT','PO':'PO','QUOT':'Q1','RP':'RP','SC':'E8','SID':'SI','SR':'SR']
			...
			current_Body?.ExternalReference?.each{current_ExternalReference ->
				...
				if(vItemCde ){
					if(N9PositionIndex < 15){
						...
						'N9' {
							'E128_01' vItemCde
							String ReferenceNumber = current_ExternalReference?.ReferenceNumber
							'E127_02' ReferenceNumber?.length() < 30 ? ReferenceNumber : ReferenceNumber?.substring(0,30)
						}
					}
				}
			}
...
//
//								if(City && County && State && PostalCode && Country){
//									String fullCode = vAddressLines?.concat(City)?.concat(County)?.concat(State)?.concat(PostalCode)?.concat(Country)?.trim()
//									'E166_02' util.substring(fullCode?.trim(), 56, 55)?.trim()
//								}
//
//							}
							
//							if(vAddressLines){
//								'E166_02' vAddressLines
//							}
			...
			'L3' {
				...
				if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
					...
					String WeightUnit = current_Body?.GeneralInformation?.BLGrossWeight?.WeightUnit
					String Weight = current_Body?.GeneralInformation?.BLGrossWeight?.Weight;
					if(WeightUnit?.equals('TON')){
						Weight = blUtil.replaceZeroAfterPoint((new BigDecimal(Weight)).multiply(thousand)?.toString())
					}
					if(Weight?.indexOf('.') > 0){
						Weight = blUtil.replaceZeroAfterPoint(Weight)
					}
					'E81_01' Weight.length() < 10 ? Weight : Weight.substring(0,10);
					'E187_02' 'G'
					...
					String WeightUnit = current_Body?.GeneralInformation?.BLNetWeight?.WeightUnit
					String Weight = current_Body?.GeneralInformation?.BLNetWeight?.Weight;
						...
						'Loop_L1' {
							'L1' {
								...
								if(current_FreightCharge?.ChargeType == blUtil.PREPAID){
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										...
										'E117_06' TotalAmtInPmtCurrency?.length() < 9? TotalAmtInPmtCurrency : TotalAmtInPmtCurrency?.substring(0,9)
									}
								}
							...
							if(current_FreightCharge?.TotalAmtInPmtCurrency?.attr_Currency){
								'C3' {
									...
									if(ExchangeRate?.trim()?.indexOf(".") > -1 && ExchangeRate?.length() < 5){
										'E280_02' padfront(ExchangeRate,5,"0")
									}else if(ExchangeRate?.trim()?.indexOf(".") > -1 && ExchangeRate?.length() > 11){
										'E280_02' ExchangeRate.substring(0,11)
									}else if(ExchangeRate?.length() < 4){
										'E280_02' padfront(ExchangeRate,4,"0")
									}else{
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.filterDELinTxnAction(current_Body, true, "Delete is filtered out", errorKeyList)
		...
		blUtil.missingBLNumber(current_Body, true,null, errorKeyList)
		blUtil.missingCntrrNum(current_Body, true,null, errorKeyList)
		blUtil.missingDeliveryDate(current_Body, true,null, errorKeyList)  // Delivery Date / B3_09_32
		blUtil.sendStatusOfEDI(current_Body, true, null, errorKeyList)
		blUtil.nonUSCAMXinPOD(current_Body, false,"Not Import shipment to NA", errorKeyList)
		blUtil.noL3(true, null, errorKeyList,current_Body)  //Sum of Cargo package Quantity is Zero
		return errorKeyList;
	}
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkNonTotalAmtCollectSum(node,true, null, errorKeyList) //Net Amount Due / B3_07_193.
	...
	void syntaxValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkV104_M(node, true, "Voyage Number /", errorKeyList)  //Voyage Number
		blUtil.checkV102_M(node, true, "Vessel Name / V1_02_182", errorKeyList)
