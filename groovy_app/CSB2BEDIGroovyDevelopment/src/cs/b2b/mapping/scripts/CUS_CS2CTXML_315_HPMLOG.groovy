...
class CUS_CS2CTXML_315_HPMLOG {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('HPMLOG', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  util.getConversionWithoutTP('CT', 'O', 'HPMLOG', vCS1EventFirst5, conn)	//get the first 5 char, in case duplicate event missing direction.
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('HPMLOG', conn)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('HPMLOG', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		//EventNotSub  pass empty string, since no corresponding int_cde in DB
		ctUtil.EventNotSub(eventCS2Cde, eventExtCde, false, '', errorKeyList)
		...
		//CSUpload	R4
		FNDCountryMustBeUS(eventCS2Cde, current_Body?.Route?.FND, false, null, errorKeyList)
		//R4 5
	...
	void FNDCountryMustBeUS(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.FND FND, boolean isError, String errorMsg , List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		def cntrcode = null
		if (FND?.CSStandardCity?.CSCountryCode && FND?.CSStandardCity?.CSCountryCode?.length() >= 2){
