...
/**
 * @author Amy
 * MGSEABOARD CT initialize on 20170621
 */
public class CUS_CS2CTXML_315_MGSEABOARD {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('MGSEABOARD', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  null 		//get the first 5 char, in case duplicate event missing direction.
		if(util.getConversion('MGSEABOARD','CT', 'O','EventDirection',vCS1EventFirst5, conn)){ 
			shipDir = util.getConversion('MGSEABOARD','CT', 'O','EventDirection',vCS1EventFirst5, conn)
		 ...
		 def ContainerSizeType = util.getConversion('MGSEABOARD','CT','O','ContainerType',current_Body?.Container?.CarrCntrSizeType,conn)
...
//===============================================N9 Start=======================================================
			...
			current_Body?.BookingGeneralInfo?.findAll{StringUtil.isNotEmpty(it?.CarrierBookingNumber?.trim())}?.each{current_BookingGeneralInfo ->
				'N9' {
					'E128_01' 'BN'
					'E127_02' util.substring(current_BookingGeneralInfo?.CarrierBookingNumber?.trim(),1,30)
				}
			}
           ...
           //---------------------------Scac--------------------------------------------------------------
			...
			//-----------------------------ExternalReference-------------------------------------------------------
            def externalReferenceTypeMap = ['CIN':'IN','CR':'CR','CTR':'CT','FM':'FM','FR':'FN','PO':'PO','PR':'Q1','RP':'RP','SC':'E8','SID':'SI','SO':'SO','SR':'SR','TARIF':'TS']
...
//===============================================Q2 START=======================================================
			...
			if (shipDir == 'I') {
				...
				'Q2' {
					...
					if (firstOceanLeg?.SVVD?.Loading?.VesselName) { //TODO: MapVesselNameFrom1stLoadingVesselNameWithSubstring
...
//===============================================R4 OVER========================================================
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('MGSEABOARD', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
