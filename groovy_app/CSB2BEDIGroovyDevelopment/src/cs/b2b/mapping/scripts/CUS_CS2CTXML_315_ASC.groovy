...
/**
 * @author LIYU
 * ASC CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_ASC {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('ASC','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('ASC', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		//def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('ASC', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('ASC','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('ASC', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
