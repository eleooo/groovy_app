...
class CUS_CS2CTXML_315_WMDAMCO {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('WMDAMCO', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  util.getConversionWithoutTP('CT', 'O', 'WMDAMCO', vCS1EventFirst5, conn)	//get the first 5 char, in case duplicate event missing direction.
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				// B4	03-157	7C
				def fndCountryCode = null
				if (current_Body.Route?.FND?.CSStandardCity?.CSCountryCode && current_Body.Route?.FND?.CSStandardCity?.CSCountryCode?.length() >= 2){
					fndCountryCode = util.substring(current_Body.Route?.FND?.CSStandardCity?.CSCountryCode,1,2)?.trim()
				}
				def tmp_int_cde = vCS1Event + '_FND_' + fndCountryCode
				def tmp_ext_cde = util.getConversion('WMDAMCO', 'CT', 'O', 'EventStatusCode', tmp_int_cde, conn)
				if (tmp_ext_cde) {
					'E157_03' tmp_ext_cde
				} else if(current_Body.GeneralInformation?.TransportMode?.toUpperCase()=='TRUCK'&& 'CS190' == vCS1EventFirst5)	{
					'E157_03' 'AM'
				} else if(current_Body.GeneralInformation?.TransportMode?.toUpperCase()!='TRUCK'&& 'CS190' == vCS1EventFirst5 ) {
					'E157_03' 'OA'
				} else {
				...
				def container_digit = (util.substring(current_Body.Container?.ContainerNumber?.trim(), 5, 6)?.trim() ?: '') + (util.substring(current_Body.Container?.ContainerCheckDigit?.trim(), 1, 2)?.trim() ?: '')
				if(container_digit){					
					'E207_08' container_digit
				} 
				...
				// B4	O	B4	11-310	21C
//				tmp_int_cde = util.substring(current_Body?.Route?.FND?.Facility?.FacilityCode, 3, 5)?.toUpperCase()		//substring of facility instead of route-filler
				tmp_int_cde = util.substring(current_Body?.Route?.FND?.Facility?.FacilityCode, 1, 5)?.toUpperCase()
				// query DB version 
//				tmp_ext_cde = getFacilityCodeExtCde('WMDAMCO', tmp_int_cde)
//				if ( StringUtil.isNotEmpty(tmp_ext_cde) ) {
//					'E310_11' tmp_ext_cde
...
//				} 
				...
				// B4	O	B4	11-310	21C hardcoded version
				def fcl_cde_map = ['CHS01' : 'L391', 'CHS05' : 'N598', 'DAL21' : 'T926', 'HOU20' : 'S639', 'JOL20' : 'H572', 
					'LAS02' : 'Y257', 'LAS03' : 'Y790', 'LAS05' : 'W158', 'LAS09' : 'Y258', 'LGB01' : 'W183', 'ORF04' : 'N195', 
					'ORF07' : 'L045', 'ORF08' : 'L005', 'SAV03' : 'L737', 'SEA10' : 'X117']
				
				tmp_ext_cde = fcl_cde_map.get(tmp_int_cde) 
				...
				//Removed by Supp case, to Syn Prod b2b1.0 before
			...
			if (current_Body.BLGeneralInfo?.size() == 0 && current_Body.BookingGeneralInfo?.size() > 0 && current_Body.BookingGeneralInfo[0]?.CarrierBookingNumber) {
				'N9' {
					'E128_01' 'BM'
					'E127_02' util.substring(current_Body.BookingGeneralInfo[0]?.CarrierBookingNumber?.trim(), 1, 30)?.trim()
			...
			// implement xpath function normalize-space() via replaceAll() method and trim() method
			current_Body.Container?.Seal?.findAll{StringUtil.isNotEmpty(it.SealNumber) && it.SealNumber.length() >= 4}.each { Seal ->
				'N9' {
					'E128_01' 'SN'
					'E127_02' util.substring(Seal?.sealNumber.replaceAll("\\s+", " ").trim(), 1, 12) 
				}
			}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		Map<String,String> duplicatedPairs = ctUtil.getDuplicatedPairs('WMDAMCO', conn)
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('WMDAMCO', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		ctUtil.EventIsNotSubscribedByParnter(eventCS2Cde, eventExtCde, true, null, errorKeyList) 
		...
		// Event not subscribe for POD:US/CA
		eventNotSubscribeForPODUSCA(eventCS2Cde, current_Body?.Route?.LastPOD, false, null, errorKeyList)
		// Event not subscribe for FND:US
		eventNotSubscribeForFNDUS(eventCS2Cde, current_Body?.Route?.FND, true, null, errorKeyList)
		// NATEDI1110L  Cannot find corresponding firm code by facility 
	//	NATEDI1110LCannotFindCorrespondingFirmCodeByFacility(eventCS2Cde, current_Body, true, null, errorKeyList)
	...
	void eventNotSubscribeForPODUSCA(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.LastPOD lastPOD, boolean isError, String errorMsg , List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		def cntrcode = null
		if (lastPOD?.Port?.CSCountryCode && lastPOD?.Port?.CSCountryCode?.length() >= 2){
			cntrcode = lastPOD.Port?.CSCountryCode?.trim()
		}
		if ( ['CS275','CS275'].contains(eventCS2Cde) ){
			errorKey = [TYPE: isError? ERROR_SUPPORT:ERROR_COMPLETE, IS_ERROR: isError? YES : NO, VALUE: errorMsg !=null? eventCS2Cde + errorMsg : eventCS2Cde + 'Event not subscribe for POD:US/CA']
			errorKeyList.add(errorKey)
		}
	}
	
	void eventNotSubscribeForFNDUS(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.FND FND, boolean isError, String errorMsg , List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		def cntrcode = null
		if (FND?.CSStandardCity?.CSCountryCode && FND?.CSStandardCity?.CSCountryCode?.length() >= 2){
			cntrcode = util.substring(FND?.CSStandardCity?.CSCountryCode,1,2)?.trim()
		}
		if(cntrcode == 'US' && ['CS300', 'CS220'].contains(eventCS2Cde)){
			errorKey = [TYPE: isError? ERROR_SUPPORT:ERROR_COMPLETE, IS_ERROR: isError? YES : NO, VALUE: errorMsg !=null? eventCS2Cde + errorMsg : eventCS2Cde + ' - FND Country Must be US']
			errorKeyList.add(errorKey)
		}
	}

	void NATEDI1110LCannotFindCorrespondingFirmCodeByFacility(String eventCS2Cde, cs.b2b.core.mapping.bean.ct.Body current_Body, boolean isError, String errorMsg, List<Map<String, String>> errorKeyList) {
		Map<String, String> errorKey = null
		def facilityCode = current_Body?.Route?.FND?.Facility?.FacilityCode
		def fcl_cde_map = ['CHS01' : 'L391', 'CHS05' : 'N598', 'DAL21' : 'T926', 'HOU20' : 'S639', 'JOL20' : 'H572',
			'LAS02' : 'Y257', 'LAS03' : 'Y790', 'LAS05' : 'W158', 'LAS09' : 'Y258', 'LGB01' : 'W183', 'ORF04' : 'N195',
			'ORF07' : 'L045', 'ORF08' : 'L005', 'SAV03' : 'L737', 'SEA10' : 'X117']
		def tmp_int_cde = util.substring(facilityCode, 1, 5)
		def tmp_ext_cde = fcl_cde_map.get(util.substring(facilityCode, 1, 5))
	
		if ((eventCS2Cde == 'CS130' || eventCS2Cde == 'CS260') && StringUtil.isEmpty(tmp_ext_cde)) {
			errorKey = [TYPE: isError ? ERROR_SUPPORT : ERROR_COMPLETE, IS_ERROR: isError ? YES : NO, VALUE: 'NATEDI1110L  Cannot find corresponding firm code by facility ' + tmp_int_cde]
			errorKeyList.add(errorKey)
		}
	}
	...
	// B4 11-310 21C
	// return the first ext_cde 
	public String getFacilityCodeExtCde(String tp_id, String int_cde, Connection conn) {
		String result = null;
		if (conn == null)
			return result;
		...
		String sql = "select Ext_cde from b2b_cde_conversion where convert_type_id = 'FacilityCode' and dir_id = 'O' and msg_type_id = 'CT' and tp_id = ? and int_cde = ? and remarks = 'Firms Code' order by create_ts desc";
