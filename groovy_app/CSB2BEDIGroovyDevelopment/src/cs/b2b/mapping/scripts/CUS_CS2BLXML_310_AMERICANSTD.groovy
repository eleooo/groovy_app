...
/**
* @author zero he
* @pattern TP=AMERICANSTD after TP=HDYOW
*/
public class CUS_CS2BLXML_310_AMERICANSTD {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			///////////////////   B3-start
			'B3' {
				if( current_Body?.GeneralInformation?.BLType!=null && current_Body?.GeneralInformation?.BLType?.toUpperCase() == 'SEA WAYBILL' ){
					'E147_01' '2'
				}else{
					'E147_01' 'B'
				}
				...
				def v_prepaid_charge = filteredFreightCharge?.findAll{ it?.ChargeType !=null && it?.ChargeType == MappingUtil_BL_O_Common.PREPAID }?.size()
				def v_collect_charge = filteredFreightCharge?.findAll{ it?.ChargeType !=null && it?.ChargeType == MappingUtil_BL_O_Common.COLLECT }?.size()
				...
				sum = new BigDecimal(BigDecimal.ZERO);
				filteredFreightCharge.each{current_FreightCharge ->
					if(current_FreightCharge?.TotalAmtInPmtCurrency != null){
						sum = sum + new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString())
					}
				}
				if( sum.compareTo( new BigDecimal(BigDecimal.ZERO) ) > 0){
			...
			current_Body?.GeneralInformation?.CarrierBookingNumber?.each{current_BookingNumber ->
				if(current_Body?.GeneralInformation?.SCACCode != null){
					if(N9PositionIndex < 15){
						N9PositionIndex = N9PositionIndex + 1
						'N9' {
							'E128_01' 'BN'
							'E127_02' current_Body?.GeneralInformation?.SCACCode + current_BookingNumber
						}
					}
				}
			}
			...
			//Loop by BLNum ONCE
			if( current_Body?.GeneralInformation?.SCACCode != null && 
				current_Body?.GeneralInformation?.BLNumber != null
				){
				if(N9PositionIndex < 15){
					N9PositionIndex = N9PositionIndex + 1
					'N9' {
						'E128_01' 'BM'
						'E127_02' current_Body?.GeneralInformation?.SCACCode + current_Body?.GeneralInformation?.BLNumber
					}
				}
			}
			...
			[
				'CR':'CR' ,'CTR':'CT' ,'EXPR':'RF' ,'FID':'FI' ,'GRN':'GN' ,'ICR':'CR' ,'INV':'IK' ,'OTH':'ZZ' ,
				'PO':'PO' ,'QUOT':'QN','SC':'E8'   ,'SCA':'SCA','SID':'SI' ,'SO':'SO'  ,'SR':'SR'  ,'TARIF':'TI',
				'WARG':'QA' ] 
				...
				'Loop_LX' {
					...
					if(util.substring(containerGroup[0]?.ContainerNumber, 5, 5)){
						'Loop_N7' {
							...
							//select * from b2b_cde_conversion t where t.convert_type_id = 'CTNRPIECECOUNTUNIT' and t.dir_id = 'O' and t.msg_type_id = 'BL' and t.msg_fmt_id = 'X.12' and t.tp_id = 'AMERICANSTD'
							//DB has no result
								...
								'Loop_H1' {
									if(current_DGCargoSpec?.UNNumber){
										'H1' {
											...
											if(current_DGCargoSpec?.EmergencyContact[0]?.FirstName){
											...
											if( null != current_DGCargoSpec?.FlashPoint?.Temperature && 
												current_DGCargoSpec?.FlashPoint?.Temperature.length() >= 1 ){
												if(null != current_DGCargoSpec?.FlashPoint?.Temperature && current_DGCargoSpec?.FlashPoint?.Temperature.length() >= 3){
													'E77_07' current_DGCargoSpec?.FlashPoint?.Temperature.substring(0, 3);
												}else{
			...
			'L3' {
				if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
					...
					if(vWeight!=null && vWeight.length()>=10){
						vWeight = vWeight.substring(0,10);
					}
					'E81_01'  vWeight
					'E187_02' 'G'
			...
			///////////////////   Loop_L1-start
			int L1_occur = 0;
			String var_CalculateMethod = "";
			String var_CalculateMethod_tirm = "";
			String var_CalculateMethod_upper = "";
			int L1_2_06_MaxLength = 9;
			String JavaCode_Output = "";
			String input_TotalAmtInPmtCurrency = "0";
			Map<String, String> CalculateMethod_pre_map =
					[ 'CONTAINER':'CONTAINER', 'GROSS CARGO WEIGHT':'GROSS_CARGO_WEIGHT', 'NET CARGO WEIGHT':'NET_CARGO_WEIGHT',
					  'LUMP SUM':'LUMP_SUM', 'MEASUREMENT':'MEASUREMENT', 'PACKAGE':'PACKAGE' ];
			Map<String, String> CalculateMethod_map =
					[ 'OTHER'				:'OTHER', 'PACKAGE'				:'PU'   , 'NET_CARGO_WEIGHT'		:'WM'   , 'MEASUREMENT'			:'VM'   ,
					 'LUMP_SUM'				:'LS'   , 'CONTAINER'			:'PA'   , 'B'					:'CS'   , 'GROSS_CARGO_WEIGHT'	:'WM' ];
			List<String> CalculateMethod_list = ['PA','LS','VM','WM','PU','CS'];
			filteredFreightCharge?.eachWithIndex{current_FreightCharge, FreightChargeIndex->
				
				if(current_FreightCharge?.ChargeType && StringUtil.isNotEmpty(current_FreightCharge?.TotalAmtInPmtCurrency?.TotalAmtInPmtCurrency)){
					'Loop_L1' {
						'L1' {
							++L1_occur;
							'E213_01' L1_occur
							if(StringUtil.isNotEmpty(current_FreightCharge?.FreightRate)){
								'E60_02' current_FreightCharge?.FreightRate
							}
							
							if(StringUtil.isNotEmpty(current_FreightCharge?.CalculateMethod)){
								var_CalculateMethod = current_FreightCharge?.CalculateMethod;
								var_CalculateMethod_tirm = var_CalculateMethod.trim(); 
								var_CalculateMethod_upper = var_CalculateMethod.toUpperCase().trim();
								
								if( var_CalculateMethod.substring(0,1) == '2' || var_CalculateMethod.substring(0,1) == '4' && (CalculateMethod_map.get("CONTAINER") != null ) ){
									var_CalculateMethod = CalculateMethod_map.get("CONTAINER")  
								}else if(null != CalculateMethod_map.get( CalculateMethod_pre_map.get(var_CalculateMethod_upper) )){
									var_CalculateMethod = CalculateMethod_map.get(CalculateMethod_pre_map.get(var_CalculateMethod_upper))
								}else if( var_CalculateMethod.length() >= 1 && var_CalculateMethod.substring(0,1) == 'B' && null != CalculateMethod_map.get("B") ){
								    var_CalculateMethod = CalculateMethod_map.get("B")
								}else if( var_CalculateMethod_tirm == 'PA' && null != CalculateMethod_map.get("OTHER") ){
								    var_CalculateMethod = var_CalculateMethod_tirm;
								}else if( CalculateMethod_list.contains(var_CalculateMethod_tirm) && null != CalculateMethod_map.get("OTHER") ){
								    var_CalculateMethod = var_CalculateMethod_tirm;
								}
								if( null!=var_CalculateMethod && var_CalculateMethod.length()==2 ){
									'E122_03' var_CalculateMethod
								}
							}
							if( null != current_FreightCharge?.ChargeType && current_FreightCharge?.ChargeType == blUtil.COLLECT &&
								current_FreightCharge?.TotalAmtInPmtCurrency != null && current_FreightCharge?.TotalAmtInPmtCurrency != '' ){
							...
							if( null != current_FreightCharge?.ChargeType && current_FreightCharge?.ChargeType == MappingUtil_BL_O_Common.PREPAID && 
								current_FreightCharge?.TotalAmtInPmtCurrency != null && current_FreightCharge?.TotalAmtInPmtCurrency != '' ){
								input_TotalAmtInPmtCurrency = current_FreightCharge?.TotalAmtInPmtCurrency?.toString();
								BigDecimal TotalAmtInPmtCurrency_BigDecimal = new BigDecimal(input_TotalAmtInPmtCurrency);
								BigDecimal hundred = new BigDecimal ('100');
								JavaCode_Output = TotalAmtInPmtCurrency_BigDecimal.multiply(hundred).doubleValue().toString();
								if(JavaCode_Output.length() >= L1_2_06_MaxLength){
									'E117_06' JavaCode_Output.substring(0,L1_2_06_MaxLength);
								}else{
									'E117_06' JavaCode_Output
								}
							}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn)  {
		...
		bl?.Body?.eachWithIndex{current_Body, current_BodyIndex ->
			...
			//ChargeFilter
			blUtil.filterByChargeType(filteredFreightCharge, null, '1')
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.nonUSCAinPOD(false,"Non US or CA shipments", errorKeyList,current_Body)
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		Node root = xmlParser.parseText(outputXml + "</T310>")
		//
		blUtil.checkL102L106L307Checker( root, true, "The Length of L307,L102,L106 is over.",errorKeyList)
		blUtil.checkL0Max(root, 120, true, null,errorKeyList)
		blUtil.checkH1_FlashPoint(root,true,"Flashpoint's length max.3", errorKeyList)
		blUtil.checkL1max( root, 20, true, "L1 segment more than 20.", errorKeyList)
		blUtil.checkL106Length( root, 9, true, "L106's length max.9", errorKeyList)
	}
	...
	void syntaxValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		XmlParser xmlParser = new XmlParser();
		Node root = xmlParser.parseText(outputXml + "</T310>")
		//Exception	EDISyntax	Exception
		blUtil.checkB2A_M(root, true,"B2A.01.353 is mandatory.",errorKeyList)
		blUtil.checkB309_B310_P(root, true, "Either B309 B310 exists then all are required.", errorKeyList)
		blUtil.checkW0902_W0903_P(root, true, "Either W0902 W0903 exists then all are required.", errorKeyList)
		blUtil.checkV101_V102_R(root, true, "At least one of V101, V102 is required.", errorKeyList)
		blUtil.checkQTY01_QTY02_R(root, true, "At least one of QTY01,QTY02 is required.", errorKeyList)
		blUtil.checkQTY01_M(root, true, "QTY_01 is mandatory.", errorKeyList)
		blUtil.checkN902_N903_R(root, true, "At least one of N902, N903 is required.", errorKeyList)
		blUtil.checkN901_N902_M(root, true, "N901 and N902 are mandatory.", errorKeyList)
		blUtil.checkN703_N704_P(root, true, "Either N703 N704 exists then all are required.", errorKeyList)
		blUtil.checkC803_C802_R(root, true, "At least one of C802,C803 required.", errorKeyList)
		blUtil.checkDTM02_DTM03_DTM_05_R( root, true, "At least one of DTM02,DTM03,DTM05 required.",  errorKeyList)
		blUtil.checkDTM04_DTM03_C( root, true, "If DTM04 is present, DTM03 is required.",  errorKeyList)
		blUtil.checkG6103_G6104_P(root, true, "Either G6103, G6104 present other are required.", errorKeyList)
		blUtil.checkH101_M(root, true, "H101 is mandatory.", errorKeyList)
		blUtil.checkH107_H108_P(root, true, "Either H107 H108 exists then all are required.", errorKeyList)
		blUtil.checkL301_L302_P(root, true, "Either L301 L302 exists then all are required.", errorKeyList)
		blUtil.checkL309_L310_P(root, true, "Either L309 L310 exists then all are required.", errorKeyList)
		blUtil.checkL312_L301_C(root, true, "If L312 is present, L301 is required.", errorKeyList)
		blUtil.checkN101_N102_M(root, true, "N101 and N102 are mandatory.", errorKeyList)
		blUtil.checkN102_N103_R(root, true, "At least one of N102, N103 is required.", errorKeyList)
	}
