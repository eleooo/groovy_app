...
public class CUS_CS2BLXML_310_GTNEXUS_LIFUNGLOGGTN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			current_Body?.Party?.each{current_Party ->
				if(partyTypeN9.get(current_Party?.PartyType)&&current_Party?.CarrierCustomerCode){
						...
						'Loop_N7' {
							...
							if(util.substring(current_Container?.ContainerNumber, 5, 10)){
								'N7' {
									...
									BigDecimal sumVolume = new BigDecimal(BigDecimal.ZERO)
			...
			//LIFUNGGTN do not need L1
//			filteredFreightCharge?.findAll{(it?.ChargeType == blUtil.COLLECT || it?.ChargeType == blUtil.PREPAID) && StringUtil.isNotEmpty(it?.TotalAmtInPmtCurrency?.TotalAmtInPmtCurrency)}.eachWithIndex{current_FreightCharge, FreightChargeIndex->
//				'Loop_L1' {
//					'L1' {
...
//			}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.nonUSCAinPOD(false, 'Not Import to US or CA', errorKeyList, current_Body)
		return errorKeyList;
	}
