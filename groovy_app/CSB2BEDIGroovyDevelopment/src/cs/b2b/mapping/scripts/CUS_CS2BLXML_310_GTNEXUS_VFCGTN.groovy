 ...
 */
public class CUS_CS2BLXML_310_GTNEXUS_VFCGTN {
		...
		outXml {
			...
			'B3' {
				...
				if(current_Body?.GeneralInformation?.BLNumber){
					'E76_02' current_Body?.GeneralInformation?.SCACCode + current_Body?.GeneralInformation?.BLNumber
				}
				...
				def v_prepaid_charge = current_Body?.FreightCharge?.findAll{it.ChargeType == blUtil.PREPAID}?.size()
				def v_collect_charge = current_Body?.FreightCharge?.findAll{it.ChargeType == blUtil.COLLECT}?.size()
			...
			String spaIORNumber = ''
			String nptIORNumber = ''
			if(util.substring(current_Body?.Route?.FirstPOL?.Port?.LocationCode?.UNLocationCode?.toUpperCase(), 1, 2) == 'US' && StringUtil.isNotEmpty(current_Body?.Party?.find{it?.PartyType == 'SHP'}?.CarrierCustomerCode?.trim())){
				sapID = current_Body?.Party?.find{it?.PartyType == 'SHP'}?.CarrierCustomerCode?.trim()
			}
			else if(util.substring(current_Body?.Route?.FirstPOL?.Port?.LocationCode?.UNLocationCode?.toUpperCase(), 1, 2) != 'US' && StringUtil.isNotEmpty(current_Body?.Party?.find{it?.PartyType == 'CGN'}?.CarrierCustomerCode?.trim())){
				sapID = current_Body?.Party?.find{it?.PartyType == 'CGN'}?.CarrierCustomerCode?.trim()
			}
			else if(current_Body?.Party?.find{it?.PartyType == 'CGN'}?.PartyName == 'To Order' || !current_Body?.Party?.find{it?.PartyType == 'CGN'}){
				if(StringUtil.isNotEmpty(current_Body?.Party?.find{it?.PartyType == 'NPT'}?.CarrierCustomerCode?.trim()) &&  util.substring(current_Body?.Route?.FirstPOL?.Port?.LocationCode?.UNLocationCode?.toUpperCase(), 1, 2) != 'US'){
					sapID = current_Body?.Party?.find{it?.PartyType == 'NPT'}?.CarrierCustomerCode?.trim()
				}
			}			

			if(StringUtil.isNotEmpty(sapID)){
				spaIORNumber = getEDICdeReffromIntCde(TP_ID, 'SAP_IOR_MAP', sapID, conn)
			}
			if(StringUtil.isEmpty(spaIORNumber) && StringUtil.isNotEmpty(current_Body?.Party?.find{it?.PartyType == 'NPT'}?.CarrierCustomerCode?.trim())){
				sapID_np = current_Body?.Party?.find{it?.PartyType == 'NPT'}?.CarrierCustomerCode?.trim()
				nptIORNumber = getEDICdeReffromIntCde(TP_ID, 'SAP_IOR_MAP', sapID_np, conn)
			}
			if(StringUtil.isNotEmpty(nptIORNumber)){
				'N9' {
					if(nptIORNumber && !nptIORNumber?.toUpperCase()?.trim()?.startsWith('SKIP')){
						'E128_01' 'TJ'
						'E127_02' nptIORNumber
					}
				}
			}else if(StringUtil.isNotEmpty(spaIORNumber)){
				'N9' {
					if(spaIORNumber && !spaIORNumber?.toUpperCase()?.trim()?.startsWith('SKIP')){
						'E128_01' 'TJ'
						'E127_02' spaIORNumber
					}
				}
			}else{
				'N9' {
					'E128_01' 'TJ'
					'E127_02' 'ERROR'
					'E369_03' "${sapID? sapID : ''}/${sapID_np? sapID_np : ''}"
				}
			}
				...
				'R4' {
					...
					if(POR){
						...
						if(POR?.CityDetails?.City){
							//Customization  OverrideR404	if(POR?.CityDetails?.City == BA RIA) 'E114_04' Vung Tau
							if(util.substring(POR?.CityDetails?.City, 1, 24)?.toUpperCase() == 'BA RIA'){
								'E114_04' 'Vung Tau'
			...
			'Loop_R4' {
				'R4' {
					...
					if(POL){
						...
						//Customization  OverrideR404	if(POL?.Port?.City == BA RIA) 'E114_04' Vung Tau
						if(util.substring(POL?.Port?.City, 1, 24)?.toUpperCase() == 'BA RIA'){
								'E114_04' 'Vung Tau'
			...
			'Loop_R4' {
				'R4' {
					...
					if(POD){
						...
						//Customization  OverrideR404	if(POD?.Port?.City == BA RIA) 'E114_04' Vung Tau
						if(util.substring(POD?.Port?.City, 1, 24)?.toUpperCase() == 'BA RIA'){
								'E114_04' 'Vung Tau'
			...
			'Loop_R4' {
				'R4' {
					...
					if(FND){
						...
						//Customization  OverrideR404	if(FND?.CityDetails?.City == BA RIA) 'E114_04' Vung Tau
						if(util.substring(FND?.CityDetails?.City, 1, 24)?.toUpperCase() == 'BA RIA'){
								'E114_04' 'Vung Tau'
									...
									if(current_DGCargoSpec?.get(0)?.UNNumber){
										'Loop_H1'{
											'H1' {
												...
												'E254_09' current_DGCargoSpec?.get(0)?.PackageGroup?.Code?.toUpperCase()?.toUpperCase()
											}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.cC_Not_USD(current_Body, false, null, errorKeyList)
		return errorKeyList;
	}
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkMISSING_SAP_IOR(node,  true, null, errorKeyList)
	}
	...
	public String getEDICdeReffromIntCde(String TP_ID, String convertTypeId, String fromValue, Connection conn) throws Exception {
		if (!conn) {
			throw new Exception("DB Connection is not available. ");
		}
		if (!TP_ID || !convertTypeId || !fromValue) {
			return ""
		}
		String ret = "";
		PreparedStatement pre = null;
		ResultSet result = null;
		String sql = "SELECT EXT_CDE FROM B2B_EDI_CDE_REF R WHERE R.SRC_FMT_ID = 'BLUPDATEO' AND R.TRG_FMT_ID = 'X12' AND R.DIR_ID = 'O' AND R.CONVERT_TYPE_ID = ? AND TP_ID = ? AND INT_CDE = ?";
		try {
