 ...
 */
public class CUS_CS2BLXML_310_ADISSEO {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			//Y2 has done by K
			current_Body?.Container?.groupBy {util.getCdeConversionImpl('GTNEXUS', '', 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', it?.CS1ContainerSizeType, true, conn)?.get("EXT_CDE")}?.each{currentContainerSizeType, currentContainerGroup->
				Map<String, String> retMap = util.getCdeConversionImpl('GTNEXUS', '', 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', currentContainerGroup[0]?.CS1ContainerSizeType, true, conn)
							...
							if(util.substring(current_Container?.ContainerNumber, 5, 10)){
								'N7' {
									...
									if(current_Container?.CS1ContainerSizeType){
										Map<String, String> retMap = util.getCdeConversionImpl('GTNEXUS', '', 'O', current_Body?.GeneralInformation?.SCACCode, 'X.12', 'ContainerType', current_Container?.CS1ContainerSizeType, true, conn)
										if(retMap.get("EXT_CDE")){
			...
			'L3' {
				...
				if(util.isDecimal(current_Body?.GeneralInformation?.BLNetWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
