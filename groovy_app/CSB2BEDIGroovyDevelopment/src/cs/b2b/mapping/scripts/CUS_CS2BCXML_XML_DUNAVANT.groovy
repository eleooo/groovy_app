 ...
 *             -- difference
 *             -- 1, EmptyPickup, FullReturn, EmptyReturn output FacilityCode and FacilityName, while DELMAR not outupt : StopOff/PickupDetails/FacilityCode
 *             -- 2, Container SizeType use CarrCntrSizeType, query with SCAC need
 *             -- 3, ContainerMovement use ISOSizeType
 *             -- 4, Booking/Confirmation/MessaeBody/ShipmentDetails/RouteInformation/StopOff output FacilityCode
 *             -- 5, has LengthUnitConvert
 *             -- 6, output file name is different
 *             
 ...
 * 
 */
public class CUS_CS2BCXML_XML_DUNAVANT {
						...
						current_Body.ContainerGroup?.Container?.each { current_Container ->
							'Containers' {
								...
								if (current_Container.CarrCntrSizeType) {
									Map<String, String> defCntr = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, current_Body.GeneralInformation?.SCACCode, 'XML', 'ContainerType', current_Container.CarrCntrSizeType, conn)
...
//													}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			String outputFileName = "allenberg_bcxml_" + currentDate.format(yyyyMMddHHmmss) + fileSeqStr
