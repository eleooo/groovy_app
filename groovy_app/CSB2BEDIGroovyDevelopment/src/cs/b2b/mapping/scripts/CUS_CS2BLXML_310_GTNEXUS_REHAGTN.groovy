 ...
 */
public class CUS_CS2BLXML_310_GTNEXUS_REHAGTN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			current_Body?.Container?.each{current_Container ->
				def var = "${current_Container?.ContainerNumber?.trim() ? current_Container?.ContainerNumber?.trim() :''}${current_Container.ContainerCheckDigit?.trim() ? current_Container.ContainerCheckDigit?.trim() :''}"
				for(int i = 0; i * 30 < var?.length(); i++ ) {
					if(countN9 > 14){
						break
					}
					int startIndex = i * 30
					int endIndex = (i + 1) * 30 < var?.length() ? ((i + 1) * 30) : var?.length()
					'N9' {
						'E128_01' 'EQ'
						'E127_02' var[startIndex..<endIndex]
					}
					countN9++
				}
			}
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkNonTotalAmtCollectSum(node, false, 'No collect charges.', errorKeyList)
	}
