 ...
 * @pattern after WGLL
 */
public class CUS_CS2BLXML_XML_TOPOCEANHK {
						...
						if (por) {
							'LocationDetails' {
								...
								if (por?.Facility?.FacilityName){
									...
									'LocationName' por?.Facility?.FacilityName
								}
							...
							if (pol) {
								'PortDetails' {
									...
									if (pol?.Port?.PortName){
										'LocationName' util?.substring(pol?.Port?.PortName, 1, 60)?.trim()
									}else{
							...
							if (pod) {
								'PortDetails' {
									...
									if (pod?.Port?.PortName){
										'LocationName' util?.substring(pod?.Port?.PortName, 1, 60)?.trim()
									}else{
					...
					'EquipmentInformation' {
						current_Body?.Container?.each { current_Container ->
							'Containers' {
								...
								Map<String, String> containerTypeMap = util.getCdeConversionFromIntCde(TP_ID, 'BL', 'O', current_Body.GeneralInformation.SCACCode, 'XML', 'ContainerType', current_Container?.CarrCntrSizeType, conn)
                                ...
                                if(current_Container?.Seal[0]?.SealNumber) {
                                    'SealNumber'('null': 'true', 'Type': current_Container?.Seal[0]?.SealType, util?.substring(current_Container?.Seal[0]?.SealNumber, 1, 17)?.trim())
                                }else{
									'SealNumber'('null': 'true', current_Container?.Seal[0]?.SealNumber)
								}
...
//								'CommodityCode' ''
								...
								Map<String, String> packageTypeMap = util.getCdeConversionFromIntCde(TP_ID, 'BL', 'O', null, 'XML', 'PackageUnit', current_Cargo?.Packaging?.PackageType, conn)

								String packageTypeConversion = packageTypeMap.get("EXT_CDE")
								if(StringUtil.isEmpty(packageTypeConversion)){
									packageTypeConversion = "ZZ"
								}

								'Package'('Type':packageTypeConversion, packageQty)
...
//														'Email' current_EmergencyContact?.ContactEmailAddress
					...
					Map<String, String> referenceTypeMap = ['CR':'CR', 'CTR':'CT', 'EXPR':'RF', 'FCBL':'FCN', 'FID':'FI', 'FR':'FN',
															'INV':'IK', 'PO':'PO', 'PR':'Q1', 'SC':'E8', 'SID':'SI',
															'SO':'SO', 'SR':'SR', 'TARIF':'TS', 'WARG':'QT']
					Map<String, String> referenceNameMap = ['CR':'Customer Reference Number', 'CTR':'Contract Number', 'EXPR':'Export Reference Number',
															'FCBL':'First Carrier B/L Number', 'FID':'File Identifier', 'FR':'Forwarder\'s/Agent\'s reference',
															'INV':'Invoice number', 'PO':'Purchase Order Number', 'PR':'Quotation Number', 'SC':'Service Contract Number',
															'SID':'Shipper\'s Identifying Number For Shipment (SID)', 'SO':'Shipper order number', 'SR':'Shipper Reference',
															'TARIF':'Tariff Number', 'WARG':'Warranty Registration Number']
					'ExternalReferences' {
...
//			}
			//output file name
			String outputFileName = "WGLL_BL_" + currentSystemDt.format(yyyyMMddHHmmss) + ".xml"
...
//      blUtil.checkCarrierBookingNumber(current_Body, true, null, errorList)
		checkCargo(current_Body, true, null, errorList)
		checkContainer(current_Body, true, null, errorList)
...
//        checkCargo(current_Body, true, "Missing Cargo information", errorList )
	 ...
	 */
	void checkContainer(cs.b2b.core.mapping.bean.bl.Body current_Body, boolean isError, String errorMsg, List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		if(current_Body.Container == null || current_Body.Container?.size() == 0){
			errorKey = [TYPE: isError? blUtil.ERROR_SUPPORT : blUtil.ERROR_COMPLETE, IS_ERROR: isError? blUtil.YES : blUtil.NO, VALUE: errorMsg ? errorMsg : "Missing Container,(EquipmentInformation mandatoty and loopbyContainer)"]
	 ...
	 * @Tested test by file in OOCLLOGISTICS
	 */
	void checkCargo(cs.b2b.core.mapping.bean.bl.Body current_Body, boolean isError, String errorMsg, List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		if(current_Body.Cargo == null || current_Body.Cargo?.size() == 0){
			errorKey = [TYPE: isError? blUtil.ERROR_SUPPORT : blUtil.ERROR_COMPLETE, IS_ERROR: isError? blUtil.YES : blUtil.NO, VALUE: errorMsg ? errorMsg : "Missing Cargo information"]
			errorKeyList.add(errorKey)
		}
	}
