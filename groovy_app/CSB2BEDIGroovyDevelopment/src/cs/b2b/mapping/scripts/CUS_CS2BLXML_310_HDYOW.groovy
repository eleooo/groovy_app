...
*/
public class CUS_CS2BLXML_310_HDYOW {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['BKG':'BN','BL':'BM','CTR':'CT','EXPR':'RF','FID':'FI','GEN':'GN','ICR':'CR','INV':'IK','MB':'BN',
								...
								if(current_FreightCharge?.FreightRate){
									...
									if(calculateMethod!=null&&calculateMethod!=""){
										if(calculateMethodList.contains(calculateMethod) || calculateMethod.charAt(0)== 'B' || calculateMethod.charAt(0)== '2' || calculateMethod.charAt(0)== '4' ||
											...
											//tibco has a bug in this place,so need to special treatment
//											if(freightRate?.indexOf(".")>0 && freightRate?.substring(freightRate?.indexOf(".")+1, freightRate?.length())?.length()==3){
//												freightRate = freightRate?.toString()?.concat("0")
//											}else if(freightRate?.indexOf(".")>0 && freightRate?.substring(freightRate?.indexOf(".")+1, freightRate?.length())?.length()==4){
...
//											}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.nonUSinFND(true,"Must be inbound to US.", errorKeyList,current_Body)
