 ...
 */
public class CUS_CS2CTXML_315_DENNISEASTYUSEN{
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('DENNISEASTYUSEN', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		//def shipDir = util.getConversion('DENNISEASTYUSEN','CT','O','EventDirection','vCS1EventFirst5',conn) // need add segnum EventStatusCode?

		def shipDir = util.getEDICdeReffromIntCde('DENNISEASTYUSEN','ShipDirection','O','INTCTXML','Q2','EventStatusCode',vCS1Event,conn)
		...
		outXml.'Loop_ST' {
			...
			'B4' {
				...
				if(current_Body?.Container?.ContainerCheckDigit){
					'E761_13'  util.substring(current_Body?.Container?.ContainerCheckDigit, 1,1)   //1.0 only get first 1 char of ContainerCheckDigit
				}
			}
			...
			//Tibco 1.0 Q2 precondition: $getB4/group/B4status ="C"  and  $getN9/root/group/N9status!="E", then getQ2
			if (N9Count>0){
				if (shipDir == 'I') {
					'Q2'{
			...
			'Loop_R4' {
				'R4' {
					'E115_01' 'R'
					if(util.isNotEmpty(POR?.CityDetails?.LocationCode?.SchedKDCode)&&util.isNotEmpty(POR?.CityDetails?.LocationCode?.SchedKDType)){
						'E309_02' POR?.CityDetails?.LocationCode?.SchedKDType
					} else if (util.isNotEmpty(POR?.CityDetails?.LocationCode?.UNLocationCode) && util.isEmpty(POR?.CityDetails?.LocationCode?.SchedKDCode)) {
						'E309_02' 'UN'
					}
			...
			'Loop_R4' {
				'R4' {
					...
					if(util.isNotEmpty(lastPOD?.Port?.LocationCode?.SchedKDCode) && util.isNotEmpty(lastPOD?.Port?.LocationCode?.SchedKDType)){
						'E309_02' lastPOD?.Port?.LocationCode?.SchedKDType
					} else if (util.isNotEmpty(lastPOD?.Port?.LocationCode?.UNLocationCode) && util.isEmpty(lastPOD?.Port?.LocationCode?.SchedKDCode)) {
						'E309_02' 'UN'
					}
			...
			'Loop_R4' {
				'R4' {
					'E115_01' 'L'
					if (util.isNotEmpty(firstPOL?.Port?.LocationCode?.SchedKDCode)&&util.isNotEmpty(firstPOL?.Port?.LocationCode?.SchedKDType)) {
						'E309_02' firstPOL?.Port?.LocationCode?.SchedKDType
					}
					else if (util.isNotEmpty(firstPOL?.Port?.LocationCode?.UNLocationCode) && util.isEmpty(firstPOL?.Port?.LocationCode?.SchedKDCode)) {
						'E309_02' 'UN'
					}
			...
			'Loop_R4' {
				'R4' {
					...
					if (util.isNotEmpty(FND?.CityDetails?.LocationCode?.SchedKDCode) && util.isNotEmpty(FND?.CityDetails?.LocationCode?.SchedKDType)) {
						'E309_02' FND?.CityDetails?.LocationCode?.SchedKDType
					} else if (util.isNotEmpty(FND?.CityDetails?.LocationCode?.UNLocationCode) && util.isEmpty(FND?.CityDetails?.LocationCode?.SchedKDCode)) {
						'E309_02' 'UN'
					}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('DENNISEASTYUSEN', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		//		ctUtil.missingContainerNumber(eventCS2Cde,current_Body?.Container,true,"- Missing Equipment Number",errorKeyList)
	...
	//CSUPLOAD
	//
	//	public static void missingEventStatusLocation(String eventCS2Cde,String eventUNLocationCode,String eventSchedKDCode,boolean isError,String errorMsg, List<Map<String,String>> errorKeyList){
	//		Map<String,String> errorKey = null
	//
	//		if(StringUtil.isEmpty(eventUNLocationCode) && StringUtil.isEmpty(eventSchedKDCode)){
