...
*/
public class CUS_CS2BLXML_310_CANONDSG {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				if(StringUtil.isNotEmpty(current_Body?.GeneralInformation?.BLNumber)){
					'E76_02' current_Body?.GeneralInformation?.SCACCode?.concat(current_Body?.GeneralInformation?.BLNumber)
				}
				...
				List<FreightCharge> filteredFreigthChargeCOLLECT = filteredFreightCharge?.findAll{it?.ChargeType == blUtil.COLLECT}
				List<FreightCharge> currencyNoneUSDFreightChargeCOLLECT = filteredFreigthChargeCOLLECT?.findAll{it?.TotalAmtInPmtCurrency?.attr_Currency != 'USD'}
...
//				}                       //i think tibco is wrong itself
			...
			if(N9PositionIndex < 15){
				...
				'N9' {
					'E128_01' 'BM'
					'E127_02' current_Body?.GeneralInformation?.SCACCode?.concat(current_Body?.GeneralInformation?.BLNumber)
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['CR':'CR','CTR':'CT','EXPR':'RF','FID':'FI','GEN':'GN','ICR':'CR',
													   'INV':'IK','OTH':'ZZ','PO':'PO','QUOT':'QN','SC':'E8','SCA':'SCA','SID':'SI',
													   'SO':'SO','SR':'SR','TARIF':'TI','WARG':'QA']
			...
			current_Body?.CarrierRate?.each{current_CarrierRate ->
				...
				if(vItemCde){
					if(N9PositionIndex < 15){
						N9PositionIndex = N9PositionIndex + 1
						'N9' {
							if(vItemCde!='SR' || (vItemCde=='SR' && current_CarrierRate?.CarrierRateNumber != systemRef)){//Customization	Default	HERBALIFEVIADSG_N9SR
								if(vItemCde=='ZZ' && current_CarrierRate?.CarrierRateNumber == systemRef ){
									'E128_01' 'FN'
								}else{
									'E128_01' vItemCde
								}
			...
			current_Body?.ExternalReference?.each{current_ExternalReference ->
				...
				if(vItemCde ){
					if(N9PositionIndex < 15){
						N9PositionIndex = N9PositionIndex + 1
						'N9' {
							if(vItemCde!='SR' || (vItemCde=='SR' && current_ExternalReference?.ReferenceNumber != systemRef)){//Customization	Default	HERBALIFEVIADSG_N9SR
								if(vItemCde=='ZZ' && current_ExternalReference.ReferenceNumber == systemRef ){
									'E128_01' 'FN'
								}else{
									'E128_01' vItemCde
								}
			...
			current_Body?.Party?.each{current_Party ->
				if(partyTypeMap.get(current_Party?.PartyType)){
					'Loop_N1' {
						...
						strArray = new String[numberOfLine];
						
						for(int i = 0; i < numberOfLine; i++){
							//Cut for each 55
							if(originalStr.length() > length){
								 strArray[i] = originalStr.substring(0, length);
							 originalStr = originalStr.substring(length);
							}else{
								 strArray[i] = originalStr.substring(0, originalStr.length());
								 originalStr = originalStr.substring(originalStr.length());
							}
						}
						
						def partyAddress = current_Party?.Address
						//i don't  konw why tibco has this but the expected doen't 
//						vAddressLines = "${vAddressLines} ${partyAddress?.City?partyAddress?.City+' ':''}${partyAddress?.County?partyAddress?.County+' ':''}${partyAddress?.State?partyAddress?.State+' ':''}${partyAddress?.PostalCode?partyAddress?.PostalCode+' ':''}${partyAddress?.Country?:''}"?.trim()
						vAddressLines = vAddressLines?.trim()
						
						strArray?.eachWithIndex{ current_str,current_Index -> 
							if(current_Index<2){
								'N3' {
									'E166_01' util.substring(vAddressLines, 1, 55)?.trim()
									if(vAddressLines.length()>55){
										vAddressLines = vAddressLines.substring(55)
									}else{
										vAddressLines = "";
									}
									
									'E166_02' util.substring(vAddressLines, 1, 55)?.trim()
									if(vAddressLines.length()>55){
										vAddressLines = vAddressLines.substring(55)
									}else{
										vAddressLines = "";
									...
									'W09' {
										...
										if(temperature?.contains(".")){
											...
											if(remainTempPoint !=""){
												...
												'E408_02' blUtil.replaceZeroAfterPoint(util.substring(temperatureBeforePoint?.concat(".")?.concat(remainTempPoint),1,4))
											}else{
												if(signal.equals("-")){
													temperatureBeforePoint = "-".concat(temperatureBeforePoint)
												}
...
//				}
					...
					if(current_FreightCharge?.TotalAmtInPmtCurrency){
						'Loop_L1' {
							'L1' {
								...
								}else if(current_FreightCharge?.ChargeCode){
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.missingCollectExchangeRateforNonUSD(current_Body, false, null, errorKeyList)
	...
	void syntaxValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkB2A_M(node, true, null, errorKeyList)
