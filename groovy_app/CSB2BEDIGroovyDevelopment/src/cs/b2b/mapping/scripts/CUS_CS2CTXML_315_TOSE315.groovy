...
/**
 * @author LIYU
 * TOSE315 CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_TOSE315 {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('TOSE315','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('TOSE315', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('TOSE315', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('TOSE315','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('TOSE315', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
