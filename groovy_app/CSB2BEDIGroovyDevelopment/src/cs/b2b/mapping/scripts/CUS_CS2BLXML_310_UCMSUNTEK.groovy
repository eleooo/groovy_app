...
*/
public class CUS_CS2BLXML_310_UCMSUNTEK {
	...
	public void generateBody(
		...
		Body current_Body,
		 Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,
		 List<FreightCharge> filteredFreightCharge,
		 List<FreightChargeCNTR> filteredFreightChargeCNTR,
		  MarkupBuilder outXml, Connection conn) {
			...
			'B3' {
				...
				filteredFreightCharge?.findAll{it?.ChargeType?.equals(blUtil.COLLECT)}?.each { current_FreightCharge ->
					if(current_FreightCharge?.TotalAmtInPmtCurrency){
						BigDecimal TotalAmtInPmtCurrency =  new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString());
//						if(TotalAmtInPmtCurrency > 0){
							sum = sum + TotalAmtInPmtCurrency
//						}
					}
					
				}
				if(sum.multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP) > 0){
					'E193_07' sum.multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP);
				}else{
					'E193_07' '0'
				}
				...
				'R4' {
					...
					if(POR){
						...
						if(POR?.CSStandardCity?.CSStateCode){
							'E156_08' POR?.CSStandardCity?.CSStateCode?.substring(0, 2)
						}
					}
				}
				'DTM' {
			...
			'Loop_R4' {
				'R4' {
					...
					if(POL){
						...
						if(POL?.CSStateCode){
							'E156_08' POL?.CSStateCode?.substring(0, 2)
						}
					}
				}
				if(oceanLegSize > 0){
			...
			'Loop_R4' {
				'R4' {
					...
					if(POD){
						...
						if(POD?.CSStateCode){
							'E156_08' POD?.CSStateCode?.substring(0, 2)
						}
					}
				}
				if(oceanLegSize > 0){
				...
				'R4' {
					...
					if(FND){
						...
						if(FND?.CSStandardCity?.CSStateCode){
							'E156_08' FND?.CSStandardCity?.CSStateCode?.substring(0, 2)
						}
					}
				}
				'DTM' {
			...
			'R2' {
				...
				'E133_02' 'O'
				Map<String, String> trafficModeMap =  ['FCLFCL':'03', 'FCLLCL': '04', 'LCLFCL' :'05', 'LCLLCL' :'02']
...
//											}
		...
		///////////////////   L3-start
			'L3' {
				if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
					...
					'E81_01'  vWeight?.length() < 10 ? vWeight : vWeight.substring(0, 10)
					'E187_02' 'G'
						...
						'Loop_L1' {
							'L1' {
								...
								if(current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)){  //1
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
										String str = blUtil.replaceZeroAfterPoint(new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString()).multiply(hundred).toString());
										'E117_06' str?.length() < 9 ? str : str.substring(0,9)
									}
								}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.nonUSCAinPOD(false, null, errorKeyList,current_Body)
		return errorKeyList;
	}
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		//blUtil.checkL106Length(node, 9, true, null, errorKeyList)   //L106's length max.9 [Invalid Prepaid Charge Amt - 286894522221520]
