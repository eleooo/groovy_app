 ...
 */
public class CUS_CS2CTXML_315_BEDBATHYUSEN{
...
//	}
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('BEDBATHYUSEN', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		//def shipDir = util.getConversion('BEDBATHYUSEN','CT','O','EventDirection','vCS1EventFirst5',conn) // need add segnum EventStatusCode?

		def shipDir = util.getEDICdeReffromIntCde('BEDBATHYUSEN','ShipDirection','O','INTCTXML','Q2','EventStatusCode',vCS1Event,conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body?.Container?.Seal?.findAll{StringUtil.isNotEmpty(it?.SealNumber) && it?.SealNumber?.length() >= 4}?.groupBy{it?.SealNumber?.trim()}?.each { sealNumber, sealGroup ->
				if(!SNs.contains(util.substring(sealNumber,1,12))){
...
//						}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('BEDBATHYUSEN', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
