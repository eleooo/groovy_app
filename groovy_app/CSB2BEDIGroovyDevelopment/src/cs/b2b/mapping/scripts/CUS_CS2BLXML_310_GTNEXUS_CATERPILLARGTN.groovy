 ...
 */
public class CUS_CS2BLXML_310_GTNEXUS_CATERPILLARGTN {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				filteredFreightCharge?.findAll{it.ChargeType == blUtil.PREPAID}?.each{current_FreightCharge ->
					sum1 = sum1 + new BigDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString())
				}
				...
				}else if(sum1.multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP) > 0){
					'E193_07' (sum1.multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP)).toString();
				}else{
					'E193_07' '0'
				}
				...
				'Loop_L1' {
					'L1' {
						...
						if(current_FreightCharge?.FreightRate){
							def vfreightRate = (new BigDecimal(current_FreightCharge?.FreightRate).setScale(2, BigDecimal.ROUND_HALF_UP))?.abs()?.toString()
							'E60_02' vfreightRate
						}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bl?.Body?.eachWithIndex{current_Body, current_BodyIndex ->
			...
			List<String> acceptChargeCodeList = ['BUC', 'OCE', 'Ocean Freight', 'PSS']
			List<String> acceptCountryCodeList = ['US']
			blUtil.filterCATERPILLARGTN(filteredFreightCharge, filteredFreightChargeCNTR, current_Body?.Route?.POR?.CSStandardCity?.CSCountryCode, current_Body?.Route?.FirstPOL?.Port?.CSCountryCode, current_Body?.Route?.LastPOD?.Port?.CSCountryCode, current_Body?.Route?.FND?.CSStandardCity?.CSCountryCode, acceptChargeCodeList, acceptCountryCodeList)
	...
	void pospValidation(cs.b2b.core.mapping.bean.bl.Body current_Body,String outputXml, List<Map<String,String>> errorKeyList){


		XmlParser xmlParser = new XmlParser();
		Node node = xmlParser.parseText(outputXml + "</T310>")
		blUtil.checkN722(current_Body, node,  true, 'Cannot convert ContainerSizeType', errorKeyList)
		blUtil.checkMissingL1_2CATERPILLARGTN(node,  false, 'No LoopL1.2 in EDI to customer', errorKeyList)
		blUtil.checkMISSING_SAP_IOR(node,  true, null, errorKeyList)
		blUtil.checkH1_FlashPoint(node,  true, null, errorKeyList)
		blUtil.checkChargeNotUSDCATERPILLARGTN(node,  true, 'Charge currency should only be USD', errorKeyList)
	}
