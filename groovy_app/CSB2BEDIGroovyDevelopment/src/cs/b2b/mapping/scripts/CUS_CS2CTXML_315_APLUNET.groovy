...
/**
 * @author Neptune
 * APLUNET CT on 20170710
 */
public class CUS_CS2CTXML_315_APLUNET {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('APLUNET', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		...
		outXml.'Loop_ST' {
			...
			if ((R403D || R404D) && !['CS160', 'CS120'].contains(vCS1EventFirst5)) {
				'Loop_R4' {
					...
					//For Tibco not  get the lastOceanleg but from last one podArrivalDtA/E!="" ;Fix this Bug :B2BSCR20170224003725
					def oceanLegArr = current_Body?.Route?.OceanLeg?.findAll {
					...
					def oceanLeg
					if(oceanLegArr.size()>0)oceanLeg = oceanLegArr[-1];
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('APLUNET', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
		...
		result=postProcess(result,"HO CHI MINH \\(CAT LAI\\)","HO CHI MINH","ReplaceAll");
		result=postProcess(result,"VNCLI","VNSGN","ReplaceAll");
		return result;
	}
