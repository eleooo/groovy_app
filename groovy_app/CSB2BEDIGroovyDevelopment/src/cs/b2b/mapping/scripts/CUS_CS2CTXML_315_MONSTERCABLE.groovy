...
/**
 * @author AmyZhou
 * MONSTERCABLE CT initialize on 201700706
 */
public class CUS_CS2CTXML_315_MONSTERCABLE {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml, Map<String,String> pospVerifyFlags ,String tp_id) {
        ...
        def vCS1EventCodeConversion = util.getConversion('MONSTERCABLE', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        outXml.'Loop_ST' {
            ...
            current_Body?.BLGeneralInfo?.each { current_BLGeneralInfo ->
                def N902 = ctUtil.emptyToString(current_BLGeneralInfo?.BLNumber)
                if (!N902.startsWith(SCAC)) {
                    N902 = SCAC + N902
                }
                if (!individualN94BM.contains(util.substring(N902?.trim(), 1, 30)) && N9SegmentsLimitation > 0) {
            ...
            def legalPartyType = ['SHP', 'FWD', 'CGN', 'NPT', 'CCP','BPT']
            def individualN94F = []
            ...
            def individualN94SN = []
            def MapSealType = ['CA','HS','TM']
            current_Body?.Container?.Seal?.each { current_Seal ->
                if (MapSealType.contains(current_Seal?.SealType) && util.isNotEmpty(current_Seal?.SealNumber) && !individualN94SN.contains(util.substring(current_Seal?.SealNumber?.trim(), 1, 30)) && N9SegmentsLimitation>0) {
...
//        }
    ...
    String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
        ...
        bodies.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getConversion('MONSTERCABLE', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
...
//                String beforeMappingOutxml=writer.toString()
    ...
    public List<Map<String, String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
        ...
        def FilterEventNotSentToCustomer_eventList = ['CS010', 'CS070', 'CS080', 'CS110', 'CS180']
        if (FilterEventNotSentToCustomer_eventList.contains(eventCS2Cde)) {
            ctUtil.filterEventNotSentToCustomer(eventCS2Cde, ['MONSTER CABLE'], current_Body, false, '-Event not for customer MONSTER CABLE', errorKeyList)
        }
        ...
        //FilterEventNotSentToCustomer  This prep exception is not work in Tibco1.0, so Groovy 1.5 doesn't add it.
