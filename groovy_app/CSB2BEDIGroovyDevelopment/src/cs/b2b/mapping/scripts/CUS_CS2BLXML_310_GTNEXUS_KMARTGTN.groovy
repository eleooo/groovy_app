...
/**
 * @author katerina
 * @pattern after TP=FRIESLANDGTN, WSIGTN
 */

public class CUS_CS2BLXML_310_GTNEXUS_KMARTGTN {
		...
		outXml.'Loop_ST' {
			...
			if(firstOceanLeg){
				'V1' {
					...
					if(firstOceanLeg?.SVVD?.Loading?.Vessel){
						'E26_03' firstOceanLeg?.SVVD?.Loading?.Vessel
					}
			...
			filteredFreightCharge?.findAll{it?.ChargeType == blUtil.COLLECT}?.eachWithIndex{current_FreightCharge, FreightChargeIndex->
				'Loop_L1' {
					'L1' {
