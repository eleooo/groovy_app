 ...
 * 
 * @author YINEM
 * @pattern after MITSUISOKOTRADE
 */
public class CUS_CS2BCXML_301_BDPGLOBAL {
		...
		outXml.'Loop_ST' {
			...
			'B1' {
				...
				if (RefNumList.size() > 0 && RefNumList[-1].ReferenceNumber) {
					 SID145 =  RefNumList[-1].ReferenceNumber.trim()
				}
				if(util.isNotEmpty(def145) &&(current_Body?.GeneralInformation?.BookingStatus=='Declined'||current_Body?.GeneralInformation?.BookingStatus=='Rejected')){
					'E145_02' util.substring(def145, 1, 30)
				}
				else if(util.isNotEmpty(SID145)){
					'E145_02' util.substring(SID145, 1, 30)
				}
				else{
					'E145_02' 'N/A'
				}
				...
				if (N1Map.get(current_Party.PartyType) && current_Party.PartyName) {
					...
					'Loop_N1' {
						'N1' {
							...
							def ccc67 = util.getConversionByTpIdMsgTypeDirFmtScac(TP_ID, MSG_TYPE_ID, DIR_ID, 'X.12', 'CarrierCustCode',  current_Body.GeneralInformation?.SCACCode, current_Party.CarrierCustomerCode, conn)
							if(ccc67)
							'E67_04' ccc67.trim()
							else
	...
	void prepValidation(Body current_Body, List<Map<String,String>> errorKeyList){
		...
		bcUtil.checkOnlyNewAllowSend(current_Body, false, 'Obsolete BC for all Booking Amendment', errorKeyList)
	}
