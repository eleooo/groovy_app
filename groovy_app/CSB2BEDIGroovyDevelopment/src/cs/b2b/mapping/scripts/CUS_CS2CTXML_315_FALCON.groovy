...
/**
 * @author LIYU
 * FALCON CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_FALCON {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('FALCON','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('FALCON', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		//def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('FALCON', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('FALCON','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
			...
			list.each{current_Party ->
				...
				if ( (customerType == 'CN' || customerType == 'SH') && N9_02) {
					'N9'{
						//						'E128_01' getLegalPartyCodeTypeExtCDE('FALCON','INTCTXML','315','O','LegalPartyCodeType', customerType,conn)
						if (customerType == 'CN') {
							'E128_01' '4E'
			...
			// hard code, when int_cde = CS190D or CS160D (Duplicate), no Q2 Output  
			if (vCS1Event != 'CS190D' && vCS1Event != 'CS160D')
			if(CSSTDQ2ExtCDE ){
//			if(CSSTDQ2ExtCDE){
				if (shipDir == 'I') {
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('FALCON', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
