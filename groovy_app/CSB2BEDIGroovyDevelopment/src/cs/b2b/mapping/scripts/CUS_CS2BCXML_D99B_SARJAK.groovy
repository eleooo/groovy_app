...
/**
 * Copy from CUS_CS2BCXML_D99B_DUMMY on 2017-07-12, changes:
 *    1, No support reject / declined : generateBody4RejectCase
 *    2, Support more 2 BC status for normal mapping: Pending / Terminated
 *    3, RFF+BL is different
 *    4, difference remarks with : DUMMY mapping - xxxxx
 *    5, add pre-validation: CheckExternalRefSystemRefNumForNonRejected
 * 
 * @author LIANGDA
 *
 */

public class CUS_CS2BCXML_D99B_SARJAK {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		outXml.'Group_UNH' {
			...
			'BGM' {
				...
				'C106_02' {
					//DUMMY mapping - different, get from description
					'E1004_01' util.substring(current_Body.ExternalReference.find { it.ReferenceDescription?.toUpperCase() == 'SYSTEM REFERENCE NUMBER'}?.ReferenceNumber?.trim(), 1, 35)
					'E1056_02' ''
					'E1060_03' ''
				}
				//DUMMY mapping - add 'Terminated':1
				def statusMap = ['Confirmed':'9', 'Cancelled':'1', 'Declined':'5', 'No Show':'NS', 'Pending':'PE', 'Rejected':'48', 'Terminated':1]
			...
			//DUMMY mapping - remove this rule
...
//			}
			...
			//Group_2
			//DUMMY mapping - code conversion change to use ReferenceType 
			def extRefTypes = ['BN':'BN', 'BILL OF LADING NUMBER':'BM', 'CONTRACT NUMBER':'CT', 'CUSTOMER REFERENCE NUMBER':'CR', 'FORWARDER REFERENCE NUMBER':'FN', 'INVOICE  NUMBER':'IV', 'MUTUALLY DEFINED REFERENCE':'ZZ', 'PURCHASE ORDER':'PO', 'SID (SHIPPER\'S IDENTIFYING NUMBER FOR SHIPMENT)':'SI', 'TARIFF NUMBER':'AFG']
					...
					if (convertVal) {
						'Group2_RFF' {
							'RFF' {
								'C506_01' {
									'E1153_01' convertVal
									def val = current_ExtRef.ReferenceNumber?.trim()
									if (convertVal=='BM') {
										String scac = current_Body.GeneralInformation?.SCACCode
										if (val && scac && !(val?.startsWith(scac))) {
											val = scac + val?.trim()
										}
									}
									'E1154_02' util.substring(val, 1, 35)
								}
							}
						}
					}
			...
			//DUMMY mapping - add BM 
			//if no BLNumber found
			//   --- it's use B/L REFERENCE NUMBER, while cde_ref use BILL OF LADING NUMBER, it's not match, 1.0 defect here 
//			def blData = current_Body.ExternalReference?.find{it.ReferenceDescription?.toUpperCase() == 'B/L REFERENCE NUMBER'}
//			if (blData==null && current_Body.GeneralInformation?.CarrierBookingNumber) {
//				'Group2_RFF' {
//					'RFF' {
//						'C506_01' {
//							'E1153_01' 'BM'
//							def val = current_Body.GeneralInformation?.CarrierBookingNumber
//							if (val && current_Body.GeneralInformation?.SCACCode == 'OOLU' && !(val?.startsWith('OOLU'))) {
//								val = 'OOLU' + val?.trim()
//							} else if (val) {
//								val = val?.trim()
//							}
...
//			}
	...
	void prepValidation(Body current_Body, int bodyLoopIndex, List<Map<String,String>> errorKeyList) {
		...
		allowStatusList.add("Pending")
		allowStatusList.add("Terminated")
		...
		//2. CheckExternalRefSystemRefNumForNonRejected -- Data not needed as it has not SYSTEM REFERENCE NUMBER.
		bcUtil.checkExternalRefSystemRefNumForNonRejected(current_Body, false, null, errorKeyList)
		
		//3. carrier booking number is mandatory
		bcUtil.checkCarrierBookingNumber(current_Body, true, "Missing mandatory Carrier Booking Number.", errorKeyList)
		
		//4. check if OOLU reject bc has Shipper Ref or not
		bcUtil.checkShipperRefNonNullForOOLUReject(current_Body, true, null, errorKeyList)
		
		//5. CheckExternalRefCSBKGForRejected
