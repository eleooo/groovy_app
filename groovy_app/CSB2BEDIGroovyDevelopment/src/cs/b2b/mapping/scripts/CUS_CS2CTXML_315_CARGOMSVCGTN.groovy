 ...
 */
public class CUS_CS2CTXML_315_CARGOMSVCGTN {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml, String tp_id) {
        ...
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //for CustomizationRule(SyncLocation), when CS1Event in following events, B406 will be replaced by R403
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        outXml.'Loop_ST' {
            ...
            'B4' {
                ...
                def B404DT = current_Body?.Event?.EventDT?.GMT
                if (current_Body?.Event?.EventDT?.LocDT) {
                    B404DT = current_Body?.Event?.EventDT?.LocDT
                }
                'E373_04' util.convertXmlDateTime(B404DT, yyyyMMdd)
                'E161_05' util.convertXmlDateTime(B404DT, HHmm)
                ...
                if (B406ToB411Flag && current_Body?.Event?.Location?.LocationCode?.UNLocationCode == "VNCLI")
                    B411 = 'Ho Chi Minh'
            ...
            def N9SegmentsLimitation = 30
                current_Body?.BLGeneralInfo?.each { current_BLGeneralInfo ->
                    ...
                    if (util.isNotEmpty(N902) && N9SegmentsLimitation > 0) {
                        'N9' {
                            'E128_01' 'BM'
                ...
                def legalPartyType = ['SHP', 'FWD', 'CGN', 'NPT', 'CCP']
                def individualN94F = []
                ...
                //no individualN94EQ berify in 1.0 app, but Container maxoccur="1"
                def individualN94EQ = []
                ...
                current_Body?.Container?.Seal?.each { current_Seal ->
                    if (util.isNotEmpty(current_Seal?.SealNumber) && !individualN94SN.contains(util.substring(current_Seal?.SealNumber?.trim(), 1, 30)) && N9SegmentsLimitation > 0) {
                        def SealNum = ctUtil.ReplaceSpecialChar(current_Seal?.SealNumber)
                        //1.0 config common prep rule 'RemoveElementFromSealNumber'
                        if (util.isNotEmpty(SealNum) && SealNum != "N/A" && ["CA","TM","HS"].contains(current_Seal?.SealType?.trim())) {   //avoid outputing N9*SN~ or N9*SN*N/A
                            'N9' {
                                'E128_01' 'SN'
            ...
            current_Body?.Route?.OceanLeg[0]?.POL?.DepartureDT?.find { it.attr_Indicator == 'A' }?.each {
                def N902TT = ctUtil.getFacilityCodeQuery(current_Body?.Route?.LastPOD?.Facility?.FacilityCode, 'FacilityCode', 'O', 'CT', 'GTNEXUS', 'X.12', conn)?.trim()
                if (N9SegmentsLimitation > 0 && N902TT) {
                    'N9' {
                        'E128_01' 'TT'
                        'E127_02' N902TT
                    }
...
//===============================================Q2 START=======================================================
            ...
            if (shipDir == 'I') {
                'Q2' {
                    ...
                    if (util.substring((lastOceanLeg?.SVVD?.Discharge?.Voyage ? lastOceanLeg?.SVVD?.Discharge?.Voyage?.trim() : '') + (lastOceanLeg?.SVVD?.Discharge?.Direction ? lastOceanLeg?.SVVD?.Discharge?.Direction?.trim() : ""), 1, 10)) {
                        'E55_09' util.substring((lastOceanLeg?.SVVD?.Discharge?.Voyage ? lastOceanLeg?.SVVD?.Discharge?.Voyage?.trim() : '') + (lastOceanLeg?.SVVD?.Discharge?.Direction ? lastOceanLeg?.SVVD?.Discharge?.Direction?.trim() : ""), 1, 10)
                    }
...
//                    }
                ...
                'Q2' {
                    ...
                    if (util.substring((firstOceanLeg?.SVVD?.Loading?.Voyage ? firstOceanLeg?.SVVD?.Loading?.Voyage?.trim() : '') + (firstOceanLeg?.SVVD?.Loading?.Direction ? firstOceanLeg?.SVVD?.Loading?.Direction : ""), 1, 10)) {
                        'E55_09' util.substring((firstOceanLeg?.SVVD?.Loading?.Voyage ? firstOceanLeg?.SVVD?.Loading?.Voyage?.trim() : '') + (firstOceanLeg?.SVVD?.Loading?.Direction ? firstOceanLeg?.SVVD?.Loading?.Direction : ""), 1, 10)
                    }
...
//===============================================R4 START=======================================================
            //special logic for R403/04---------rule name: NATEDI688AA
...
//                    R4AtLeastOneDTMCheckingFlags=false
            ...
            ////////////Customization logic (RemoveR4DIfCS180AndUncdeSame) for CS180 remove R4D when R4D03=R4E03
...
//        }
    ...
    String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
        ...
        bodies.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            if (errorKeyList.size() == 0) {
                ...
                //main mapping
                generateBody(current_Body, outXml, TP_ID)
                outputBodyCount++
            }
    ...
    public List<Map<String, String>> prepValidation(Body current_Body,
                                                    def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
        List<Map<String, String>> errorKeyList = new ArrayList<Map<String, String>>();

        def querCarrCntrSizeType = util.getConversionByTpIdDirFmtScac('GTNEXUS', 'O', 'X.12', 'ContainerType', current_Body?.GeneralInformation?.SCAC, current_Body?.Container?.CarrCntrSizeType, conn)
        def queryCSContainerSizeType = util.getConversionByTpIdDirFmtScac('GTNEXUS', 'O', 'X.12', 'ContainerType_CSType', current_Body?.GeneralInformation?.SCAC, current_Body?.Container?.CSContainerSizeType, conn)
        if (!["CS140", "CS220"].contains(eventCS2Cde)) {
            ctUtil.missingEventStatusCode(eventCS2Cde, eventExtCde, true, null, errorKeyList)
        }
        if (!(querCarrCntrSizeType || queryCSContainerSizeType)) {
        ...
        current_Body?.Party?.each { current_party ->
            if (current_party?.PartyLevel != "BL" && !current_party?.CarrierCustomerCode && current_party?.PartyLevel) {
                current_party?.PartyLevel = "NBL"
            }
        }
            ctUtil.MissingCarrierCustomerCodeWithoutPartyLevel(eventCS2Cde, current_Body?.Party, "NBL", ['SHP', 'FWD', 'CGN'], true, null, errorKeyList)
        ...
        ctUtil.filterContainerNumberMinLengthForB40708Pair(eventCS2Cde, current_Body?.Container, 10, true, "Container Number Length Less Than 10", errorKeyList)

        return errorKeyList;
    }

    public List<Map<String, String>> pospValidation(
            def eventCS2Cde, List<Map<String, String>> errorKeyList, Map<String, String> pospVerifyFlags) {
