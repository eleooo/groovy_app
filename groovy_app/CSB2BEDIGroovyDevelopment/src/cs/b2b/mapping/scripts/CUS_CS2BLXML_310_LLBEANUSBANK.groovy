...
/**
* @author RENGA
* @pattern after TP=POLOUSBANK
*/
public class CUS_CS2BLXML_310_LLBEANUSBANK {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			Map<String, String> IBTrafficModeN902Map = ['FCL':'CL', 'LCL':'LCL']
			Map<String, String> IBTrafficModeN903Map = ['FCL':'FC', 'LCL':'PC']
			if(IBTrafficModeN902Map.containsKey(current_Body?.GeneralInformation?.TrafficMode?.InBound)){
				'N9' {
					'E128_01' '6P'
					'E127_02' IBTrafficModeN902Map.get(current_Body?.GeneralInformation?.TrafficMode?.InBound)
					'E369_03' IBTrafficModeN903Map.get(current_Body?.GeneralInformation?.TrafficMode?.InBound)
				}
			}
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex , List<String> expectedCalculateList){
		...
		blUtil.NonUSinPOD(false, null, errorKeyList, current_Body)
