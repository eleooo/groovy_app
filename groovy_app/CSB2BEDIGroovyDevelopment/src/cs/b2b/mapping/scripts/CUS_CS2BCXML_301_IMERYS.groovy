...
import cs.b2b.core.mapping.bean.bc.DnD
...
/**
 * @author RENGA
 * @change NATEDI1401c-1 http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=29876
 * @change NATEDI1401c-2 http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=31270
 */
public class CUS_CS2BCXML_301_IMERYS {
	...
	Map bookingStatusMap = ['Cancelled':'D','Declined':'D']	//NATEDI1401c-1 
	
	Map<String,String> referenceTypeMap = ['BL':'BM','INV':'IK', 'CR':'CR', 'FR':'FN', 'PO':'PO', 'SC':'CT', 'SID':'SI', 'TARIF':'TS']
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		outXml.'Loop_ST' {
			...
			'B1' {
				...
				if (RefNumList.size() > 0 && RefNumList[-1].ReferenceNumber && RefNumList[-1].ReferenceNumber!='EDI Booking Confirmation' ){
					'E145_02' util.substring(RefNumList[-1].ReferenceNumber, 1, 30)
				} else if(StringUtil.isNotEmpty(current_Body?.AssociatedExternalReference?.find{it.CSReferenceType == 'OTH' && it.EDIReferenceType == '50'}?.ReferenceNumber)){	//NATEDI1401c-1
					'E145_02' util.substring(current_Body?.AssociatedExternalReference?.find{it.CSReferenceType == 'OTH' && it.EDIReferenceType == '50'}?.ReferenceNumber, 1, 30)
				}
				//NATEDI1401c-1
				if(StringUtil.isNotEmpty(current_Body?.GeneralInformation?.BookingStatusDT?.LocDT?.LocDT)) {
					'E373_03' util.convertXmlDateTime(current_Body?.GeneralInformation?.BookingStatusDT?.LocDT, yyyyMMdd)
				}else if(StringUtil.isNotEmpty(current_Body?.GeneralInformation?.BookingStatusDT?.GMT)){
					'E373_03' util.convertXmlDateTime(current_Body?.GeneralInformation?.BookingStatusDT?.GMT, yyyyMMdd)
				}
				//NATEDI1401c-1
				if(bookingStatusMap.get(current_Body?.GeneralInformation?.BookingStatus)){
					'E558_04' bookingStatusMap.get(current_Body?.GeneralInformation?.BookingStatus)
				}else if(current_Body?.EventInformation?.EventDescription == 'NEW'){
					'E558_04' 'A'	// 	NATEDI1401c-2
				}else{
					'E558_04' 'U'
				}
			...
			'Y3' {
				...
				List<FullReturn> fullreturnList = current_Body.ContainerGroup?.ContainerFlowInstruction?.FullReturn
				if(fullreturnList!=null && fullreturnList.size()>0 && fullreturnList.get(0)?.Facility?.FacilityName){
					'E112_06' util.substring(fullreturnList.get(0).Facility.FacilityName.trim(),1,14).trim()
				}
			...
			emptyCntrList.eachWithIndex { current_EmptyPickup, idx ->
				...
				'Loop_Y4' {
					'Y4' {
						...
						'E24_06' defConvType
						if(current_EmptyPickup.Facility?.FacilityCode?.trim()){
							'E309_08' 'ZZ'
							'E310_09' current_EmptyPickup.Facility.FacilityCode
						}
			...
			'Loop_R4' {
				...
				//NATEDI1401c-1
				List<DnD> demDnD = current_Body?.DnD?.findAll{it?.attr_Type == 'Demurrage' && it?.DemFreeReceivedDT?.LocDT}
				if (demDnD?.size() > 0) {
					Date earliestDemFreeReceivedDT = null;
					demDnD?.each {current_DnD ->
						if(earliestDemFreeReceivedDT == null){
							earliestDemFreeReceivedDT = util.sdfXmlFmt.parse(current_DnD?.DemFreeReceivedDT?.LocDT?.LocDT)
						}else{
							Date date = util.sdfXmlFmt.parse(current_DnD?.DemFreeReceivedDT?.LocDT?.LocDT)
							if(date.before(earliestDemFreeReceivedDT)){
								earliestDemFreeReceivedDT = date
							}
						}
					}
					'DTM' {
						'E374_01' '635'
						'E373_02' earliestDemFreeReceivedDT.format(yyyyMMdd)
						'E337_03' earliestDemFreeReceivedDT.format(HHmm)
					}
				}
			}
			//loopType = 'POL'
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bc?.Body?.eachWithIndex{current_Body, current_BodyIndex ->
			...
			//prep checking
			 prepValidation(current_Body, errorKeyList)			
	...
	void prepValidation(Body current_Body, List<Map<String,String>> errorKeyList){
				
				bcUtil.pendingStatusChecking(current_Body, false, null, errorKeyList)		
			}
