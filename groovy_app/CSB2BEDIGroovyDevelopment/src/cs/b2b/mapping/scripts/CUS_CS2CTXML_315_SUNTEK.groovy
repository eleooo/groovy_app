...
/**
 * @author Amy
 * SUNTEK CT initialize on 20170616
 */
public class CUS_CS2CTXML_315_SUNTEK {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getConversion('SUNTEK', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		def shipDir =  null 		//get the first 5 char, in case duplicate event missing direction.
		if(util.getConversion('SUNTEK','CT', 'O','EventDirection',vCS1EventFirst5, conn)){ 
			shipDir = util.getConversion('SUNTEK','CT', 'O','EventDirection',vCS1EventFirst5, conn)
		 ...
		 def ContainerSizeType = util.getConversion('SUNTEK','CT','O','ContainerType',current_Body?.Container?.CarrCntrSizeType,conn)
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
		...
		ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('SUNTEK', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
