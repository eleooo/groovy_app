...
/**
 * copy from CUS_CS2BCXML_XML_DUMMYBCXML on June 29, 2017, 
 *     -- difference
 *     -- Carrier Party is different, details: LegalParties, 'PartyName' 'COSU' for OOLU
 * 
 *     -- Remarks 1, 1.0 defect fixed: ReeferSettings xxIndicator charging, 1.0 use exists, now fixed.
 *     -- Remarks 2, 1.0 missing ContainerMovement if no party / address, now present ContainerMovement even not details info, as need provide cntr number and sizeType
 *     
 * @author LIANGDA
 * @change liangda B2BSCR20170626004090c remove ApprovalCode http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=31412
 * 
 */
public class CUS_CS2BCXML_XML_OMLOG {
...
//												}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			String outputFileName = TP_ID + "_BC_" + MSG_REQ_ID + ".xml"
