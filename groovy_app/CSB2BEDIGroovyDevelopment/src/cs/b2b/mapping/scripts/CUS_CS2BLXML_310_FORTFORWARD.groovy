...
/**
* @author XIEMI
* @pattern after TP=UNIPAC
*/
public class CUS_CS2BLXML_310_FORTFORWARD {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				//new fix
				List<FreightCharge> filteredFreightCharge0 = filteredFreightCharge?.findAll{current_FreightCharge -> current_FreightCharge?.ChargeType?.equals(blUtil.COLLECT)};
				List<FreightCharge> filteredFreightCharge1 = filteredFreightCharge?.findAll{current_FreightCharge -> current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)};
			...
			current_Body?.CarrierRate?.each{current_CarrierRate ->
					...
					if(vItemCde){
						if(N9PositionIndex < 15){
							...
							'N9' {
								'E128_01' vItemCde
								String CarrierRateNumber = current_CarrierRate?.CarrierRateNumber
								'E127_02' CarrierRateNumber?.length() < 35 ? CarrierRateNumber : CarrierRateNumber?.substring(0,35)
								if(CarrierRateNumber?.length() > 35){
									
									'E369_03' CarrierRateNumber.substring(35, CarrierRateNumber.length())
								}
					...
					if(vItemCde ){
						if(N9PositionIndex < 15){
							...
							'N9' {
								...
								'E127_02' ReferenceNumber?.length() < 35 ? ReferenceNumber : ReferenceNumber?.substring(0,35)
								if(ReferenceNumber?.length() > 35){	
									'E369_03' ReferenceNumber.substring(35, ReferenceNumber.length())  //old 36
								}
							}
						}
					}
			...
			//loop by Loading LoopOceanLegRtnF
			if(firstOceanLeg){
				'V1' {
					if(firstOceanLeg?.SVVD?.Loading?.LloydsNumber){
						...
						String CallSign = firstOceanLeg?.SVVD?.Loading?.CallSign
						'E597_01' CallSign?.length() < 8? CallSign : CallSign.substring(0,8)
					}
			...
			//loop by Discharge	LoopOceanLegRtnL
			if(lastOceanLeg){
				'V1' {
					if(lastOceanLeg?.SVVD?.Discharge?.LloydsNumber){
						...
						String CallSign = lastOceanLeg?.SVVD?.Discharge?.CallSign
						'E597_01' CallSign?.length() < 8? CallSign : CallSign?.substring(0,8);
					}
									...
									if(current_DGCargoSpec?.UNNumber){
										'H1' {
											...
											if(StringUtil.isNotEmpty(current_DGCargoSpec?.FlashPoint?.Temperature)){
												def temperature = util.substring(current_DGCargoSpec?.FlashPoint?.Temperature, 1, 3)
...
//						  vWeight = new BigDecimal(vWeight).setScale(1,BigDecimal.ROUND_HALF_UP).toString()
					}
					'E81_01'  vWeight
					'E187_02' 'G'
...
//						  vWeight = new BigDecimal(vWeight).setScale(1,BigDecimal.ROUND_HALF_UP).toString()  
				...
				if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
					...
					if(current_Body?.GeneralInformation?.BLNetWeight?.WeightUnit == 'TON'){
						...
						///////////////////   Loop_L1-start fixing
			int L1_Occurs = 0;
			filteredFreightCharge?.eachWithIndex{current_FreightCharge, FreightChargeIndex->
				if(current_FreightCharge?.ChargeType?.equals(blUtil.COLLECT) || current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)){
					if(current_FreightCharge?.TotalAmtInPmtCurrency){
						'Loop_L1' {
							'L1' {
								L1_Occurs = L1_Occurs+ 1;
								'E213_01'  L1_Occurs
								...
								if(calculateMethod!=null && calculateMethod!=""){
									...
									if(calculateMethod?.toUpperCase() == 'CONTAINER' || calculateMethod.charAt(0) == '2' || calculateMethod.charAt(0) == '4'){//
										...
										'E122_03' 'WM' //
									}else if(calculateMethod?.equals('LUMP SUM')){  //
										'E122_03' 'LS'
								...
								if(current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)){  //1
									if(current_FreightCharge?.TotalAmtInPmtCurrency){
								...
								//Element 'Loop_[1]/L1/E150_08' value min-len required (1)
...
//				blUtil.filterByApprovedForCustomer(filteredFreightCharge, filteredFreightChargeCNTR)
//				blUtil.filterByChargeType(filteredFreightCharge, filteredFreightChargeCNTR, '1')
...
//				
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkL106LengthWithPoint(node, 12, true, null, errorKeyList)  //L106's length max.
		blUtil.checkL104Length(node, 12, true, "L104's length max.12", errorKeyList)
		blUtil.checkL102L106L307Checker(node,true, null, errorKeyList)  //The Length of L307,L102,L106 is over
