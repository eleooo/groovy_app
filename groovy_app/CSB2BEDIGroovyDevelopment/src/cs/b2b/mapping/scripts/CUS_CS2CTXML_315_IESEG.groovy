...
/**
 * @author LIYU
 * IESEG CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_IESEG {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('IESEG','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('IESEG', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		//def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('IESEG', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('IESEG','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
			...
			list.each{current_Party ->
				...
				if (customerType == 'CN' || customerType == 'SH') {
					'N9'{
						//						'E128_01' getLegalPartyCodeTypeExtCDE('ASC','INTCTXML','315','O','LegalPartyCodeType', customerType,conn)
						if (customerType == 'CN' && util.substring(current_Party?.CarrierCustomerCode,1,10)?.trim()) {
							'E128_01' '4E'
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('IESEG', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
