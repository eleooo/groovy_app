...
public class CUS_CS2BLXML_310_GTNEXUS_DOWCHEMICALGTN {
			...
			if(lastOceanLeg){
				'V1' {
					...
					if(lastOceanLeg?.SVVD?.Discharge?.LloydsNumber&&lastOceanLeg?.SVVD?.Discharge?.LloydsNumber?.length()<=8){
						'E897_08' 'L'
					}else if(lastOceanLeg?.SVVD?.Discharge?.CallSign&&(lastOceanLeg?.SVVD?.Discharge?.LloydsNumber==null||lastOceanLeg?.SVVD?.Discharge?.LloydsNumber.length()==0)){
						'E897_08' 'C'
					}
				}
			}
