...
/**
 * @author Amy
 * TOPOCEAN CT initialize on 20170619
 */
public class CUS_CS2CTXML_315_TOPOCEAN {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		def vCS1EventCodeConversion = util.getCdeConversionFromIntCde('TOPOCEAN', 'CT', 'O','<QUERY-NULL-SCAC>',null, 'EventStatusCode', vCS1Event, conn)
		def vCS1EventCodeConversionWithSCAC=util.getConversionWithScac('TOPOCEAN', 'CT', 'O', 'EventStatusCode', vCS1Event, SCAC, conn)
		
		def shipDir =  null 		//get the first 5 char, in case duplicate event missing direction.
		if(util.getConversion('TOPOCEAN','CT', 'O','EventDirection',vCS1EventFirst5, conn)){ 
			shipDir = util.getConversion('TOPOCEAN','CT', 'O','EventDirection',vCS1EventFirst5, conn)
...
//===============================================N9 Start=======================================================
			...
			if(!existsBLNumber && current_Body?.BookingGeneralInfo?.find{StringUtil.isNotEmpty(it?.CarrierBookingNumber?.trim())}?.CarrierBookingNumber){
				'N9' {
					'E128_01' 'BM'
					'E127_02' util.substring(current_Body?.BookingGeneralInfo?.find{StringUtil.isNotEmpty(it?.CarrierBookingNumber?.trim())}?.CarrierBookingNumber, 1, 30)
				}
			}
			current_Body?.BookingGeneralInfo?.findAll{StringUtil.isNotEmpty(it?.CarrierBookingNumber?.trim())}?.groupBy{it?.CarrierBookingNumber?.trim()}?.each {bkNumber,current_BKGeneralInfo ->
				'N9' {
					'E128_01' 'BN'
			...
			//-------------Customization: N9RemoveDuplicateSegment-------------------------
			if(existsBLNumber){
				current_Body?.BLGeneralInfo?.findAll{StringUtil.isNotEmpty(it?.BLNumber?.trim())}?.groupBy {it?.BLNumber?.trim()}?.each {blNumber,current_blGeneralInfo->
					'N9' {
						'E128_01' 'BM'
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
		...
		ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('TOPOCEAN', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
