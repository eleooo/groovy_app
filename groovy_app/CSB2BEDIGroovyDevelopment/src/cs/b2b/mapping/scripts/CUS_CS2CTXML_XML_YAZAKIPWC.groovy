 ...
 */
class CUS_CS2CTXML_XML_YAZAKIPWC {
    ...
    public void generateBody(Body current_Body, Header ctHeader, MarkupBuilder outXml) {
       ...
       // String SCAC = current_Body?.GeneralInformation?.SCAC
        def vCS1EventCodeConversion = util.getConversionWithScac('YAZAKIPWC', 'CT', 'O', 'EventCodeType', vCS1Event,current_Body?.GeneralInformation?.SCAC, conn)
        def vCS1EventDescription = util.getConversionWithScac('YAZAKIPWC', 'CT', 'O', 'EventCodeDescType', vCS1Event,current_Body?.GeneralInformation?.SCAC, conn)
        ...
        outXml.'CargoTracking'{
            ...
            'GeneralInfo'{
                'TransactionInfo'{
                    if(ctHeader.ControlNumber){
                        ...
                        def xmlCtrlNum = util.getXMLControlNumber(TP_ID, 'CT', 'XML', 'CGOSTAT', conn)
                ...
                if(ctHeader.Action=='NEW'){
                    'Action' 'A'
                }else if(ctHeader.Action=='UPD'){
                    'Action' 'U'
                }
            ...
            'ShipmentDetails'{
                ...
                def externalRefMap = ['BLN':'BM','BN':'BN','CRN':'CR','CSO':'CRA','FN':'FN','GEN':'ZZ','INV':'IK','IT':'IT',
                                      'OTH':'ZZ','PRN':'PO','QTN':'Q1','SC':'E8','ST':'SR','SID':'SI','SO':'SO','SR':'SR','999':'ZZ']
                ...
                current_Body?.ExternalReference?.findAll {externalRefMap.containsKey(it?.CSReferenceType)}each {current_ExtRef->
                    'ExternalReferences'{
                        ...
                        'ReferenceNumber' current_ExtRef?.ReferenceNumber
                    }
                }
                ...
                current_Body?.Party?.findAll{partyTypeMap.containsKey(it?.PartyType)}?.each {current_Party ->
                    if(util.isEmpty(current_Party?.PartyLevel)){
                        'LegalParties'{
            ...
            'SummaryDetails'{
                ...
                current_Body?.MileStones?.each {current_MilesStones ->
                    def mlesStoneEventConversion =  util.getConversionWithScac('YAZAKIPWC', 'CT', 'O', 'EventCodeType', current_MilesStones?.CS1Event,current_Body?.GeneralInformation?.SCAC, conn)
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
        ...
        ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getConversionWithScac('YAZAKIPWC', 'CT', 'O', 'EventCodeType', eventCS2Cde,current_Body?.GeneralInformation?.SCAC,conn)
            //prep checking
...
//            pospValidation()
        ...
        if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
            ...
            String emailSubject = "[CargoSmart CT data ${ct?.Body[0]?.GeneralInformation?.SCAC}]"
            String outputFileName = "YAZAKIPWC_CT_" + MSG_REQ_ID + ".xml" + emailSubject
    ...
    public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
        ...
        //CHECK_MANDATORY_FUNCTIONCODE
        //CHECK_MANDATORY_LOCDETAILSDETAILS
        //CargoTrackingXML_CargoTracking_ShipmentDetails_VesselVoyageInformation_PortDetails -->only oncaen leg misssed
        //CHECK_MANDATORY_FUNCTIONCODE
        //CHECK_MANDATORY_LOCDETAILS
        //CHECK_MANDATORY_REFERENCETYPE
        //CHECK_MANDATORY_CARGO

//        ctUtil.missingCargo(eventCS2Cde,current_Body,true,null,errorKeyList)

        //CHECK_MANDATORY_MILESTONE ,type4 -ignore
//        current_Body?.MileStones?.each {current_mileStone->
//            def mlesStoneEventConversion =  util.getConversionWithScac('WGLL', 'CT', 'O', 'EventCodeType', current_mileStone?.CS1Event,current_Body?.GeneralInformation?.SCAC, conn)
//            ctUtil.missingEventStatusCode(current_mileStone?.CS1Event,mlesStoneEventConversion,true," - Cannot convert milestone event.",errorKeyList)
//        }

        //ctUtil.missingMileStonesEventCode(mileStonesEventCodevalidation,true,null,errorKeyList)
