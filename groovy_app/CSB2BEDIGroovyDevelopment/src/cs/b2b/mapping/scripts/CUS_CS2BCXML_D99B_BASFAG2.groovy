...
public class CUS_CS2BCXML_D99B_BASFAG2 {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		outXml.'Group_UNH' {
			...
			if (current_Body.GeneralInformation?.CarrierBookingNumber) {
				...
				ExternalReference extRefBKG = current_Body.ExternalReference?.find() {it.ReferenceDescription?.toUpperCase().trim() == 'BOOKING NUMBER' && it.ReferenceNumber }
				if (extRefBKG) {
					'Group2_RFF' {
						'RFF' {
							'C506_01' {
								'E1153_01' 'BN'
								'E1154_02' util.substring(extRefBKG.ReferenceNumber, 1, 35)?.trim()
							}
						}
					}
				}
			...
			if (isRejectBC && bcRejectRef) {
				...
				List<ExternalReference> extRefs = current_Body.ExternalReference?.findAll() {it.ReferenceDescription?.toUpperCase()?.trim() == 'SHIPPER REFERENCE' && it.ReferenceNumber }
				extRefs?.each { current_Reference ->
					'Group2_RFF' {
						'RFF' {
							'C506_01' {
								'E1153_01' 'SI'
