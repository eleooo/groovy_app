...
*/
public class CUS_CS2BLXML_310_TOPOCEAN {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml, Connection conn) {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				if(filteredFreightCharge?.find{it?.ChargeType == blUtil.COLLECT && it?.TotalAmtInPmtCurrency?.TotalAmtInPmtCurrency} && sum>=0){
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['BKG':'BN','CTR':'CT','ICR':'CR','PO':'PO','QUOT':'Q1','RP':'RP','SC':'E8','SID':'SI','SR':'SR']
			...
			///////////////////   C8-start
			if(!blUtil.isLCL(current_Body)){ //TOPOCEAN_FCLGrouping
			...
			///////////////////   LX-start
			//LX
			if(blUtil.isLCL(current_Body)){   //TOPOCEAN_FCLGrouping isLCL case
									...
									'W09' {
										...
										if(temperature?.contains(".")){
											...
											if(remainTempPoint !=""){
												...
												'E408_02' util.substring(blUtil.replaceZeroAfterPoint(temperatureBeforePoint?.concat(".")?.concat(remainTempPoint)),1,4)
											}else{
												if(signal.equals("-")){
													temperatureBeforePoint = "-".concat(temperatureBeforePoint)
												}
												'E408_02' util.substring(temperatureBeforePoint,1,4)
											}
										}else{
											'E408_02' util.substring(current_ReeferCargoSpec?.Temperature?.Temperature,1,4)
										}
									...
									'Loop_H1' {
										if(current_DGCargoSpec?.UNNumber){
											'H1' {
												...
												if(current_DGCargoSpec?.EmergencyContact[0]?.ContactPhone?.Number){
										...
										if(current_DGCargoSpec?.TechnicalName){
											'H2'{
												'E64_01' util.substring(current_DGCargoSpec?.TechnicalName,1,30)
			...
			//predo before TOPOCEAN_FCLGrouping isNotLCL case 
			//N7_22 the important key is CS1ContainerSizeType
			
			
			List<String> cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList = []
			Map<String,List<Integer>> N7_22NotEmptyOrginalContainerIndexMap = new HashMap()
			List<Integer> ContainerIndexListAboutn7_22isEmpty = []
			
			associateContainerAndCargo?.eachWithIndex{ current_Container, current_cargoList, Container_index ->
				def ExtCDE = util.getConversionWithScac(TP_ID, 'BL', 'O', 'ContainerType', current_Container?.CS1ContainerSizeType,current_Body?.GeneralInformation?.SCACCode, conn)
				if(current_Container?.CS1ContainerSizeType && StringUtil.isNotEmpty(ExtCDE) && StringUtil.isNotEmpty(current_Container?.ContainerNumber)){   //when the containerNumber is Empty the N7 is also null
					cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.add(ExtCDE)
				}
				if( StringUtil.isEmpty(ExtCDE) || StringUtil.isEmpty(current_Container?.ContainerNumber)){   //the case N7_22 is empty or not exist, the case ContainerNumber  empty is also this case     
					ContainerIndexListAboutn7_22isEmpty?.add(Container_index) 
				}
			}
			
			String[] distinctValues = []
			def noOfLX = cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.size()
			Set<String> set = new HashSet<String>();
			
			for (int i=0;i<noOfLX;i++) {
				set.add(cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList[i]);
			}
					
			distinctValues = set.toArray(new String[0]);
			cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList = []
			if(distinctValues?.length>0){
				distinctValues?.each{current_ExtCDE ->
					cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.add(util.getConversionWithScacByExtCde(TP_ID, 'BL', 'O', 'ContainerType', current_ExtCDE,current_Body?.GeneralInformation?.SCACCode, conn))
				}
			}
			
			associateContainerAndCargo?.eachWithIndex{ current_Container, current_cargoList, Container_index ->
				if(!N7_22NotEmptyOrginalContainerIndexMap?.keySet()?.contains(current_Container?.CS1ContainerSizeType)){
					N7_22NotEmptyOrginalContainerIndexMap.put(current_Container?.CS1ContainerSizeType, [Container_index])
				}else{
					List<Integer> l = N7_22NotEmptyOrginalContainerIndexMap?.get(current_Container?.CS1ContainerSizeType)
					l.add(Container_index)
					N7_22NotEmptyOrginalContainerIndexMap.put(current_Container?.CS1ContainerSizeType, l )
				}
			}
			...
			//LX
			//TOPOCEAN_FCLGrouping isNotLCL case
			if(!blUtil.isLCL(current_Body)){
				
				//LX
				// loop the lx about the N7_22 value is not empty
				
				cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.eachWithIndex{ current_CS1ContainerSizeType, loopindex ->
					
					def oneCS1SizeTypeGroup =   associateContainerAndCargo?.findAll{ it?.getKey()?.CS1ContainerSizeType == current_CS1ContainerSizeType}
					
					'Loop_LX'{
						
						'LX'{
							'E554_01' loopindex +1  //TOPOCEAN_FCLGrouping  LoopByDistinctContainer ReorderLX
						}
						
						//associated N7 start
						oneCS1SizeTypeGroup?.eachWithIndex{ relatedContainer, relatedCargosList, oneGroupSubIndex ->
							Container current_Container = relatedContainer
							List<Cargo> current_cargoList = relatedCargosList
							List<Container> containerGroup = current_Body.Container.findAll{it?.ContainerNumber == current_Container?.ContainerNumber && it?.ContainerCheckDigit == current_Container?.ContainerCheckDigit && it?.CS1ContainerSizeType == current_Container?.CS1ContainerSizeType}
							if(util.substring(containerGroup[0]?.ContainerNumber, 5, 5)){
								'Loop_N7' {
									if(util.substring(current_Container?.ContainerNumber, 5, 10)){
										...
										'W09' {
											...
											if(temperature?.contains(".")){
												...
												if(remainTempPoint !=""){
													...
													def result = blUtil.replaceZeroAfterPoint(util.substring(temperatureBeforePoint?.concat(".")?.concat(remainTempPoint),1,4))
													if(result?.endsWith(".")){
														result = util.substring(result,1,3)
													}
													'E408_02' result
												}else{
													if(signal.equals("-")){
						...
						oneCS1SizeTypeGroup?.eachWithIndex{ relatedContainer, relatedCargosList, oneGroupSubIndex ->
							...
							current_cargoList?.eachWithIndex{current_Cargo, cargoIndex->
								'Loop_L0' {
									'L0' {
										if(loopindex==0){                  //TOPOCEAN_FCLGrouping   process Map-Data-2
											'E213_01' N7_22NotEmptyOrginalContainerIndexMap.get(current_Container?.CS1ContainerSizeType)[countContainerOrgIndex] + 1        //TOPOCEAN_FCLGrouping  process  LoopByDistinctContainer LoopL0
										}else{
											'E213_01' (((N7_22NotEmptyOrginalContainerIndexMap.get(current_Container?.CS1ContainerSizeType)[countContainerOrgIndex] + 1) - (loopindex+1)) + 1)  //TOPOCEAN_FCLGrouping   process Map-Data-2
										}
										def groWeigthKGS = null
										def groWeigthLBS = null
										def groWeigthTON = null
							...
							countContainerOrgIndex = countContainerOrgIndex + 1
						}
						//associated L0 end
					}
				}
				...
				//LX
				// loop the lx about the N7_22 value is empty
				
				
				if(ContainerIndexListAboutn7_22isEmpty?.size>0){
					'Loop_LX'{
						'LX'{
							'E554_01' cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.size() + 1   //TOPOCEAN_FCLGrouping  ReorderLX-N722Empty
						}
						
						//associated N7 start
						associateContainerAndCargo?.eachWithIndex{ relatedContainer, relatedCargosList, oneGroupSubIndex ->
							if(ContainerIndexListAboutn7_22isEmpty?.contains(oneGroupSubIndex)){
								Container current_Container = relatedContainer
								List<Cargo> current_cargoList = relatedCargosList
								List<Container> containerGroup = current_Body.Container.findAll{it?.ContainerNumber == current_Container?.ContainerNumber && it?.ContainerCheckDigit == current_Container?.ContainerCheckDigit && it?.CS1ContainerSizeType == current_Container?.CS1ContainerSizeType}
								if(util.substring(containerGroup[0]?.ContainerNumber, 5, 5)){
						...
						associateContainerAndCargo?.eachWithIndex{ relatedContainer, relatedCargosList, oneGroupSubIndex ->
							if(ContainerIndexListAboutn7_22isEmpty?.contains(oneGroupSubIndex)){
								...
								current_cargoList?.eachWithIndex{current_Cargo, cargoIndex->
									'Loop_L0' {
										'L0' {
											'E213_01' cS1ContainerSizeTypeAboutN7_22NotEmptyDistinctList?.size() + 1  //TOPOCEAN_FCLGrouping process ReorderLX-N722Empty
											def groWeigthKGS = null
											def groWeigthLBS = null
											def groWeigthTON = null
			...
			'L3' {
				...
				if(current_Body?.Cargo?.find{StringUtil.isNotEmpty(it?.GrossWeight?.Weight)} && !blUtil.isLCL(current_Body)){ //TOPOCEAN_FCLGrouping
				...
				if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
					...
					if(current_Body?.Cargo?.findAll{StringUtil.isNotEmpty(it?.GrossWeight?.Weight)}?.size == 0 && !blUtil.isLCL(current_Body)){ //TOPOCEAN_FCLGrouping
						'E81_01'  vWeight
					}
					'E187_02' 'G'
				...
				if(sum.multiply(hundred).setScale(0,BigDecimal.ROUND_HALF_UP)>0 && !blUtil.isLCL(current_Body)){ //TOPOCEAN_FCLGrouping
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.missingChargeTOPOCEAN(current_Body, false, null, errorKeyList)
		return errorKeyList;
	}	
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkL106Length(node, 9, true, "Field length of Prepaid Charge (L1_06) is greater then 9.", errorKeyList)
		blUtil.checkL1max(node, 20, true, "L1 segment more than 20.", errorKeyList)
		blUtil.checkL0Max(node, 120, true, "L0 Max is 120 120", errorKeyList)
	...
	void syntaxValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkDTM02_DTM03_DTM_05_R(node, false, outputXml, errorKeyList)
		blUtil.checkDTM04_DTM03_C(node, false, outputXml, errorKeyList)
		blUtil.checkQTY01_M(node, false, outputXml, errorKeyList)
