...
/**
 * xml old std. for OLL
 * copy baseline from CUS_CS2BCXML_XML_OOCLLOGISTICS, and has some change
 *    -- difference for OOCLLOGISTICS:
 *    -- 1, ContainerType, default SHA will use CS1ContainerSizeType while OOCLLOGISTICS default use CSContainerSizeType
 *    -- 2, SHA has special customized logic: CustBCXMLDirectMap_OLLSHA_RefFromRemarks , add ref ZZ
 *    
 * @author liangda
 * @change renga CSEDI6637  http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=29865
 * @change liangda B2BSCR20170626004090c remove ApprovalCode http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=31412
 */
public class CUS_CS2BCXML_XML_OOCLLOGISTICSSHA {
	...
	public void generateBody(Body current_Body, Header bcHeader, MarkupBuilder outXml) {
		outXml.'Confirmation' {
			'MessageHeader' {
				...
				'MessageFunction' current_Body.GeneralInformation?.BookingStatus
					...
					'RouteInformation' {
						'EventDate' {
							...
							if (current_Body.GeneralInformation?.VGMCutOffDT?.LocDT?.LocDT) {
								'VGMCutOffDate' 'TimeZone':current_Body.GeneralInformation?.VGMCutOffDT?.LocDT?.attr_TimeZone, util.convertXmlDateTime(current_Body.GeneralInformation?.VGMCutOffDT?.LocDT?.LocDT, yyyyMMddHHmmss)
							}
						}
					...
					'EquipmentInformation' {
						current_Body.ContainerGroup?.Container?.each { current_Container ->
							'Containers' {
								...
								def defCntrType = current_Container.CS1ContainerSizeType
								HashMap<String, String> defCntrConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, '', 'XML', 'ContainerType', current_Container.CS1ContainerSizeType, conn)
								if (defCntrConvert?.get('EXT_CDE')) {
									defCntrType = defCntrConvert.get('EXT_CDE')
								}
								'ContainerType' defCntrType
						...
						'ContainerMovement' {
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup.each { current_EmptyPickup ->
								HashMap<String, String> cntrTypeConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, '', 'XML', 'ContainerType', current_EmptyPickup.ISOSizeType, conn)
								def cntrType = current_EmptyPickup.ISOSizeType
								if (cntrTypeConvert?.get("EXT_CDE")) {
									cntrType = cntrTypeConvert.get("EXT_CDE")
								}
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyReturn.each { current_EmptyReturn ->
								HashMap<String, String> cntrTypeConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, '', 'XML', 'ContainerType', current_EmptyReturn.ISOSizeType, conn) 
							...
							//FullPickup
							if (current_Body.Route?.FND) {
								'FullPickup' {
									'LocationDetails' {
										if (current_Body.Route?.FND?.Facility?.FacilityCode) {
											'FacilityCode' current_Body.Route?.FND?.Facility?.FacilityCode.trim()
										}
										if (current_Body.Route?.FND?.Facility?.FacilityName) {
											'FacilityName' util.substring(current_Body.Route?.FND?.Facility?.FacilityName.trim(), 1, 35)
										}
										if (current_Body.Route?.FND?.LocationName) {
											'Location' current_Body.Route?.FND?.LocationName?.trim()
										}
										'LocationCode' {
											if (current_Body.Route?.FND?.CityDetails?.LocationCode?.UNLocationCode) {
												'UNLocationCode' current_Body.Route?.FND?.CityDetails?.LocationCode?.UNLocationCode.trim()
											}
										}
										//end of Address
										if (current_Body.Route?.FND?.CityDetails?.City) {
											'CityName' current_Body.Route?.FND?.CityDetails?.City.trim()
										}
										if (current_Body.Route?.FND?.CityDetails?.State) {
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.FullReturn.each { current_FullReturn ->
								HashMap<String, String> cntrTypeConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, '', 'XML', 'ContainerType', current_FullReturn.ISOSizeType, conn)
						...
						//DetentionDemurrage
						current_Body.DnD?.each { current_DnD ->
							def defDndType = ''
							if (current_DnD.attr_Type=='Demurrage' && current_DnD.DemFreeReceivedDT?.GMT) {
								defDndType = 'DEM'
							} else if (current_DnD.attr_Type=='Detention' && current_DnD.FreeTimeStartDT?.GMT) {
								defDndType = 'DET'
							} else if (current_DnD.attr_Type=='Det/Dem' && current_DnD.FreeTimeStartDT?.GMT) {
								defDndType = 'COMBINED'
							}
							if (defDndType) {
								'DetentionDemurrage' {
									'Type' defDndType
									'ContainerNumber' 'CheckDigit': current_DnD.ContainerCheckDigit, current_DnD.ContainerNumber
									'EventDate' {
										 if (defDndType == 'DEM') {
											 'FreeDemurrageStartDate' 'TimeZone':'GMT', util.convertXmlDateTime(current_DnD.DemFreeReceivedDT?.GMT, yyyyMMddHHmmss)
										 } else if (defDndType == 'DET' || defDndType == 'COMBINED') {
										 	'FreeDetentionStartDate' 'TimeZone':'GMT', util.convertXmlDateTime(current_DnD.FreeTimeStartDT?.GMT, yyyyMMddHHmmss)
										 }
									}
									if (current_DnD.FreeTime) {
										'FreeTime' 'Type':current_DnD.FreeTimeType, current_DnD.FreeTime
									}
								}
							}
						}
						//end of DetentionDemurrage
					}
						...
						'CargoInformation' {
							current_Body.Cargo?.each { current_Cargo ->
								'CargoGroup' {
									...
									if (current_Cargo.NetWeight?.Weight && Double.parseDouble(current_Cargo.NetWeight?.Weight) > 0) {
									...
									if (current_Cargo.GrossWeight?.Weight && Double.parseDouble(current_Cargo.GrossWeight?.Weight) > 0) {
...
//													}
									...
									'ReeferCargo' {
										current_Cargo.ReeferCargoSpec?.each { current_ReeferCargo ->
											'ReeferCargoInfo' {
												...
												'ReeferSettings' ('CAIndicator':defCAIndicator, 'CO2':defCO2, 'GenSetType':current_ReeferCargo.GensetType, 'Humidity':defHumidity, 'O2':defO2, 'PreCooling':defPreCooling,  'ROIndicator':defROIndicator) {
													if (current_ReeferCargo.Temperature?.Temperature) {
														'Temperature' 'Units':current_ReeferCargo.Temperature?.TemperatureUnit, current_ReeferCargo.Temperature?.Temperature
													}
													if (current_ReeferCargo.Ventilation?.Ventilation) {
					...
					'ExternalReferences' {
						...
						if (refCSBkg==null && current_Body.GeneralInformation?.CSBookingRefNumber) {
							'References' {
								'ReferenceType' 'ZZ'
								'ReferenceName' 'CS Reference Number'
								'ReferenceNumber' util.substring(current_Body.GeneralInformation?.CSBookingRefNumber, 1, 35)
							}
						}
						...
						if(current_Body?.AssociatedExternalReference?.findAll{it.CSReferenceType == 'PO' && it.ReferenceNumber}?.size() > 0){
							current_Body?.AssociatedExternalReference?.findAll{it.CSReferenceType == 'PO' && it.ReferenceNumber}?.each {current_ExternalReference ->
								'References' {
									'ReferenceType' 'PO'
									if(current_ExternalReference?.ReferenceDescription) {
										'ReferenceName' current_ExternalReference?.ReferenceDescription?.trim()
									}else{
										'ReferenceName' 'Purchase Order Number'
									}
									'ReferenceNumber' util.substring(current_ExternalReference?.ReferenceNumber?.trim(), 1, 35)
								}
							}
						}else if(current_Body?.ExternalReference?.findAll{it.ReferenceDescription && it.ReferenceNumber && it?.ReferenceDescription == 'Purchase Order Number'}?.size() > 0){
							current_Body?.ExternalReference?.findAll{it.ReferenceDescription && it.ReferenceNumber && it?.ReferenceDescription == 'Purchase Order Number'}?.each {current_ExternalReference ->
								'References' {
									'ReferenceType' 'PO'
									'ReferenceName' current_ExternalReference?.ReferenceDescription?.trim()
									'ReferenceNumber' util.substring(current_ExternalReference?.ReferenceNumber?.trim(), 1, 35)
						...
						//CustBCXMLDirectMap_OLLSHA_RefFromRemarks -- OOCLLOGISTICSSHA only
						current_Body.RemarkLines?.findAll{it.attr_RemarkType=='Special' && util.isNotEmpty(it.RemarkLines)}?.each { current_RemarkLine ->
							'References' {
								'ReferenceType' 'ZZ'
								'ReferenceName' 'Special Remarks'
								'ReferenceNumber' util.substring(current_RemarkLine.RemarkLines, 1, 35)
							}
						}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			//output file name
			String outputFileName = TP_ID + "_BC_" + currentDate.format(yyyyMMddHHmmss) + fileSeqStr + ".xml"
