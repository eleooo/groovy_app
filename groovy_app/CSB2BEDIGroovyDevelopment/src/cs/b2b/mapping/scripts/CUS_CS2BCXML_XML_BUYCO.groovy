...
/**
 * xml : pattern from BCXML_Std_BUYCO and BCXML_Std
 * 
 * @author liangda init groovy migration - CSEDI5811 - http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=28650
 * 
 ...
 * 
 */
public class CUS_CS2BCXML_XML_BUYCO {
	...
	public void generateBody(Body current_Body, Header bcHeader, MarkupBuilder outXml) {
		outXml.'Confirmation' {
			'MessageHeader' {
				...
				if (current_Body.GeneralInformation?.BookingStatus == 'Wait Listed') {
					'MessageFunction' 'Pending'
				} else if (current_Body.GeneralInformation?.BookingStatus == 'Rejected') {
					'MessageFunction' 'Declined'
				} else {
						...
						parties.each { party ->
							'Party' {
								...
								if (party.CarrierCustomerCode) {
									'CarrierCustomerCode' util.substring(party.CarrierCustomerCode, 1, 14)
								}
								'PartyLocation' {
									'Address' {
...
//													}
					...
					'ExternalReferences' {
						current_Body.ExternalReference?.each { current_ExternalReference ->
							if (current_ExternalReference.CSReferenceType && current_ExternalReference.ReferenceNumber && current_ExternalReference.CSReferenceType!='SID' && current_ExternalReference.CSReferenceType!='CR') {
						...
						List<AssociatedExternalReference> listAssExtSIDs = current_Body.AssociatedExternalReference?.findAll{it.CSReferenceType == 'SID'}
						List<ExternalReference> listExtSIDs = current_Body.ExternalReference?.findAll{it.CSReferenceType == 'SID'}
						if (listAssExtSIDs!=null && listAssExtSIDs.size()>0) {
							listAssExtSIDs.each { currentAssSID ->
								'References' {
									'ReferenceType' util.getEDICdeReffromIntCde(TP_ID, 'CS2_BC_ExternalReferences', DIR_ID, 'BCCS2X',  'References', 'ReferenceType', 'SID', conn)
									if (currentAssSID.ReferenceDescription) {
										'ReferenceName' currentAssSID.ReferenceDescription.trim()
									} else {
										'ReferenceName' 'Shipper\'s Identifying Number for Shipment (SID)'
									}
									'ReferenceNumber' util.substring(currentAssSID.ReferenceNumber?.trim(), 1, 35)
								}
							}
						} else if (listExtSIDs!=null && listExtSIDs.size()>0) {
							listExtSIDs.each { currentSID ->
								'References' {
									'ReferenceType' util.getEDICdeReffromIntCde(TP_ID, 'CS2_BC_ExternalReferences', DIR_ID, 'BCCS2X',  'References', 'ReferenceType', 'SID', conn)
									if (currentSID.ReferenceDescription) {
										'ReferenceName' currentSID.ReferenceDescription.trim()
									} else {
										'ReferenceName' 'Shipper\'s Identifying Number for Shipment (SID)'
									}
									'ReferenceNumber' util.substring(currentSID.ReferenceNumber?.trim(), 1, 35)
								}
							}
						}
						
						List<AssociatedExternalReference> extAssCRs = current_Body?.AssociatedExternalReference?.findAll{it.CSReferenceType == 'CR' && it.ReferenceNumber}
						List<ExternalReference> extCRs = current_Body?.ExternalReference?.findAll{it.ReferenceNumber && it?.CSReferenceType == 'CR'}
						if(extAssCRs?.size() > 0){
							extAssCRs.each { current_ExternalReference ->
								'References' {
									'ReferenceType' 'CR'
									if (current_ExternalReference?.ReferenceDescription) {
										'ReferenceName' current_ExternalReference?.ReferenceDescription?.trim()
									} else {
										'ReferenceName' 'Customer Reference Number'
									}
									'ReferenceNumber' util.substring(current_ExternalReference?.ReferenceNumber?.trim(), 1, 35)
								}
							}
						} else if (extCRs?.size() > 0) {
							extCRs.each { current_ExternalReference ->
								'References' {
									'ReferenceType' 'CR'
									if (current_ExternalReference?.ReferenceDescription) {
										'ReferenceName' current_ExternalReference?.ReferenceDescription?.trim()
						...
						if ((listAssExtSIDs==null || listAssExtSIDs.size()==0) && (listExtSIDs==null || listExtSIDs.size()==0) && (extAssCRs==null || extAssCRs.size()==0) && (extCRs==null || extCRs.size()==0)) {
							//special logic to get from BR cross data
							def extBRRef = util.getEDICrossProcessData('BR/BC_MAEU_BUYCO', current_Body.GeneralInformation?.CSBookingRefNumber, null, conn)
							if (extBRRef) {
								'References' {
									'ReferenceType' 'SI'
									'ReferenceName' 'Shipper\'s Order Identifying Number for Shipment (SID)'
									'ReferenceNumber' util.substring(extBRRef?.trim(), 1, 35)
								}
							}
						}
					}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			//output file name
			String outputFileName = TP_ID + "_" + MSG_TYPE_ID + "_" + MSG_REQ_ID + ".xml"
...
//		bcUtil.checkCarrierBookingNumberForNonCancelled(current_Body, true, null, errorKeyList)
