...
/**
 * xml : copy from 1.0 BCXML app - BCXML_Std, similar with BCXML_BUYCO, and copy from CUS_CS2BCXML_XML_STD.groovy
 *        -- output file name is different.
 *        
 ...
 * 
 */
public class CUS_CS2BCXML_XML_DELMAR {
...
//													}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			String scac = '' 
			if (bc.Body && bc.Body.size()>0 && bc.Body[0]) {
				scac = bc.Body[0].GeneralInformation?.SCACCode
			}
			if (scac==null) {
				scac = ''
			}
			String outputFileName = TP_ID + "_" + scac + "_BCXML_" + MSG_REQ_ID + ".xml"
