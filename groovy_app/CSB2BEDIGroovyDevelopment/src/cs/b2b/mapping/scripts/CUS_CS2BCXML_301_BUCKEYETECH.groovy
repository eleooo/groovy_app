 ...
 * 
 * @author YINEM
 * @pattern after DSG_STD3
 */
public class CUS_CS2BCXML_301_BUCKEYETECH {
	...
	Map bookingStatusMap = ['Confirmed':'A','Pending':'B','Wait Listed':'B','Cancelled':'D','No Show':'D','Declined':'R']
	
	Map<String,String> referenceTypeMap = ['BL':'BM','CR':'CR', 'CTR':'CT', 'FR':'FN', 'ICR':'CR', 'INV':'IN', 'PO':'PO', 'QUOT':'Q1', 'SC':'E8', 'SID':'SI', 'SO':'SO', 'SR':'SR','TARIF':'TS']
...
//			}
			...
			current_Body?.Cargo?.eachWithIndex{ current_Cargo, index->
				'Loop_LX' {
					...
					//LoopBUCKEYETECHSpecialRemark
					def var_FaciiltyCode = ''
					def var_FullReturnDT = ''
					def var_FulCutOffDT = current_Body.DocumentInformation?.RequiredDocument?.find{it.DocumentType == 'Shipping Instruction/BL Master' && it.DueDT?.LocDT?.LocDT}?.DueDT?.LocDT?.LocDT
					current_Body.ContainerGroup?.ContainerFlowInstruction?.FullReturn?.each {current_FullReturn ->
						if(current_FullReturn.Facility?.FacilityCode){
							if(current_FullReturn.Facility.FacilityCode.toUpperCase() == 'HOU20' || current_FullReturn.Facility.FacilityCode.toUpperCase() == 'MEM21'){
								if(current_FullReturn.Facility.FacilityCode.toUpperCase() == 'HOU20')
									var_FaciiltyCode = 'BNSF'
								else
									var_FaciiltyCode = 'NS'

								var_FullReturnDT = current_FullReturn.MvmtDT?.LocDT?.LocDT
							}
						}
						if(!util.isEmpty(var_FaciiltyCode)){
							'K1' {
								'E61_01' 'Rail SCAC='+var_FaciiltyCode
								'E61_02' ''
							}

							if(!util.isEmpty(var_FullReturnDT)){
								'K1' {
									'E61_01' 'Rail cutoff'
									'E61_02' util.convertDateTime(var_FullReturnDT, "yyyy-MM-dd'T'HH:mm:ss", 'yyyyMMddHHmm')
								}
							}
							// tibco logic no empty condition checking, always output it
//							if(var_FulCutOffDT){
								'K1' {
									'E61_01' 'DOC Cutoff'
									'E61_02' util.convertDateTime(var_FulCutOffDT, "yyyy-MM-dd'T'HH:mm:ss", 'yyyyMMddHHmm')
								}
//							}
						}
					}
...
//			}
	...
	void prepValidation(Body current_Body, int bodyLoopIndex, List<Map<String,String>> errorKeyList){
		bcUtil.systemReferenceNumberM(current_Body,false,'No System Reference Number',errorKeyList)
	}
