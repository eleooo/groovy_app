...
/**
* @author XIEMI
* @pattern after TP=UCMSUNTEK, KODAKNNR
*/
public class CUS_CS2BLXML_310_UNIPAC {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				if(filteredFreightCharge0.size() > 0){
					filteredFreightCharge0.each{ current_FreightCharge->
						if(current_FreightCharge?.TotalAmtInPmtCurrency?.TotalAmtInPmtCurrency){
			...
			filteredFreightCharge?.eachWithIndex{current_FreightCharge, FreightChargeIndex->
				if(current_FreightCharge?.ChargeType?.equals(blUtil.COLLECT) || current_FreightCharge?.ChargeType?.equals(blUtil.PREPAID)){
					if(current_FreightCharge?.TotalAmtInPmtCurrency && L1_Occurs < 20){
						'Loop_L1' {
							'L1' {
...
//
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.sendStatusOfEDI(current_Body, true, null, errorKeyList)  //Missing Send Status of EDI
		...
		blUtil.filterDELinTxnAction(current_Body, true, "Delete is filtered out", errorKeyList)  //Filter DEL status in Transaction
		return errorKeyList;
	}
