...
/**
 * @author Neptune
 * GTIAPLL CT initialize on 20170711
 */
public class CUS_CS2CTXML_315_GTIAPLL {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml, Map<String,String> pospVerifyFlags) {
        ...
        def vCS1EventCodeConversion = util.getConversion('GTIAPLL', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
...
//==================================================B4 START============================================================
            'B4' {
                ...
                if (current_Body.Container?.ContainerNumber) {
                    'E207_08' util.substring(util.substring(current_Body.Container?.ContainerNumber, 5, current_Body.Container?.ContainerNumber?.length() - 4) + current_Body?.Container?.ContainerCheckDigit, 1, 10)
                }
                ...
                if (util.isNotEmpty(current_Body?.Container?.CarrCntrSizeType)) {
                    'E24_10' current_Body?.Container?.CarrCntrSizeType?.trim()
                }
                def B411
                if (util.isNotEmpty(current_Body?.Event?.Location?.CityDetails?.City)) {
                ...
                if (B411) {
                    'E310_11' util.substring(B411.trim(), 1, 30).toUpperCase()
                }
                'E309_12' 'UN'
            }
//=======================================================B4 END=========================================================
...
//=======================================================N9 START=======================================================
            ...
            def N9SegmentsLimitation=30
                ...
                current_Body?.BLGeneralInfo?.each { current_BLGeneralInfo ->
                    def BMN902 = current_BLGeneralInfo?.BLNumber
                    def IBN902 = current_BLGeneralInfo?.CustomsReferenceNumber
                    if (util.isNotEmpty(BMN902) && !individualN94BM.contains(util.substring(BMN902.trim(), 1, 30)) && N9SegmentsLimitation>0) {
                        'N9' {
                            'E128_01' 'BM'
                            'E127_02' util.substring(BMN902.trim(), 1, 30)
                            individualN94BM.add(util.substring(BMN902.trim(), 1, 30))
                        }
                        N9SegmentsLimitation--
                    }
                    if(current_BLGeneralInfo?.CustomsReferenceNumber && current_BLGeneralInfo?.CustomsReferenceType=="IT" && !individualN94IB.contains(util.substring(IBN902.trim(), 1, 30)) && N9SegmentsLimitation>0 ){
                        'N9' {
                            'E128_01' 'IB'
                            'E127_02' util.substring(IBN902.trim(), 1, 30)
                            individualN94IB.add(util.substring(IBN902.trim(), 1, 30))
                        }
                        N9SegmentsLimitation--
                    }
                }
            def legalRefType = ['PO']
            def individualN9PO = []
            current_Body?.ExternalReference?.find{legalRefType.contains(it?.CSReferenceType)}?.each {current_ExternalReference ->
                if(!individualN9PO.contains(util.substring(current_ExternalReference?.ReferenceNumber,1,50)) && current_ExternalReference?.ReferenceNumber && N9SegmentsLimitation>0){
                    'N9' {
                        'E128_01' 'PO'
                        'E127_02' util.substring(current_ExternalReference?.ReferenceNumber,1,50)
                        individualN9PO.add(util.substring(current_ExternalReference?.ReferenceNumber,1,50))
                    }
                    N9SegmentsLimitation--
                }
            }
            ...
            def legalPartyType = ['CGN']
            def individualN94F = []
            current_Body?.Party?.find{legalPartyType.contains(it?.PartyType)}?.each { current_Party ->
                if (!individualN94F.contains(util.substring(current_Party?.CarrierCustomerCode?.trim(), 1, 30)) && N9SegmentsLimitation>0 && current_Party?.CarrierCustomerCode?.trim()) {
                    'N9' {
                        'E128_01' 'IC'
                        'E127_02' util.substring(current_Party?.CarrierCustomerCode?.trim(), 1, 30)
                        'E-369_03' util.substring(current_Party?.PartyName?.trim(),1,45)
            ...
            if (shipDir == 'I') {
                'Q2' {
                    ...
                    if(lastOceanLeg?.POL?.DepartureDT?.find{it?.attr_Indicator=="A"}?.LocDT){
                        'E373_04' util.convertDateTime(lastOceanLeg?.POL?.DepartureDT?.find{it?.attr_Indicator=="A"}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
                    }else if(lastOceanLeg?.POL?.DepartureDT?.find{it?.attr_Indicator=="E"}?.LocDT){
                        'E373_04' util.convertDateTime(lastOceanLeg?.POL?.DepartureDT?.find{it?.attr_Indicator=="E"}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
                    }
                    if(lastOceanLeg?.POD?.ArrivalDT?.find{it?.attr_Indicator=="A"}?.LocDT){
                        'E373_05' util.convertDateTime(lastOceanLeg?.POD?.ArrivalDT?.find{it?.attr_Indicator=="A"}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
                    }else if(lastOceanLeg?.POD?.ArrivalDT?.find{it?.attr_Indicator=="E"}?.LocDT){
                        'E373_05' util.convertDateTime(lastOceanLeg?.POD?.ArrivalDT?.find{it?.attr_Indicator=="E"}?.LocDT, xmlDateTimeFormat, yyyyMMdd)
                    }
