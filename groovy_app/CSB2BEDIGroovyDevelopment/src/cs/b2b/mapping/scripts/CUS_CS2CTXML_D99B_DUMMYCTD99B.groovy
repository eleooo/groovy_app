 ...
 * script copy from AGILITY
 * http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=29536
 */
class CUS_CS2CTXML_D99B_DUMMYCTD99B {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml) {
        ...
        def contianerTypeConvertion = util.getConversionWithScac('DUMMYCTD99B','CT','O','ContainerType',current_Body?.Container?.CarrCntrSizeType,SCAC,conn)
        outXml.'Group_UNH' {
            'UNH' {
                'E0062_01' '-999'
                'S009_02' {
            ...
            'Group4_CNI' {
                ...
                'Group5_STS' {
                    'STS' {
                        ...
                        def queryEventConversion = util.getCdeConversionFromIntCde('DUMMYCTD99B','CT','O','<QUERY-NULL-SCAC>','EDIFACT','EventStatusCode',vCS1Event,conn)
                        'C555_02' {
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
        ...
        ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getCdeConversionFromIntCde('DUMMYCTD99B', 'CT', 'O','<QUERY-NULL-SCAC>','EDIFACT', 'EventStatusCode', eventCS2Cde, conn).get('EXT_CDE')
