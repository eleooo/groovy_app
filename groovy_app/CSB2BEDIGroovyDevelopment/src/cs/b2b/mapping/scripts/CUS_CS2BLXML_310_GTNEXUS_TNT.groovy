 ...
 */
public class CUS_CS2BLXML_310_GTNEXUS_TNT {
	...
	public void generateBody(Body current_Body, Map<cs.b2b.core.mapping.bean.bl.Container, List<cs.b2b.core.mapping.bean.bl.Cargo>> associateContainerAndCargo,List<FreightCharge> filteredFreightCharge, List<FreightChargeCNTR> filteredFreightChargeCNTR, MarkupBuilder outXml) {
		...
		outXml.'Loop_ST' {
			...
			Map<String, String> actionMap = ['NEW':'00','UPD':'53', 'DEL': '01']
			'B2A' {
			...
			Map<String, String> referenceCodeCS2Map = ['SC':'CT']
			Map<String, String> referenceRemarkCS2Map = ['SC':'CONTRACT NUMBER']
			...
			for(int i = 0; i * 30 < current_Body?.GeneralInformation?.BLNumber?.length(); i++ ) {
				...
				'N9' {
					'E128_01' 'BM'
					'E127_02' current_Body?.GeneralInformation?.BLNumber[startIndex..<endIndex]
				}
				countN9++
			}
			...
			current_Body?.Party?.each{current_Party ->
				if(partyTypeMap.get(current_Party?.PartyType)){
					'Loop_N1' {
						...
						List<String> vAddressLinesList  = blUtil.SplitTextWithConnector(vAddressLines?.trim(), 55)
						if(vAddressLinesList?.size() >= 2 && (vAddressLinesList[0] + vAddressLinesList[1]) != ''){
							addressLinesList[0] = vAddressLinesList[0] + vAddressLinesList[1]
						}else if(vAddressLinesList?.size() >= 1 && vAddressLinesList[0] != ''){
							addressLinesList[0] = vAddressLinesList[0]
						}else if(vAddressLinesList?.size() >= 4 && (vAddressLinesList[2] + vAddressLinesList[3]) != ''){
							addressLinesList[1] = vAddressLinesList[2] + vAddressLinesList[3]
						}else if(vAddressLinesList?.size() >= 3 && vAddressLinesList[2]  != ''){
							addressLinesList[1] = vAddressLinesList[2] 
						}
						if(vAddressLinesList?.size() >= 1 && vAddressLinesList[0] != ''){
							'N3' {
								'E166_01' vAddressLinesList[0]?.trim()
								if(vAddressLinesList?.size() >= 2 && vAddressLinesList[1] != ''){
									'E166_02' vAddressLinesList[1]?.trim()
								}
							}
						}
						if(vAddressLinesList?.size() >= 3 && vAddressLinesList[2] != ''){
							'N3' {
								'E166_01' vAddressLinesList[2]?.trim()
								if(vAddressLinesList?.size() >= 4 && vAddressLinesList[3] != ''){
									'E166_02' vAddressLinesList[3]?.trim()
								}
							}
						}
					}
				...
				'Loop_L1' {
					'L1' {
						...
						if(current_FreightCharge?.FreightRate){
							vfreightRate = (new BigDecimal(current_FreightCharge?.FreightRate).setScale(0, BigDecimal.ROUND_HALF_UP)).abs().toString()
						}
