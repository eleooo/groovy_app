...
*/
public class CUS_CS2BLXML_310_SARECYCLING {
		...
		outXml.'Loop_ST' {
			...
			'B3' {
				...
				filteredFreightCharge.each{current_FreightCharge ->
					if(current_FreightCharge?.TotalAmtInPmtCurrency?.toString() && util.isDecimal(current_FreightCharge?.TotalAmtInPmtCurrency?.toString())){
			...
			def vItemCde = null
			Map<String, String> referenceCodeCS2Map = ['CTR':'CT','ICR':'CR','PO':'PO',
													   'QUOT':'Q1','RP':'RP','SC':'E8','SR':'SR']
			...
			if(N9PositionIndex < 15){
				N9PositionIndex = N9PositionIndex + 1
				String N902SI = current_Body?.ExternalReference?.find{it?.CSReferenceType == "SID"}?.ReferenceNumber
				if(StringUtil.isEmpty(N902SI)){
					N902SI = current_Body?.ExternalReference?.find{it?.CSReferenceType == "SIR"}?.ReferenceNumber
				}
				'N9' {
					'E128_01' 'SI'
			...
			if(firstOceanLeg){
				if(lastOceanLeg == null && (firstOceanLeg?.SVVD?.Discharge?.VesselName || firstOceanLeg?.SVVD?.Discharge?.LloydsNumber || firstOceanLeg?.SVVD?.Discharge?.CallSign)) {
					'V1' {
						...
						if(firstOceanLeg?.SVVD?.Discharge?.VesselName){
							'E182_02' util.substring(firstOceanLeg?.SVVD?.Discharge?.VesselName, 2, 27)	//maybe a bug
						}else if(StringUtil.isEmpty(firstOceanLeg?.SVVD?.Discharge?.LloydsNumber) && StringUtil.isEmpty(firstOceanLeg?.SVVD?.Discharge?.CallSign)){
							'E182_02' 'TBD'
						}

						if(firstOceanLeg?.SVVD?.Discharge?.Voyage){
							if(firstOceanLeg?.SVVD?.Discharge?.Direction){
								'E55_04' util.substring((firstOceanLeg?.SVVD?.Discharge?.Voyage + firstOceanLeg?.SVVD?.Discharge?.Direction), 1, 4)
							}else{
								'E55_04' util.substring(firstOceanLeg?.SVVD?.Discharge?.Voyage, 1, 4)
							}
						}
					...
					'V1' {
						...
						}else if(StringUtil.isEmpty(firstOceanLeg?.SVVD?.Loading?.LloydsNumber) && StringUtil.isEmpty(firstOceanLeg?.SVVD?.Loading?.CallSign)){
							'E182_02' 'TBD'
						}
			...
			if(sumL311 != BigDecimal.ZERO ||
				...
				'L3' {
					...
					if(util.isDecimal(current_Body?.GeneralInformation?.BLGrossWeight?.Weight) && current_Body?.GeneralInformation?.BLGrossWeight?.Weight?.toBigDecimal() > 0){
						'E81_01'  blUtil.replaceZeroAfterPoint(current_Body?.GeneralInformation?.BLGrossWeight?.Weight)
						'E187_02' 'G'
						...
						'E81_01'  blUtil.replaceZeroAfterPoint(current_Body?.GeneralInformation?.BLNetWeight?.Weight)
						'E187_02' 'N'
					}
				...
				if(FreightChargeIndex < 20){
					'Loop_L1' {
						'L1' {
							...
							if(current_FreightCharge?.ChargeType == blUtil.PREPAID && current_FreightCharge?.TotalAmtInPmtCurrency?.toString() && current_FreightCharge?.TotalAmtInPmtCurrency?.toString() != "0"){
...
//				blUtil.OceanLegReorder(current_Body.Route?.OceanLeg)
	...
	List<Map<String,String>> prepValidation(cs.b2b.core.mapping.bean.bl.Body current_Body, int current_BodyIndex){
		...
		blUtil.checkCurrency(current_Body, blUtil.PREPAID, "USD", true, null, errorKeyList)
	...
	void pospValidation(String outputXml, List<Map<String,String>> errorKeyList){
		...
		blUtil.checkL102LengthWithPoint(node, 9, true, "L102,L106,L307 length is over the max length", errorKeyList)
		blUtil.checkL106LengthWithPoint(node, 12, true, "L102,L106,L307 length is over the max length", errorKeyList)
		blUtil.checkL307Length(node, 8, true, "L102,L106,L307 length is over the max length", errorKeyList)
	}
