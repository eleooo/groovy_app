...
/**
 * @author LIYU
 * UNIQUEAP CT initialize on 20170227
 * Reference WEYERHAEUSER and EFREIGHT
 */
public class CUS_CS2CTXML_315_UNIQUEAP {
	...
	public void generateBody(Body current_Body, MarkupBuilder outXml) {
		...
		//		def shipDir=getShipDir('UNIQUEAP','Q2','O','ShipDirection',vCS1EventFirst5,'EventStatusCode',conn)
		...
		def vCS1EventCodeConversion = util.getConversion('UNIQUEAP', 'CT', 'O', 'EventStatusCode', vCS1Event, conn)
		//def vCarrCntrSizeTypeCodeConversion = util.getConversionWithScac('UNIQUEAP', 'CT', 'O', 'ContainerType', vCarrCntrSizeType, 'OOLU', conn)
		...
		outXml.'Loop_ST' {
			...
			current_Body.ExternalReference?.findAll{StringUtil.isNotEmpty(it.CSReferenceType) }?.each { ExternalReference ->
				//				def ExtReferenceCode=getExtReferenceCode('UNIQUEAP','INTCTXML','315','O','ExtReferenceCode','N9','01-128',ExternalReference?.get(0).CSReferenceType,conn)
			...
			list.each{current_Party ->
				...
				if (customerType == 'CN' || customerType == 'SH') {
					...
					def aao = util.substring(current_Party?.CarrierCustomerCode,1,10)?.trim()
					if ( aao && !uniqueAAO.contains(aao) ) {
						'N9'{
							'E128_01' 'AAO'
							'E127_02' aao 
						}
			...
			CSSTDQ2ExtCDE=ctUtil.getCodeConversionbyCovertType('CT','O','CSSTDQ2',vCS1EventFirst5,conn)
			if(CSSTDQ2ExtCDE){
				if (shipDir == 'I') {
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		bodies.eachWithIndex { current_Body, current_BodyIndex ->
			...
			def eventExtCde = util.getConversion('UNIQUEAP', 'CT', 'O', 'EventStatusCode', eventCS2Cde, conn)
...
//			result = writer?.toString()
	...
	public List<Map<String,String>> prepValidation(Body current_Body, def current_BodyIndex, def eventCS2Cde, def eventExtCde) {
		...
		shipperPartyCodeNotRegistered(eventCS2Cde, current_Body.Party, false, null, errorKeyList)

		return errorKeyList;
	}
	
	//CSUpload- Shipper Party code is not registered.
	public void shipperPartyCodeNotRegistered(String eventCS2Cde, List<cs.b2b.core.mapping.bean.ct.Party> party, boolean isError, String errorMsg , List<Map<String,String>> errorKeyList){
		Map<String,String> errorKey = null
		//SELECT ext_cde FROM b2b_edi_cde_ref WHERE TP_ID = 'UNIQUEAP' and convert_type_id = 'LegalPartyCode' and seg_id='N9' and seg_num='02-127' and int_cde='SHFILTER';
		def CarrierCustomerCodeList= [
			'6409549000', '6432045000', '6456164000', '6509847000', '6519903000', '6528830000', '6537617000', '6539422000',
			'6582460000', '6584602000', '6592143000', '6601502000', '6601502001', '6631253000', '6638893000', '6642521000',
			'6650153000', '6655028000', '6656376000', '6660759000', '6660783000', '6660837000', '6996023000', '7050247000', '7050247000']
		
		def carrierCustomerCode = party?.find{current_party -> current_party.PartyType == 'SHP'}?.CarrierCustomerCode
		
		if (party?.findAll{it.PartyType == 'SHP'}?.size() == 0){
			errorKey = [TYPE: isError? 'ES' : 'EC', IS_ERROR: isError? 'YES' : 'NO', VALUE: errorMsg != null? eventCS2Cde + ' [ ' + carrierCustomerCode + ' ] ' +  errorMsg : eventCS2Cde + ' [ ' + carrierCustomerCode + ' ] ' +  '- Shipper Party code is not registered']
			errorKeyList.add(errorKey)
		}else {
			def partyList = party?.findAll{it.PartyType == 'SHP' && CarrierCustomerCodeList.contains(it.CarrierCustomerCode)}
