...
/**
 * xml : copy from 1.0 BCXML app - BCXML_Std, similar and copy from CUS_CS2BCXML_XML_DUNAVANT.groovy
 *             -- difference
 *             -- 1, Container SizeType use CS1CntrSizeType, no use SCAC for query
 *             -- 2, output file name macro is special
 *             -- 3, more Error validation for rule : CheckReeferVentilation
 *             
 *             
 ...
 * 
 */
public class CUS_CS2BCXML_XML_PACIFICBRANDS {
					...
					'EquipmentInformation' {
						current_Body.ContainerGroup?.Container?.each { current_Container ->
							'Containers' {
								...
								if (current_Container.CS1ContainerSizeType) {
									Map<String, String> defCntr = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, null, 'XML', 'ContainerType', current_Container.CS1ContainerSizeType, conn)
						...
						'ContainerMovement' {
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyPickup.each { current_EmptyPickup ->
								Map<String, String> cntrConvertTypes = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, null, 'XML', 'ContainerType', current_EmptyPickup.ISOSizeType, conn)
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.EmptyReturn.each { current_EmptyReturn ->
								Map<String, String> defCntrConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, null, 'XML', 'ContainerType', current_EmptyReturn.ISOSizeType, conn)
							...
							current_Body.ContainerGroup?.ContainerFlowInstruction?.FullReturn.each { current_FullReturn ->
								Map<String, String> cntrConverts = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, null, 'XML', 'ContainerType', current_FullReturn.ISOSizeType, conn)
							...
							current_Body.Cargo?.each { current_Cargo ->
								'CargoGroup' {
									...
									if (current_Cargo.Packaging?.PackageQty || current_Cargo.Packaging?.PackageType) {
										...
										def ptypeConvert = util.getCdeConversionFromIntCde(TP_ID, MSG_TYPE_ID, DIR_ID, null, 'XML', 'PackageUnit', current_Cargo.Packaging?.PackageType, conn)
...
//													}
	...
	String mapping(String inputXmlBody, String[] runtimeParams, Connection conn) {
		...
		if (txnErrorKeys.findAll{it.size == 0}.size != 0) {
			...
			//DUNAVANT BCXML output filename macro
			String outputFileName = TP_ID + "_301_" + fileSeqStr
	...
	void prepValidation(Body current_Body, int bodyLoopIndex, List<Map<String,String>> errorKeyList) {
		...
		//3. CheckReeferVentilation - Missing ReeferCargoSpec Ventilation or VentilationUnit.
		bcUtil.checkReeferVentilation(current_Body, true, null, errorKeyList)
		
		//4. CheckContainerGroupSealNumberMaxLength15 - E
