 ...
 * http://i2isd/sites/csisa/Lists/Workplan/DispForm.aspx?ID=31497
 */
class CUS_CS2CTXML_D99B_NBEPORTAL {
    ...
    public void generateBody(Body current_Body, MarkupBuilder outXml) {
        ...
        def contianerTypeConvertion = util.getConversionWithScac('NBEPORTAL','CT','O','ContainerType',current_Body?.Container?.CarrCntrSizeType,SCAC,conn)
        def queryEventConversion = util.getCdeConversionFromIntCde('NBEPORTAL','CT','O','<QUERY-NULL-SCAC>','EDIFACT','EventStatusCode',vCS1Event,conn)
...
//		List<Body> bodies = ctUtil.CTEventDuplication(blDupBodies, duplicatedPairs)
        ...
        ct.Body.eachWithIndex { current_Body, current_BodyIndex ->
            ...
            def eventExtCde = util.getCdeConversionFromIntCde('NBEPORTAL', 'CT', 'O','<QUERY-NULL-SCAC>','EDIFACT', 'EventStatusCode', eventCS2Cde, conn).get('EXT_CDE')
