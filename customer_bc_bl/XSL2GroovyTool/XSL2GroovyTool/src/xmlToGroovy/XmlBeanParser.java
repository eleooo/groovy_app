...
public class XmlBeanParser {
	...
	public <T> T xmlParser(String xml, Class<T> t) throws Exception {
		...
		System.out.println("xml to dom " + (end - start) + " ms");
		...
		System.out.println("dom to json " + (end - start) + " ms");
		...
		System.out.println("json to obj " + (end - start) + " ms");
					...
					if (childNode.getChildNodes().getLength() == 1 && childNode.getFirstChild().getNodeType() != org.w3c.dom.Node.ELEMENT_NODE) {
						...
						if(childNode.getChildNodes().getLength() > 0) {
							...
							if(isMultiElement(childNode, childNodeName)){
								...
								if(isLoopElementStart(childNode)){
									sb.append(SINGLE_QUOTES + childNodeName + SINGLE_QUOTES + JSON_DELIMITER + ARRAY_START);
								}
								if (hasAttr(childNode)){
									sb.append(OBJ_START);
									attrHandler(childNode, sb);
...
//		if(result == false){
//			//check full path
//			StringBuffer sb = new StringBuffer(currentNodeName);
//			result = checkFullParentNodePath(node, sb, MultiElementList); 
//		}
	...
	private boolean checkFullParentNodePath(org.w3c.dom.Node node, StringBuffer sb, Set<String> MultiElementList){
		
		String currentPath = sb.toString();
		
 		if(node.getParentNode().getNodeName().equals("#document")){
 			return false;
		}else if(MultiElementList.contains(currentPath)){
			return true;
		}else{
			sb.insert(0,removeNamespacePrefix(node.getParentNode().getNodeName()) +  ".");
			return checkFullParentNodePath(node.getParentNode(),sb, MultiElementList);
		}
